-- F-ALERT-003 / T-PG-002: bounded provider-authority reconciliation and
-- asynchronous external compensation. Expand only; rollback disables both
-- workers and retains every request, authority check and provider receipt.
BEGIN;
...
-- The legacy bootstrap omitted the partial idempotency index even though the
-- HTTP command uses it as its ON CONFLICT arbiter. Repair it additively before
-- accepting new compensation controls.
