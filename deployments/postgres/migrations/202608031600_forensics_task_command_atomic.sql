-- T-PG-002 / F-FORENSICS-001: authoritative forensic task command boundary.
-- Expand-only. Runtime services must not create or alter these objects.
BEGIN;
...
CREATE INDEX IF NOT EXISTS idx_forensics_task_outbox_ready
  ON forensics_task_outbox (available_at,occurred_at,outbox_id)
  WHERE status='pending';
CREATE INDEX IF NOT EXISTS idx_forensics_task_outbox_reclaim
  ON forensics_task_outbox (locked_until,outbox_id)
