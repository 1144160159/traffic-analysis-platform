-- F-DASHBOARD-002: provider-confirmed dashboard task compensation.
-- Expand: add compensation states, idempotent control receipts, leased attempts,
-- durable provider receipts, and two additive lifecycle event types.
-- Backfill: none. Existing terminal tasks remain unchanged and are not made
-- compensatable until they already have a confirmed execution receipt.
-- Cutover: keep DASHBOARD_TASK_COMPENSATION_V1_ENABLED=false until the same
-- candidate has passed PG/Kafka/provider shadow verification.
-- Verify: reconcile every compensating task to one control request and attempt;
-- reconcile every compensated task to a confirmed compensation receipt.
-- Rollback: disable the compensation flag and drain/preserve in-flight receipts.
-- Do not remove expand objects or downgrade task state constraints.
BEGIN;
...
CREATE TABLE IF NOT EXISTS dashboard_task_compensation_receipts (
  ...
  trace_id TEXT NOT NULL, compensated_at TIMESTAMPTZ NOT NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
