-- F-ALERT-003 / T-PG-002 / T-KAFKA-003: provider-authoritative alert response execution.
-- Expand only. Existing simulation and blocked receipts are retained and
-- backfilled with explicit provider/effect identities. Rollback disables
-- ALERT_RESPONSE_EXTERNAL_EXECUTOR_V1_ENABLED and retains every receipt/audit.
BEGIN;
...
ALTER TABLE alert_response_execution_receipts
  ADD CONSTRAINT alert_response_execution_receipts_state_check
    CHECK (state IN ('simulated_completed','blocked_external_executor','completed','partial','failed')),
  ADD CONSTRAINT alert_response_execution_receipts_effect_state_check
    CHECK (effect_state IN ('confirmed','none','unknown')),
  ADD CONSTRAINT alert_response_execution_receipts_effect_ids_check
    CHECK (jsonb_typeof(effect_ids)='array'),
  ADD CONSTRAINT alert_response_execution_receipts_receipt_sha256_check
    CHECK (receipt_sha256 ~ '^[0-9a-f]{64}$'),
  ADD CONSTRAINT alert_response_execution_receipts_completed_effect_check
    CHECK (state<>'completed' OR (external_effect AND effect_state='confirmed' AND jsonb_array_length(effect_ids)>0)),
  ADD CONSTRAINT alert_response_execution_receipts_failed_effect_check
    CHECK (state<>'failed' OR (NOT external_effect AND effect_state='none'));

CREATE UNIQUE INDEX IF NOT EXISTS uq_alert_response_provider_receipt
  ON alert_response_execution_receipts(provider,provider_receipt_id)
  WHERE provider<>'' AND provider_receipt_id<>'';
CREATE INDEX IF NOT EXISTS idx_alert_response_receipts_trace
  ON alert_response_execution_receipts(tenant_id,trace_id,executed_at DESC)
  WHERE trace_id<>'';
