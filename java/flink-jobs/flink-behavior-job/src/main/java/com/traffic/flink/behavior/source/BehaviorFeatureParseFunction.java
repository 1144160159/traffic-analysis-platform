...
/** Parses canonical FeatureStat input and preserves source coordinates for rejection handling. */
public final class BehaviorFeatureParseFunction
        extends ProcessFunction<RawKafkaRecord, ValidatedBehaviorFeature> {

    private static final long serialVersionUID = 1L;
    private final OutputTag<CanonicalDlqMessage> dlqTag;

    public BehaviorFeatureParseFunction(OutputTag<CanonicalDlqMessage> dlqTag) {
        this.dlqTag = dlqTag;
    }

    @Override
    public void processElement(
            RawKafkaRecord source, Context context, Collector<ValidatedBehaviorFeature> out) {
        ParseResult result = parse(source);
        if (result.input != null) out.collect(result.input);
    ...
    static ParseResult parse(RawKafkaRecord source) {
        ...
        return new ParseResult(new ValidatedBehaviorFeature(source, feature), null);
    }
    ...
    static final class ParseResult {
        final ValidatedBehaviorFeature input;
        final CanonicalDlqMessage failure;

        ParseResult(ValidatedBehaviorFeature input, CanonicalDlqMessage failure) {
            this.input = input;
