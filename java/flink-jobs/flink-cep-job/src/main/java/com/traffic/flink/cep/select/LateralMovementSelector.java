////////////////////////////////////////////////////////////////////////////////
// FILE PATH: flink-jobs/flink-cep-job/src/main/java/com/traffic/flink/cep/select/LateralMovementSelector.java
////////////////////////////////////////////////////////////////////////////////

package com.traffic.flink.cep.select;

import com.traffic.flink.cep.model.CampaignType;
import com.traffic.proto.traffic.v1.Alert;
import com.traffic.proto.traffic.v1.Campaign;
import com.traffic.proto.traffic.v1.EventHeader;

import org.apache.flink.cep.functions.PatternProcessFunction;
import org.apache.flink.util.Collector;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.util.*;
import java.util.stream.Collectors;

/**
 * 横向移动选择器
 * 
 * 从 CEP 模式匹配结果构建 Campaign
 * 攻击阶段：初始访问 → 发现 → 横向移动
 */
public class LateralMovementSelector extends PatternProcessFunction<Alert, Campaign> {

    private static final long serialVersionUID = 1L;
    private static final Logger LOG = LoggerFactory.getLogger(LateralMovementSelector.class);

    @Override
    public void processMatch(
            Map<String, List<Alert>> pattern,
            Context ctx,
            Collector<Campaign> out
    ) throws Exception {
        List<Alert> compromiseAlerts = pattern.get("compromise");
        List<Alert> internalAlerts = pattern.get("internal_activity");
        List<Alert> lateralAlerts = pattern.get("lateral");

        if (compromiseAlerts == null || compromiseAlerts.isEmpty() || 
            lateralAlerts == null || lateralAlerts.isEmpty()) {
            LOG.warn("Invalid lateral movement pattern: compromise={}, lateral={}",
                    compromiseAlerts != null ? compromiseAlerts.size() : 0,
                    lateralAlerts != null ? lateralAlerts.size() : 0);
            return;
        }

        try {
            Campaign campaign = buildCampaign(compromiseAlerts, internalAlerts, lateralAlerts);
            if (campaign != null) {
                out.collect(campaign);
                LOG.info("Generated Lateral Movement Campaign: id={}, hops={}, score={}", 
                        campaign.getCampaignId(), lateralAlerts.size(), campaign.getScore());
            }
        } catch (Exception e) {
            LOG.error("Error creating lateral movement campaign: {}", e.getMessage(), e);
        }
    }

    private Campaign buildCampaign(
            List<Alert> compromiseAlerts,
            List<Alert> internalAlerts,
            List<Alert> lateralAlerts
    ) {
        // 提取所有告警
        List<Alert> allAlerts = new ArrayList<>();
        allAlerts.addAll(compromiseAlerts);
        if (internalAlerts != null) {
            allAlerts.addAll(internalAlerts);
        }
        allAlerts.addAll(lateralAlerts);

        // 按时间排序
        allAlerts.sort(Comparator.comparingLong(Alert::getFirstSeen));

        if (allAlerts.isEmpty()) {
            return null;
        }

        // 提取时间范围
        long tsStart = allAlerts.stream()
                .mapToLong(Alert::getFirstSeen)
                .min()
                .orElse(System.currentTimeMillis());
        
        long tsEnd = allAlerts.stream()
                .mapToLong(Alert::getLastSeen)
                .max()
                .orElse(System.currentTimeMillis());

        // 提取实体（统一使用 ip: 前缀）
        Set<String> entities = extractEntities(allAlerts);

        // 提取告警 ID
        List<String> alertIds = allAlerts.stream()
                .map(Alert::getAlertId)
                .distinct()
                .collect(Collectors.toList());

        // 构建攻击路径
        String attackPath = buildAttackPath(compromiseAlerts, lateralAlerts);

        // 计算评分（跳板数量越多，分数越高）
        float score = calculateScore(compromiseAlerts, internalAlerts, lateralAlerts);

        // 生成摘要（包含攻击路径）
        String summary = buildSummary(compromiseAlerts, lateralAlerts, attackPath);

        // 提取租户信息
        String tenantId = allAlerts.get(0).getTenantId();
        if (tenantId == null || tenantId.isEmpty()) {
            tenantId = "unknown";
        }

        // 生成 Campaign ID
        String campaignId = generateCampaignId(tenantId, tsStart);

        // 生成 Event ID
        String eventId = UUID.randomUUID().toString();
        long now = System.currentTimeMillis();

        // 构建 EventHeader（Alert 没有 header 字段，使用默认值）
        EventHeader header = EventHeader.newBuilder()
                .setEventId(eventId)
                .setTenantId(tenantId)
                .setRunId("realtime")
                .setEventTs(tsEnd)
                .setIngestTs(now)
                .setProbeId("cep-engine")
                .setFeatureSetId("campaign")
                .build();

        // 攻击阶段
        List<String> attackPhases = Arrays.asList(
                "initial_access", "discovery", "lateral_movement"
        );

        // 提取规则/模型 ID
        List<String> ruleIds = extractRuleIds(allAlerts);
        List<String> modelIds = extractModelIds(allAlerts);

        // 构建 Campaign
        return Campaign.newBuilder()
                .setHeader(header)
                .setTenantId(tenantId)
                .setCampaignId(campaignId)
                .setTsStart(tsStart)
                .setTsEnd(tsEnd)
                .addAllEntities(new ArrayList<>(entities))
                .addAllAlerts(alertIds)
                .setScore(score)
                .setSummary(summary)
                .setEventId(eventId)
                .setIngestTs(now)
                .setCampaignType(CampaignType.LATERAL_MOVEMENT.getCode())
                .addAllAttackPhases(attackPhases)
                .addAllRuleIds(ruleIds)
                .addAllModelIds(modelIds)
                .build();
    }

    /**
     * 提取实体（统一使用 ip: 前缀）
     */
    private Set<String> extractEntities(List<Alert> alerts) {
        Set<String> entities = new LinkedHashSet<>();
        for (Alert alert : alerts) {
            if (alert.getSrcIp() != null && !alert.getSrcIp().isEmpty()) {
                entities.add("ip:" + alert.getSrcIp());
            }
            if (alert.getDstIp() != null && !alert.getDstIp().isEmpty()) {
                entities.add("ip:" + alert.getDstIp());
            }
        }
        return entities;
    }

    /**
     * 构建攻击路径
     */
    private String buildAttackPath(List<Alert> compromiseAlerts, List<Alert> lateralAlerts) {
        StringBuilder path = new StringBuilder();
        
        // 初始入侵点
        if (!compromiseAlerts.isEmpty()) {
            path.append(compromiseAlerts.get(0).getDstIp());
        }
        
        // 横向移动路径
        Set<String> visitedIps = new HashSet<>();
        if (!compromiseAlerts.isEmpty()) {
            visitedIps.add(compromiseAlerts.get(0).getDstIp());
        }
        
        for (Alert lateral : lateralAlerts) {
            String dstIp = lateral.getDstIp();
            if (dstIp != null && !visitedIps.contains(dstIp)) {
                path.append(" → ").append(dstIp);
                visitedIps.add(dstIp);
            }
        }
        
        return path.toString();
    }

    /**
     * 计算评分
     */
    private float calculateScore(
            List<Alert> compromiseAlerts,
            List<Alert> internalAlerts,
            List<Alert> lateralAlerts
    ) {
        // 基础分
        float baseScore = 0.5f;
        
        // 横向移动次数加分
        float lateralScore = Math.min(lateralAlerts.size() * 0.15f, 0.3f);
        
        // 内部活动加分
        float internalScore = 0.0f;
        if (internalAlerts != null && !internalAlerts.isEmpty()) {
            internalScore = Math.min(internalAlerts.size() * 0.05f, 0.1f);
        }
        
        // 唯一目标数量加分
        Set<String> uniqueTargets = new HashSet<>();
        for (Alert alert : lateralAlerts) {
            if (alert.getDstIp() != null) {
                uniqueTargets.add(alert.getDstIp());
            }
        }
        float uniqueTargetScore = Math.min(uniqueTargets.size() * 0.05f, 0.1f);
        
        return Math.min(baseScore + lateralScore + internalScore + uniqueTargetScore, 1.0f);
    }

    /**
     * 构建摘要
     */
    private String buildSummary(
            List<Alert> compromiseAlerts,
            List<Alert> lateralAlerts,
            String attackPath
    ) {
        String attacker = "unknown";
        String initialTarget = "unknown";
        
        if (!compromiseAlerts.isEmpty()) {
            attacker = compromiseAlerts.get(0).getSrcIp();
            initialTarget = compromiseAlerts.get(0).getDstIp();
        }
        
        // 统计唯一目标数
        Set<String> uniqueTargets = new HashSet<>();
        for (Alert alert : lateralAlerts) {
            if (alert.getDstIp() != null) {
                uniqueTargets.add(alert.getDstIp());
            }
        }
        
        return String.format(
                "检测到横向移动攻击链：攻击者 %s 入侵 %s 后，进行了 %d 次横向移动，影响 %d 台主机，攻击路径：%s",
                attacker,
                initialTarget,
                lateralAlerts.size(),
                uniqueTargets.size(),
                attackPath
        );
    }

    /**
     * 生成 Campaign ID
     */
    private String generateCampaignId(String tenantId, long tsStart) {
        return String.format("campaign-lateral-%s-%d-%s", 
                tenantId, 
                tsStart, 
                UUID.randomUUID().toString().substring(0, 8));
    }

    /**
     * 提取规则 ID
     */
    private List<String> extractRuleIds(List<Alert> alerts) {
        return alerts.stream()
                .map(Alert::getRuleVersion)
                .filter(id -> id != null && !id.isEmpty())
                .distinct()
                .collect(Collectors.toList());
    }

    /**
     * 提取模型 ID
     */
    private List<String> extractModelIds(List<Alert> alerts) {
        return alerts.stream()
                .map(Alert::getModelVersion)
                .filter(id -> id != null && !id.isEmpty())
                .distinct()
                .collect(Collectors.toList());
    }
}