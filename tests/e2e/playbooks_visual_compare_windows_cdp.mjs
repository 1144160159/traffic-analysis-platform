...
const revision = process.env.PLAYBOOKS_EVIDENCE_REVISION?.trim() || 'r338';
...
const compared = await page.evaluate(async ({ sourceUrl, actualUrl, scoringRegion, tolerance }) => {
  ...
  if (source.naturalWidth !== actual.naturalWidth || source.naturalHeight !== actual.naturalHeight) {
    throw new Error('Playbooks visual inputs have different dimensions');
  }
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      if (mismatch) diffPixels.data.set([255, 42, 164, 255], index);
      else {
...
const metrics = {
  target_id: 'playbooks', route: '/playbooks', status: ratio <= maxRatio ? 'pass' : 'fail',
