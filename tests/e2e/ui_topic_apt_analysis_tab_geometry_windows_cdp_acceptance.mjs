...
const revision = process.env.TOPIC_REVISION ?? 'topic-panel-apt-analysis-tab-geometry';
const evidenceRoot = path.join(root, 'evidence/ui-image-breakdowns/pages/topics-apt-campaign');
const outputPath = path.join(
  root,
  'evidence/ui-image-breakdowns/pages',
  `topics-apt-analysis-tab-geometry-${revision}.json`,
);
const tabLabels = [
  'ATT&CK阶段覆盖',
  '战役耗时线',
...
function smokeToken() {
  ...
  const claims = Buffer.from(JSON.stringify({
    ...
    tenant_id: 'default',
    username: 'codex-apt-tab-geometry-admin',
    roles: ['admin'],
    permissions: ['*', 'admin:*', 'topic:read'],
    token_type: 'access',
    session_id: `codex-apt-tab-geometry-${revision}`,
    iat: now,
    exp: now + 3_600,
  })).toString('base64url');
...
async function inspectState(page, label) {
  return page.locator('.taf-topic-apt-analysis-panel').evaluate((panel, expectedLabel) => {
    ...
    const tabs = panel.querySelector('.taf-topic-apt-tabs');
...
async function inspectViewport(context, token, viewport) {
  ...
  const url = new URL(`/topics?topic=apt&tab=apt&aptGeometryTs=${Date.now()}`, baseUrl);
  url.hash = `codex_smoke_token=${token}`;
  await page.goto(url.toString(), { waitUntil: 'domcontentloaded', timeout: 45_000 });
  await page.locator('.taf-topic-apt-analysis-panel').waitFor({ state: 'visible', timeout: 20_000 });
  await page.waitForFunction(() => {
    const pageRoot = document.querySelector('.taf-topic-page');
    return Boolean(pageRoot
      && pageRoot.textContent?.includes('关联战役数')
  ...
  for (const label of tabLabels) {
    ...
    await page.waitForFunction(
      ([selector, expected]) => document.querySelector(selector)?.getAttribute('data-active-tab') === expected,
      ['.taf-topic-apt-analysis-grid', label],
    );
    await page.waitForTimeout(120);
    states.push(await inspectState(page, label));
  }

  await page.getByRole('tab', { name: '处置动作状态', exact: true }).click();
  await page.locator('.taf-topic-apt-analysis-panel').scrollIntoViewIfNeeded();
  await page.waitForTimeout(180);
  const screenshotPath = path.join(
    evidenceRoot,
    `latest-apt-analysis-fixed-geometry-${viewport.width}x${viewport.height}-${revision}.png`,
  );
  fs.mkdirSync(path.dirname(screenshotPath), { recursive: true });
  await page.locator('.taf-topic-apt-analysis-panel').screenshot({
    path: screenshotPath,
    timeout: 120_000,
  });
