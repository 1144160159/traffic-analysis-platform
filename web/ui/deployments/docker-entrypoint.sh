...
: "${AUTH_ENABLED:=true}"
: "${USE_MOCK:=false}"
: "${ALERT_DETAIL_API_ENABLED:=false}"
...
export AUTH_ENABLED USE_MOCK ALERT_DETAIL_API_ENABLED ENABLE_REALTIME SCREEN_ACCESS_MODE DESKTOP_SMOKE_TOKEN_ENABLED

envsubst '${API_BACKEND_HOST} ${API_BACKEND_PORT} ${WS_BACKEND_HOST} ${WS_BACKEND_PORT}' \
  < /etc/nginx/nginx.conf.template \
  > /etc/nginx/nginx.conf

envsubst '${API_BASE_URL} ${WS_URL} ${ARKIME_BASE_URL} ${AUTH_ENABLED} ${USE_MOCK} ${ALERT_DETAIL_API_ENABLED} ${ENABLE_REALTIME} ${SCREEN_ACCESS_MODE} ${DESKTOP_SMOKE_TOKEN_ENABLED}' \
