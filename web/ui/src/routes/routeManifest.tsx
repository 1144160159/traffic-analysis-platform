...
const pageOverrides: Record<string, Partial<PageSpec>> = {
  ...
  "topic-exfil": {
    ...
    subtitle:
      "聚合大流量上传、异常目的地、风险类型和外传路径，形成数据外传专题。",
  ...
  probes: {
    variant: "pipeline",
    subtitle:
      "证明系统看得见、采得全、采得稳，覆盖部署、心跳、吞吐和证书状态。",
  ...
  alerts: {
    ...
    apiHints: [
      "/api/v1/alerts",
      "/api/v1/alerts/batch/status items[].state_version",
  ...
  forensics: {
    variant: "queue",
    subtitle:
      "围绕告警、资产、时间窗完成 PCAP、Session、日志证据检索与下载审计。",
    tabs: [],
    kpis: [
      "取证任务",
      "处理中",
  ...
  fusion: {
    ...
    apiHints: [
      ...
      "/api/v1/fusion/evidence-packages",
    ],
  },
  baselines: {
    variant: "quality",
  ...
  "audit-log": {
    ...
    background: "audit",
    subtitle:
      "关键操作全链路留痕，覆盖检索、详情 Diff、高风险复核、关联链路、留存校验和取证导出。",
    tabs: [
      "日志检索",
      "操作详情",
      "高风险审计",
  ...
  notifications: {
    ...
    background: "settings",
    subtitle:
      "配置告警、系统异常、验收缺口和运营任务的通知渠道、订阅规则、升级策略和静默窗口。",
    tabs: [
      "通知渠道",
      "订阅规则",
      "升级策略",
...
const routeScopes: Record<string, string[]> = {
  ...
  compliance: ["admin:*", "compliance:read"],
  notifications: ["admin:*"],
...
export const detailRoutes: NavRoute[] = [
  ...
  page: {
    ...
    rightRailTitle:
      route.id === "alert-detail" ? "处置与反馈" : "处置流程与复盘结论",
    actions:
      route.id === "alert-detail"
...
export const findRouteByPath = (pathname: string) => {
  ...
  if (withoutQuery.startsWith("/alerts/"))
    return detailRoutes.find((route) => route.id === "alert-detail");
  if (withoutQuery.startsWith("/campaigns/"))
    return detailRoutes.find((route) => route.id === "campaign-detail");
  return undefined;
};
