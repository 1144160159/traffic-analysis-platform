...
const apisixRoutePrefixes = Array.from(
  apisixRoutesYaml.matchAll(/uri:\s+(\/api\/v1\/[^\s*"]+)\*/g),
...
describe("pageApiPlans", () => {
  ...
  it("binds encrypted egress operations to the audited action endpoint", () => {
    ...
    const createAlert = getPageActionPlan(
      "encrypted-traffic",
      "egress-create-alert",
    );
    const evidenceLookup = getPageActionPlan(
      "encrypted-traffic",
      "egress-evidence-lookup",
    );
    const graphDrilldown = getPageActionPlan(
      "encrypted-traffic",
      "egress-entity-graph",
    );
    const writeAudit = getPageActionPlan(
      "encrypted-traffic",
      "egress-audit-write",
    );
    ...
    expect(createAlert?.guardrails).toContain(
    ...
    expect(isCoveredByApisix("/v1/encrypted-traffic/egress-actions")).toBe(
      true,
    );
  });
  ...
  it("binds encrypted evidence center reads and actions to audited APIs", () => {
    const endpoints = getPlanEndpoints(getPageApiPlan("encrypted-traffic"));
    const createTask = getPageActionPlan(
      "encrypted-traffic",
      "evidence-create-task",
    );
    const downloadPcap = getPageActionPlan(
      "encrypted-traffic",
      "evidence-download-pcap",
    );
    const verifyHash = getPageActionPlan(
      "encrypted-traffic",
      "evidence-verify-hash",
    );
    const exportPackage = getPageActionPlan(
      "encrypted-traffic",
      "evidence-export-package",
    );
    const preservation = getPageActionPlan(
      "encrypted-traffic",
      "evidence-preserve",
    );
    const expertReview = getPageActionPlan(
      "encrypted-traffic",
      "evidence-expert-review",
    );
    const exportReport = getPageActionPlan(
      "encrypted-traffic",
  ...
  it("binds topic governance overlays to audited control-plane APIs", () => {
    ...
    const createSubscriptionAction = getPageActionPlan(
      "topics",
      "topic-subscription-create",
    );
    const toggleSubscriptionAction = getPageActionPlan(
      "topics",
      "topic-subscription-toggle",
    );
    const reportExportAction = getPageActionPlan(
      "topics",
      "topic-report-export",
    );
    const evidenceExportAction = getPageActionPlan(
      "topics",
      "topic-evidence-package-export",
    );
    ...
    expect(createSubscriptionAction?.guardrails).toContain(
      "subscriptions must persist topic_subscriptions rows",
    );
    expect(toggleSubscriptionAction?.method).toBe("PATCH");
    expect(toggleSubscriptionAction?.guardrails).toContain(
    ...
    expect(evidenceExportAction?.auditEvent).toBe(
      "TOPIC_EVIDENCE_PACKAGE_EXPORTED",
    );
  ...
  it("binds fusion to the threat intel service through APISIX", () => {
    ...
    const conflictAction = getPageActionPlan(
      "fusion",
      "fusion-conflict-resolve",
    );
    const ruleAction = getPageActionPlan("fusion", "fusion-rule-update");
    const evidenceAction = getPageActionPlan("fusion", "fusion-evidence-export");

    expect(endpoints).toContain("/v1/fusion/workbench");
    ...
    expect(endpoints).toContain("/v1/fusion/evidence-packages");
    ...
    expect(evidenceAction?.auditEvent).toBe("FUSION_EVIDENCE_EXPORTED");
    ...
    expect(isCoveredByApisix("/v1/fusion/evidence-packages")).toBe(true);
  });
  ...
  it("binds behavior baseline reset to an audited alert-write action", () => {
    ...
    expect(resetAction?.guardrails).toContain(
  ...
  it("binds asset discovery to SNMP/LLDP credential and run APIs", () => {
    ...
    const credentialAction = getPageActionPlan(
      "assets",
      "asset-discovery-credential-register",
    );
    ...
    expect(credentialAction?.auditEvent).toBe(
      "ASSET_DISCOVERY_CREDENTIAL_REGISTER",
    );
    expect(credentialAction?.guardrails).toContain(
    ...
    expect(runAction?.guardrails).toContain(
  ...
  it("binds probe operations to audited control-plane APIs", () => {
    ...
    const batchUpgradeAction = getPageActionPlan(
      "probes",
      "probe-batch-upgrade",
    );
    const configAction = getPageActionPlan("probes", "probe-config-push");
    const connectivityAction = getPageActionPlan(
      "probes",
    ...
    expect(configAction?.guardrails).toContain(
      "tenant_id must come from the authenticated token for write operations",
    );
    expect(connectivityAction?.auditEvent).toBe(
      "PROBE_CONNECTIVITY_TEST_QUEUED",
    );
    expect(connectivityAction?.guardrails).toContain(
      "cross-tenant probe operations must return 404",
    );
    expect(certAction?.auditEvent).toBe("PROBE_CERT_ROTATE_QUEUED");
    expect(certAction?.defaultBody).toMatchObject({
      secret_ref: "k8s://traffic-analysis/traffic-credentials#PROBE_MTLS_CERT",
    });
    expect(certAction?.guardrails).toContain(
    ...
    expect(isCoveredByApisix("/v1/probes/PROBE-DC-01/connectivity-test")).toBe(
      true,
    );
    expect(
      isCoveredByApisix("/v1/probes/PROBE-DC-01/certificates/rotate"),
    ).toBe(true);
  });
  ...
  it("binds rules enable and disable to the rule state-machine APIs", () => {
    ...
    expect(disableAction?.guardrails).toContain(
  ...
  it("binds deployment actions to the deployment state-machine APIs", () => {
    ...
    const grayAction = getPageActionPlan(
      "deployments",
      "deployment-start-gray",
    );
    const activateAction = getPageActionPlan(
      "deployments",
      "deployment-activate",
    );
    ...
    const rollbackAction = getPageActionPlan(
      "deployments",
      "deployment-rollback",
    );
  ...
  it("binds model version actions to the model registry state-machine APIs", () => {
    ...
    const registerAction = getPageActionPlan(
      "models",
    ...
    const deprecateAction = getPageActionPlan(
      "models",
    ...
    const evaluationAction = getPageActionPlan(
      "models",
    ...
    expect(activateAction?.guardrails).toContain(
    ...
    expect(deprecateAction?.guardrails).toContain(
    ...
    expect(isCoveredByApisix("/v1/models/model-001/versions/v2/activate")).toBe(
      true,
    );
    expect(
      isCoveredByApisix("/v1/models/model-001/versions/v2/deprecate"),
    ).toBe(true);
    expect(isCoveredByApisix("/v1/models/model-001/feedback-samples")).toBe(
      true,
    );
    expect(isCoveredByApisix("/v1/models/model-001/retrain")).toBe(true);
    expect(isCoveredByApisix("/v1/models/model-001/versions/v2/evaluate")).toBe(
      true,
    );
    expect(isCoveredByApisix("/v1/models/model-001/versions/v2/rollback")).toBe(
      true,
    );
  ...
  it("binds MLOps orchestration controls to audited action contracts", () => {
    ...
    expect(
      isCoveredByApisix("/v1/mlops/workflows/mlops-manual-live/stop"),
    ).toBe(true);
  });

  it("binds compliance workflows to dedicated audited scopes", () => {
    const endpoints = getPlanEndpoints(getPageApiPlan("compliance"));
    const generateAction = getPageActionPlan(
      "compliance",
      "compliance-report-generate",
    );
    const exportAction = getPageActionPlan(
      "compliance",
      "compliance-report-export",
    );
    const remediationAction = getPageActionPlan(
      "compliance",
      "compliance-remediations-create",
    );
    const finalizeAction = getPageActionPlan(
      "compliance",
      "compliance-report-finalize",
    );
    ...
    expect(endpoints).toContain("/v1/compliance/reports/{id}/evidence-package");
    expect(generateAction?.method).toBe("POST");
    expect(generateAction?.requiredScopes).toContain("compliance:write");
    expect(generateAction?.auditEvent).toBe("COMPLIANCE_REPORT_GENERATED");
    expect(generateAction?.defaultBody).toMatchObject({
      report_type: "weekly",
    });
    expect(generateAction?.requiredScopes).toEqual(["compliance:write"]);
    expect(generateAction?.guardrails).toContain(
    ...
    expect(exportAction?.requiredScopes).toEqual(["compliance:export"]);
    expect(remediationAction?.requiredScopes).toEqual(["compliance:remediate"]);
    expect(finalizeAction?.requiredScopes).toEqual(["compliance:finalize"]);
  ...
  it("binds whitelist governance to audited create, approval, extension and disable APIs", () => {
    ...
    const approvalAction = getPageActionPlan(
      "whitelist",
      "whitelist-submit-approval",
    );
    ...
    expect(createAction?.guardrails).toContain(
  ...
  it("binds notification governance to settings, test-send and silence rule APIs", () => {
    ...
    const settingsAction = getPageActionPlan(
      "notifications",
      "notification-settings-update",
    );
    const testAction = getPageActionPlan(
      "notifications",
      "notification-test-send",
    );
    const createSilenceAction = getPageActionPlan(
      "notifications",
      "notification-silence-rule-create",
    );
    const toggleSilenceAction = getPageActionPlan(
      "notifications",
      "notification-silence-rule-toggle",
    );
    ...
    expect(settingsAction?.guardrails).toContain(
    ...
    expect(isCoveredByApisix("/v1/notifications/silence-rules/rule-001")).toBe(
      true,
    );
  });

  it("binds settings governance to tenant settings, actions, audit and token lifecycle APIs", () => {
    const endpoints = getPlanEndpoints(getPageApiPlan("settings"));
    const systemSaveAction = getPageActionPlan("settings", "settings-system-save");
    const connectionAction = getPageActionPlan("settings", "settings-connection-test");
    const securityAuditAction = getPageActionPlan("settings", "settings-security-audit");
    const impactAction = getPageActionPlan("settings", "settings-impact");
    const saveAction = getPageActionPlan(
      "settings",
      "settings-preferences-save",
    );
    const createAction = getPageActionPlan("settings", "settings-token-create");
    const regenerateAction = getPageActionPlan(
      "settings",
      "settings-token-regenerate",
    );
    const revokeAction = getPageActionPlan("settings", "settings-token-revoke");
    const scopeAction = getPageActionPlan(
      "settings",
      "settings-token-scope-update",
    );
    const validateAction = getPageActionPlan(
      "settings",
      "settings-token-validate",
    );

    expect(endpoints).toContain("/v1/auth/system-settings");
    expect(endpoints).toContain("/v1/auth/system-settings/actions/connection-test");
    expect(endpoints).toContain("/v1/auth/system-settings/actions/security-audit");
    expect(endpoints).toContain("/v1/auth/system-settings/impact");
    ...
    expect(systemSaveAction?.method).toBe("PUT");
    expect(systemSaveAction?.requiredScopes).toContain("admin:write");
    expect(systemSaveAction?.auditEvent).toBe("system_settings_update");
    expect(systemSaveAction?.guardrails).toContain(
      "expected_revision must prevent lost updates",
    );
    expect(connectionAction?.auditEvent).toBe("system_settings_connection_test");
    expect(securityAuditAction?.auditEvent).toBe("system_settings_security_audit");
    expect(impactAction?.method).toBe("GET");
...
function isCoveredByApisix(endpoint: string) {
  return apisixRoutePrefixes.some(
