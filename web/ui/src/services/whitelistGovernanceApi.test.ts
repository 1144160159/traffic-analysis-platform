...
describe('whitelistGovernanceApi', () => {
  ...
  it('creates a draft without trusting client tenant identity', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/v1/whitelist', expect.objectContaining({ status: 'draft', approval_status: 'draft' }), {
      headers: { 'X-Action-ID': 'whitelist-create', 'Idempotency-Key': expect.stringMatching(/^whitelist:whitelist-create:/) },
    });
    expect(api.post).not.toHaveBeenCalledWith('/v1/whitelist', expect.objectContaining({ tenant_id: expect.anything() }), expect.anything());
  });
