...
function ApprovalDetail({ entry, reason, onReasonChange, pending, onAction }: { entry: WhitelistEntry; reason: string; onReasonChange: (value: string) => void; pending: boolean; onAction: (action: WhitelistTransition) => void }) {
  ...
  return <div className="taf-whitelist-approval-detail">
    ...
    <WorkPanel title="审计与规则投影 ACK"><Input.TextArea aria-label="审批意见" value={reason} onChange={(event) => onReasonChange(event.target.value)} rows={3} maxLength={500} showCount /><p>版本 v{entry.version} · 创建人 {entry.created_by || '-'} · 审批人 {entry.approved_by || '待审批'}</p><p>规则状态 {entry.rule_effect_status || '未请求'} · 目标 {entry.rule_desired_state || '-'} · 规则版本 {entry.rule_revision || '等待 ACK'}</p><p>ACK {entry.rule_ack_event_id || '-'} · Kafka {entry.rule_kafka_partition ?? '-'}:{entry.rule_kafka_offset ?? '-'} · {entry.rule_acknowledged_at ? formatDateTime(entry.rule_acknowledged_at) : '尚未确认'}</p>{entry.rule_last_error && <Alert type="error" showIcon message="规则投影失败" description={entry.rule_last_error} />}</WorkPanel>
...
const formatDateTime = (value?: string) => value && Number.isFinite(Date.parse(value)) ? new Date(value).toISOString().replace('T', ' ').slice(0, 19) + 'Z' : '-';
