-- Canonical bootstrap counterpart of migration 202608031610_whitelist_governance_v2.sql.
BEGIN;
...
-- Durable rule-manager projection consumed by the alert ingestion path. An
-- effect is acknowledged as applied only in the same transaction that writes
-- this projection, preventing UI state from claiming enforcement prematurely.
CREATE TABLE IF NOT EXISTS whitelist_rule_projection (
  ...
  scope TEXT NOT NULL DEFAULT '', expires_at TIMESTAMPTZ,
