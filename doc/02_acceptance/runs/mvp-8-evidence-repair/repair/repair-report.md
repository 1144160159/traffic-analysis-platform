...
5. `verify` `LOCAL_REPORT_MISSING` `local-report.md`: Run the declared local verification and preserve command output with exit codes. Source: Task declares local verification but `local-report.md` is missing.
6. `review` `REVIEW_NOT_PASSED` `review-report.md`: Complete the third-view reviewer decision and resolve any blocking findings. Source: Reviewer decision is `pending`.
7. `evidence` `EVIDENCE_REPORT_MISSING` `evidence-report.md`: Publish a concrete evidence report that maps close_when items to artifacts and the required evidence layer. Source: `evidence-report.md` is missing, so close_when cannot be proven.
