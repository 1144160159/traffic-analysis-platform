...
```json
{
  "ranking_model": {
    ...
    "request": "doc/02_acceptance/runs/mvp-50-loop-one-round-first-p0-route-daemon-i1/guidance/ranking-reasoning-request.md",
    "schema": "doc/02_acceptance/runs/mvp-50-loop-one-round-first-p0-route-daemon-i1/guidance/ranking-reasoning-schema.json",
    "command_template": "doc/02_acceptance/runs/mvp-50-loop-one-round-first-p0-route-daemon-i1/guidance/ranking-reasoning-command.txt",
    "command": "codex exec --model gpt5.5 --sandbox read-only --output-schema doc/02_acceptance/runs/mvp-50-loop-one-round-first-p0-route-daemon-i1/guidance/ranking-reasoning-schema.json 'Read the Codex Loop ranking reasoning request at {prompt}. Return JSON only matching the provided schema.'"
  },
  "rules": [
    "explicit prerequisites block scheduling and closure until CLOSED",
    "tasks that unlock blocked dependents receive priority",
  ...
  "findings": [
    ...
    },
    {
      "level": "warning",
      "code": "RUN_EVIDENCE_INCOMPLETE",
      "target": "mvp-50-loop-one-round-first-p0-route-daemon",
      "message": "Run is missing core files: run-summary.json, task.yaml, plan.md, review-report.md.",
      "suggestion": "Do not use this run to close a task until core evidence exists."
    },
    {
      "level": "warning",
