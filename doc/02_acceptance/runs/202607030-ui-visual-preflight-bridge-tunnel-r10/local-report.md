...
- Run ID: `202607030-ui-visual-preflight-bridge-tunnel-r10`
- Result: `blocked`
- Target routes: 28
- Visual targets: 30
...
- Matrix: `doc/02_acceptance/runs/202607030-ui-visual-preflight-bridge-tunnel-r10/ui-visual-interaction-matrix.json`
...
- `desktop-chrome` Codex Desktop Chrome wrapper is available for screenshot and interaction capture: Windows 10.3.6.59 tunnel and Windows Codex node_repl MCP config are present: mcp list/get shows node_repl enabled over stdio with native-pipe env names, and direct SSH stdio can initialize/list tools. However codex remote-control start --json is Unix-only on Windows, and both normal plus explicit full-env tools/call js fail before Chrome control with Windows sandbox firewall rule creation denied (SetRemotePorts HRESULT 0x80070005 / 拒绝访问); current Codex session still lacks trusted Desktop Chrome MCP wrapper tools.
