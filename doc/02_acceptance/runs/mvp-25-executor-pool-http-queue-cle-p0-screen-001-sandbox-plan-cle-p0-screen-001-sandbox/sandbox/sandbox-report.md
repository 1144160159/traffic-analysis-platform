...
- run_id: `mvp-25-executor-pool-http-queue-cle-p0-screen-001-sandbox-plan-cle-p0-screen-001-sandbox`
...
```bash
docker run --read-only --rm --network none --workdir /workspace --cap-drop ALL --security-opt no-new-privileges --cpus 1 --memory 1Gi -e PYTHONUNBUFFERED=1 -e CODEX_LOOP_SANDBOX=1 -v /home/wangwt/phase_2/code/traffic-analysis-platform:/workspace --tmpfs /tmp:rw,noexec,nosuid,size=256m traffic-analysis/codex-loop:local python -B scripts/codex_loop/workflow.py --task scripts/codex_loop/tasks/CLE-P0-SCREEN-001.yaml --run-id mvp-25-executor-pool-http-queue-cle-p0-screen-001-sandbox-plan-cle-p0-screen-001-sandbox-workflow --stage prepare
```
