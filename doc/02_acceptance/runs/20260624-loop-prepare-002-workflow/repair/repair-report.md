...
1. `evidence` `PREP_EVIDENCE_CANNOT_CLOSE` `run-summary.json`: Publish a concrete evidence report that maps close_when items to artifacts and the required evidence layer. Source: Evidence type `acceptance-prep` is preparation evidence and cannot close `acceptance-prep`.
