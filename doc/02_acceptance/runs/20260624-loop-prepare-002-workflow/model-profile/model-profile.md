...
- output_schema: `doc/02_acceptance/runs/20260624-loop-prepare-002-workflow/patch-runner/codex-output-schema.json`
...
```bash
codex exec --model gpt-5 --sandbox read-only --output-schema doc/02_acceptance/runs/20260624-loop-prepare-002-workflow/patch-runner/codex-output-schema.json 'Read and follow the high-risk Codex patch request at {prompt}. Return only output matching the provided schema. Highlight contract, security, evidence, and rollback risks.'
```
