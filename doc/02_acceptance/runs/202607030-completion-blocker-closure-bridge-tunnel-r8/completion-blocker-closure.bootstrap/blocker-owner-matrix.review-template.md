...
|---|---|---|---|
| desktop_browser_smoke | QA / Desktop runtime | windows_tunnel_node_repl_reachable_but_js_permission_denied | run the generated payload from the trusted Windows Codex Desktop / VSCode context or restore the current-session Desktop Chrome bridge tool; direct SSH stdio can list Node REPL MCP tools but tools/call js is blocked by Windows sandbox/firewall permission, so Chrome extension capture still needs the trusted bridge execution path |
