...
- run_id: `mvp-41-daemon-objective-stop-dry-i1-objective-stop`
...
- `blocker` `GUIDANCE_HAS_BLOCKERS` `guidance/guidance.json`: Guidance contains 1 blocker findings. Recommendation: Resolve guidance blockers before objective stop.
- `blocker` `LATEST_REQUIRED_RUN_BLOCKED` `doc/02_acceptance/runs/mvp-41-release-blocked-by-objective-stop`: Latest `release_freeze` run `mvp-41-release-blocked-by-objective-stop` has status `RELEASE_BLOCKED`. Recommendation: Repair this run kind before objective stop.
