...
- checked_run: `doc/02_acceptance/runs/mvp-12-persistent-queue-metrics-i1-worker-cle-p0-screen-001`
...
- `blocker` `PATCH_RUNNER_BLOCKER` `patch-runner/patch-intake.json`: GUIDANCE_BLOCKER: SCREEN_AUTH_BOUNDARY: /screen is outside ProtectedLayout. Suggestion: Resolve the patch runner finding before closing the task.
...
- `blocker` `REVIEW_NOT_PASSED` `review-report.md`: Reviewer decision is `design_update_required`. Suggestion: Resolve reviewer findings before using this evidence to close the task.
