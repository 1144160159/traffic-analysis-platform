...
最新页面校正（2026-07-25，战役域）：`/campaigns`、`/campaigns/:id`、五类影响范围 Modal、报告导出、`/attack-chains` 及两个详情 Drawer 已按前端 r748、后端/数据库 r750 完成集成闭环。真实 APISIX/JWT/PostgreSQL/ClickHouse 预检为 33/33；Windows Chrome `127.0.0.1:9224` 下 11/11 截图与 diff、5/5 稳定关闭、14/14 服务端审计、6/6 ECharts 图谱、54/54 六视口响应式均通过。五类影响范围在 1366×768 下统一为 1040×671 内容型 Modal，不全屏，关闭后保留同一战役业务内容。逻辑/布局复审 P0=0、P1=0，主线程裁决 `ACCEPT`。稳定入口为 `02-regression/campaign-workbench-development-progress-latest.json`、`campaign-workbench-review-adjudication-latest.json` 与 `campaign-workbench-rollout-r750.json`；该页面域闭环不替代项目级性能、安全、现场清单和第三方签认门禁。
...
专题治理与三图重开发闭环已补齐：`20260727-topic-panel-r823` 为 `pass`，48/48 通过，证明三个数据库模拟专题读取、保存视图创建/共享/收藏、专题范围更新、专题动作、订阅创建/停用、报告导出、证据包导出、viewer 403、跨租户隔离、PostgreSQL 持久化和 `TOPIC_*` 审计落库均已通过真实 APISIX/JWT/PostgreSQL 验证。Windows Chrome r826 进一步通过三页视觉、交互和响应式门；开发流程与证据索引见 `02-regression/topics-development-progress-latest.md`。
...
| 专题面板 | 2026-07-28 r850/r851 已按用户参考图完成加密隧道整页最终回修：三个主画布统一为 API 动态 ECharts graph，区分实线/虚线并支持漫游缩放/自动适配；局部影响面为资产组→探针→高危源→协议→代理/跳板→外部端点六层链（PostgreSQL 20 节点、26 边、3 摘要卡）。中部业务区在 1080/1300 高度均保持 381px；资产卡和协议卡框/图标/标题/明细同步缩放；证据表固定 10 条/页，仅空间不足时内部滚动。1600/1366 响应式无左右区重叠、文字裁切或水平溢出；隧道最终控件 `88/88`。真实报告预览、五类独立行级业务结果、PostgreSQL 制品/动作/审计闭环及逻辑/布局双复审均通过。详见 `02-regression/topics-development-progress-latest.*` | 后续扩展真实导出对象存储、订阅渠道投递回执和专题模板审批 |
