...
合规审计闭环已补齐：`20260630-compliance-audit-preflight-r1` 为 `pass`，19/19 通过，证明 admin 合规报告生成、报告查询、合规 audit-trail、审计日志 API、PostgreSQL 持久化、跨租户隔离和 viewer 403 负例均已通过真实 APISIX/JWT/PostgreSQL 验证。该轮同时将 alert-service 滚动到 `docker.io/traffic/alert-service@sha256:5302515a84340940a539f458cd7ed648a5f83369131577678003a5449f2cd4c2`。
...
系统设置治理闭环已补齐：`20260630-settings-governance-preflight-r1` 为 `pass`，45/45 通过，证明 `/settings` 前端动作契约、`/v1/auth/settings/display` 保存/读取、API token 创建、Scope 更新、轮换、吊销、raw token validate、viewer 403、跨租户 404、`user_settings` / `api_tokens` 持久化和 `create_token` / `regenerate_token` / `revoke_token` 审计落库均已通过真实 APISIX/JWT/PostgreSQL 验证；明文 token 仅保存在临时文件，latest 证据已脱敏。
...
| 通知配置 | 2026-06-30 已新增 `tests/e2e/live_notification_governance_preflight.sh`，alert-service 滚动到 `docker.io/traffic/alert-service@sha256:d46cffeb90f649eacd07da2b8f34713706d4819c72adbff4dd8c74c55972061c`；r1 `20260630-notification-governance-preflight-r1` 为 26/26 passed，覆盖通知设置更新、inline secret 拒绝、测试发送、静默规则创建/停用、viewer 写入 403、跨租户 404、PostgreSQL `notification_silence_rules` 持久化和 `NOTIFICATION_*` 审计落库；前端 `notifications` API plan 约束 settings/test/silence actions，snapshot adapter 消费 `/v1/notifications/silence-rules` | 后续扩展到真实邮件/企业微信/Webhook 供应商回执、升级策略 SLA 和可靠队列降级专项 |
...
| 白名单治理 | 2026-06-30 已新增 `tests/e2e/live_whitelist_governance_preflight.sh`，alert-service 滚动到 `docker.io/traffic/alert-service@sha256:c0bf72ca45d3c22acf2c3586e3ef88c49c79edd507a870b704131d6678d27f22`；r2 `20260630-whitelist-governance-preflight-r2` 为 26/26 passed，覆盖白名单草案创建、提交审批、审批激活、延期、停用、`/v1/whitelist/check` 命中/停用负例、viewer 写入 403、跨租户 404、PostgreSQL `whitelist` 持久化和 `WHITELIST_*` 审计落库；前端 `whitelist` API plan 已约束 create/submit-approval/extend/disable actions，route manifest 记录 `/api/v1/alerts/{id}/feedback add_to_whitelist -> /api/v1/whitelist draft` 链路 | 后续扩展到批量延期 UI 实操、审批人分离策略、真实告警详情页到白名单页的 Desktop 浏览器视觉巡检 |
| 行为基准 | 2026-06-30 已新增 `tests/e2e/live_baseline_governance_preflight.sh`，alert-service 滚动到 `docker.io/traffic/alert-service@sha256:b72cabec23b3cce37332501fe4a90fc05a54020b132b69c9121d2db6fcc6ba4f`；r2 `20260630-baseline-governance-preflight-r2` 为 16/16 passed，覆盖 `/v1/baselines` 列表、`/v1/baselines/{id}` 详情、`POST /v1/baselines/{id}/reset`、viewer 写入 403、跨租户审计隔离、PostgreSQL `behavior_baseline_resets` 持久化和 `BEHAVIOR_BASELINE_RESET` 审计落库；前端 `baselines` API plan 已约束 detail/reset endpoints，reset 需要 `alert:write` 或 admin scope | 后续扩展到生成告警、加入观察名单、阈值版本审批和 Desktop 浏览器视觉巡检 |
...
| SOAR 剧本 | 2026-06-29 已新增 `tests/e2e/live_playbook_state_machine.sh` 与 `02-regression/playbook-state-machine-latest.*`；r1 为 21/21 passed，覆盖 `/v1/playbooks/catalog`、`/v1/playbooks/{name}/execute`、`PATCH /v1/playbooks/{name}`、禁用门禁、手动 `max_runs`、执行记录落库、`/v1/playbooks/executions` 和 `/playbooks` 前端消费 | 后续扩展到跨剧本审批、通知/工单集成和更多处置节点负例 |
...
| 合规审计 | 2026-06-30 已新增 `tests/e2e/live_compliance_audit_preflight.sh`，alert-service 滚动到 `docker.io/traffic/alert-service@sha256:5302515a84340940a539f458cd7ed648a5f83369131577678003a5449f2cd4c2`；r1 `20260630-compliance-audit-preflight-r1` 为 19/19 passed，覆盖 admin 生成合规报告、报告查询、合规 audit-trail、审计日志 API、PostgreSQL `compliance_reports`/`audit_logs` 行级持久化、跨租户审计查询隔离和 viewer 生成报告 403 | 高敏合规报告生成已闭环并有审计负例；后续仍需断 Kafka/可靠队列降级专项和第三方评测材料 |
