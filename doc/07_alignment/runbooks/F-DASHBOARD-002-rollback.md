...
出现以下任一情况立即停止扩大：跨租户任务可见、同一幂等键产生多个任务或提供方副作用、任务/历史/审计/outbox/执行或补偿回执数量不一致、持续PG或Kafka错误、事件积压或重试超过预算、执行器或补偿器HTTP 2xx被错误写成成功、未知副作用被写成失败、权威查询返回的`request_event_id/idempotency_key/provider`与原命令或回执不一致、没有`receipt_found`和有效耐久回执却恢复成功、候选bundle仍显示仿真成功、任务受理、执行或补偿P99及资源超过批准预算。
