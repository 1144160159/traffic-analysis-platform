...
本手册仅覆盖默认关闭的 `ALERT_BATCH_ASSIGNMENT_V1_ENABLED` 与 `ALERT_BATCH_ASSIGNMENT_PIPELINE_V1_ENABLED` 路径。该路径把服务端冻结选择、批次、逐项状态、history、outbox、inbox、投影回执、DLQ确认回执和审计写入 PostgreSQL，并通过 `alert.assignment.events.v1` 独立消费后确定性投影 ClickHouse；`202 accepted` 不代表任何告警已经完成指派。

## 停止扩大

1. 先将 `ALERT_BATCH_ASSIGNMENT_PIPELINE_V1_ENABLED=false` 停止新的 dispatcher/consumer，再将 `ALERT_BATCH_ASSIGNMENT_V1_ENABLED=false`；按单实例串行方式回滚 alert-service，禁止在仍有生产实例消费时改组或删除 topic。
2. 前端同时保持 `VITE_ALERT_BATCH_ASSIGNMENT_V1_ENABLED=false`，继续使用兼容期内的旧逐条指派入口。
3. 禁止删除或改写 `alert_assignment_selections`、`alert_assignment_batches`、`alert_assignment_batch_items`、三类 history、`alert_assignment_states`、outbox、inbox、投影回执、DLQ回执、请求收据和 `audit_logs`。
...
1. 在独立环境连续回放 `202608091900` 和 `202608092130` migration，确认无 destructive DDL，并校验 Docker/Kubernetes 兼容入口与版本化 migration 字节同步。
2. 只读核对 batch/items/history/outbox/request/audit 数量与 SHA；差异必须形成新的 occurrence。
3. 经领域、QA、SRE、安全批准后，先 shadow dispatcher，再 canary consumer；逐项验证 stale revision、unauthorized assignee、partial、Kafka重投、DLQ确认屏障和补偿边界。
