# rules-editor-test-validation.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：规则管理。
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/rules-editor-test-validation.png`
- 源图尺寸：1920 x 1080
...
| rules-editor-test-validation | `510,54,360,24` | subtitle | 是 |
| /rules | `940,54,360,24` | section | 是 |
| 管理检测规则生命周期，覆盖创建、测试、发布、命中、误报、回滚。 | `1370,54,360,24` | label | 是 |
...
| 左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“规则管理”。 | `940,138,360,24` | title | 是 |
...
| 页面名称：规则管理。 | `1370,180,360,24` | button | 是 |
| 路由：/rules。 | `80,222,360,24` | status | 是 |
| 业务模块：检测运营 / 规则管理。 | `510,222,360,24` | legend | 是 |
| 页面重点：管理检测规则生命周期，覆盖创建、测试、发布、命中、误报、回滚。 | `940,222,360,24` | hint | 是 |
| 必须包含的业务模块： | `1370,222,360,24` | acceptance | 是 |
| - 规则列表：规则名、类型、严重级别、状态、版本、命中数、误报率。 | `80,264,360,24` | title | 是 |
| - 规则编辑：条件、特征、阈值、MITRE 阶段、适用范围、例外条件。 | `510,264,360,24` | subtitle | 是 |
...
- 文本 02：`rules-editor-test-validation`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/rules`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`管理检测规则生命周期，覆盖创建、测试、发布、命中、误报、回滚。`，类型 label，来源 prompt/layer plus target visual ledger。
...
- 文本 11：`左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“规则管理”。`，类型 title，来源 prompt/layer plus target visual ledger。
...
- 文本 16：`页面名称：规则管理。`，类型 button，来源 prompt/layer plus target visual ledger。
- 文本 17：`路由：/rules。`，类型 status，来源 prompt/layer plus target visual ledger。
- 文本 18：`业务模块：检测运营 / 规则管理。`，类型 legend，来源 prompt/layer plus target visual ledger。
- 文本 19：`页面重点：管理检测规则生命周期，覆盖创建、测试、发布、命中、误报、回滚。`，类型 hint，来源 manual visual ledger。
- 文本 20：`必须包含的业务模块：`，类型 acceptance，来源 manual visual ledger。
- 文本 21：`- 规则列表：规则名、类型、严重级别、状态、版本、命中数、误报率。`，类型 title，来源 manual visual ledger。
- 文本 22：`- 规则编辑：条件、特征、阈值、MITRE 阶段、适用范围、例外条件。`，类型 subtitle，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/rules-editor-test-validation/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/rules-editor-test-validation/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/rules-editor-test-validation/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/rules-editor-test-validation/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/rules-editor-test-validation/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/rules-editor-test-validation/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/rules-editor-test-validation/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/rules-editor-test-validation/verification.json`
