...
- 分类：pages
- 标题：态势大屏
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/screen.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/screen.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/screen.json`
- 对应路由/宿主路由：`/screen`
- 当前状态：`pixel-accepted`
- 复刻等级：逐图目标 PNG 已锁定；alpha 视觉门禁已通过，内容与布局一致，亮度/透明度/抗锯齿和密集拓扑线条渲染差异按当前 alpha 规则放宽。
- 最新生产证据：`traffic/web-ui:ui-screen-pipeline-flow-20260706-r41`，Windows Chrome CDP 截图 URL `http://10.0.5.8:30180/screen?__codex_ui_breakdown_production=1&windowsCdpEvidenceTs=1783312139310`，`pixel_mismatch_ratio=0.1175684799382716`，`max_pixel_ratio=0.12`，`channel_tolerance=90`，runtime pass。
- 用户反馈回修：综合态势二级菜单必须包含 `专题面板`；态势大屏右侧 `风险区域密度` 与 `外联流向强度` 必须呈现世界地图轮廓，且业务图示不能用图片替代。当前实现将两张地图拆为 ECharts/React 数据驱动图示：大陆轮廓为内联矢量 path，风险热点与外联流向为 `scatter`/`lines` 数据层，可由 API 数据动态更新；截图/图片资源仅用于独立图标或徽标类元素。
- 用户反馈回修 r39：`战役簇密度` 不能只是 custom 线条或普通 scatter，必须按业务动态图示规则使用 ECharts 密度图。当前实现改为 `CampaignDensityChart` 的 ECharts `heatmap` + `scatter` 组合：API/typed fallback 的战役阶段点位会确定性展开为密度簇，叠加 radar grid、高/中/低 legend，不使用截图或位图资源。证据 crop：`evidence/ui-image-breakdowns/pages/screen/production-campaign-density-r39-crop.png`。
- 用户反馈回修 r41：`采集与流处理管道（全流量处理链路）` 与 UI 图不一致、箭头不美观。当前实现改为数据驱动的紧凑工序卡：每个节点包含图标/名称、两行业务指标、ECharts `SparklineChart` 趋势线，节点之间使用 CSS 流向箭头，详情入口定位在管道面板内部，不使用截图或位图资源。证据 crop：`evidence/ui-image-breakdowns/pages/screen/production-pipeline-r41-crop.png`。
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 态势大屏 | `80,54,360,24` | title | 是 |
| screen | `510,54,360,24` | subtitle | 是 |
| /screen | `940,54,360,24` | section | 是 |
| 态势大屏。 | `1370,54,360,24` | label | 是 |
| 领导汇报、售前演示、值班大屏和验收展示，一屏讲清全流量采集分析闭环。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：仪表盘、态势大屏、专题面板，当前高亮“态势大屏”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：态势大屏。 | `80,306,360,24` | metric | 是 |
| 路由：/screen。 | `510,306,360,24` | button | 是 |
| 业务模块：综合态势 / 态势大屏。 | `940,306,360,24` | status | 是 |
| 页面重点：领导汇报、售前演示、值班大屏和验收展示，一屏讲清全流量采集分析闭环。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 园区拓扑总览：楼宇覆盖率、园区在线覆盖、核心链路状态、汇聚链路状态、异常链路位置、探针覆盖地图。 | `510,348,360,24` | acceptance | 是 |
| - 威胁态势：攻击阶段热度、战役簇密度、风险区域密度、异常链路影响面、外联流向强度。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`态势大屏`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`screen`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/screen`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`态势大屏。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`领导汇报、售前演示、值班大屏和验收展示，一屏讲清全流量采集分析闭环。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：仪表盘、态势大屏、专题面板，当前高亮“态势大屏”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：态势大屏。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/screen。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：综合态势 / 态势大屏。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：领导汇报、售前演示、值班大屏和验收展示，一屏讲清全流量采集分析闭环。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 园区拓扑总览：楼宇覆盖率、园区在线覆盖、核心链路状态、汇聚链路状态、异常链路位置、探针覆盖地图。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 威胁态势：攻击阶段热度、战役簇密度、风险区域密度、异常链路影响面、外联流向强度。`，类型 title，来源 manual visual ledger。
...
| 控件/区域 | 状态 | 触发方式 | 期望表现 |
|---|---|---|---|
| `target-loaded` | ready | load image route in Windows Chrome CDP | 1920x1080 content is visible without browser chrome or scroll shift |
...
- 交互 `target-loaded`：状态 ready。
  触发：load image route in Windows Chrome CDP
...
- 映射说明：The evidence screenshot is captured from the real `/screen` production route; the breakdown arrays map the same image to semantic frontend units.
...
- 目标图：`evidence/ui-image-breakdowns/pages/screen/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/screen/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/screen/diff.png`
...
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/screen/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/screen/metrics.json`
...
- 生产 URL：`http://10.0.5.8:30180/screen?__codex_ui_breakdown_production=1&windowsCdpEvidenceTs=1783312139310`
- 最终 URL：`http://10.0.5.8:30180/screen?__codex_ui_breakdown_production=1&windowsCdpEvidenceTs=1783312139310`
- 管道目标裁剪：`evidence/ui-image-breakdowns/pages/screen/target-pipeline-r41-crop.png`
- 管道生产裁剪：`evidence/ui-image-breakdowns/pages/screen/production-pipeline-r41-crop.png`
- 复现步骤：先检查 `curl -i --max-time 5 http://127.0.0.1:9224/json/version` 和 `/json/list`，再用 Windows Chrome CDP 打开真实生产路由，按 `--channel-tolerance 90 --max-pixel-ratio 0.12` 生成 implementation.png、diff.png、metrics.json，并读取 verification.json。

## 差异清单

| 类型 | 位置 | 当前 | 期望 | 状态 |
|---|---|---|---|---|
| visual-diff | full image | alpha production diff ratio `0.1175684799382716`, channel tolerance `90` | pixel mismatch ratio <= 0.12 under alpha business gate | accepted |
| pipeline-business-diagram | 采集与流处理管道 | React process cards + ECharts sparkline + CSS arrows, detail link contained in panel | business pipeline must be dynamic, readable, and visually close to target | fixed-r41 |
...
- 主线程判定覆盖当前 alpha 视觉门禁：内容和布局一致；亮度、透明度、glow 强度、字体抗锯齿和密集拓扑线条差异不阻塞。

## r247 业务约束回归

- 生产镜像：`traffic/web-ui:ui-screen-echarts-20260711-r247`。园区覆盖地图和园区数字孪生拓扑均由 ECharts canvas 渲染，数据来自页面 API 快照或同构 typed fallback。
- 交互：`interaction-r247.json` 覆盖两个新增 canvas、2D 切换与节点选择；拓扑 ECharts 关系层采用 `pointer-events: none`，不会遮挡既有业务节点操作。
- 业务 ROI：`metrics-business-r247.json` 为 `0.09511460514200094`，低于 `0.125`；整图诊断 `metrics.json` 为 `0.09130738811728395`，同样通过。

## r259 原始代码回退

- 当前生产镜像：`traffic/web-ui:ui-screen-original-svg-20260711-r259`。态势大屏 `ProbeCoverageMap` 和 `TopologyTwinLayer` 已恢复原始 API 驱动 SVG 代码，r247 的两个拓扑 ECharts canvas 不再是当前实现。
- 原始园区轮廓、分区、道路、建筑、链路、探针节点和 2D/3D 状态均已恢复；节点选择和拓扑下钻状态保持可用，其他非拓扑 ECharts 图表不受本次回退影响。
- `interaction-r259-original-svg.json` 验证探针覆盖 SVG 与园区拓扑 SVG 各 1 个、2D 状态和节点选择有效，且无 4xx/5xx、请求失败、console error 或 page error。
- `metrics-business-r259.json` 的业务 ROI 为 `0.10485385738730421 <= 0.125`，diff 为 `diff-business-r259.png`，评分区域为 `content-root:198,80,1722,917`。
