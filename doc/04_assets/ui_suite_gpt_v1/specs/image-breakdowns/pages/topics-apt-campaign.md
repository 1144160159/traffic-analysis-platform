# topics-apt-campaign.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：Topics Apt Campaign
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/topics-apt-campaign.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：无直接 prompt
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| Topics Apt Campaign | `80,54,360,24` | title | 是 |
| topics-apt-campaign | `510,54,360,24` | subtitle | 是 |
| /topics | `940,54,360,24` | section | 是 |
| 园区网络全流量采集与分析系统 | `1370,54,360,24` | label | 是 |
| 综合态势 | `80,96,360,24` | metric | 是 |
| 采集监测 | `510,96,360,24` | button | 是 |
| 威胁分析 | `940,96,360,24` | status | 是 |
| 资产图谱 | `1370,96,360,24` | legend | 是 |
| 检测运营 | `80,138,360,24` | hint | 是 |
| 审计配置 | `510,138,360,24` | acceptance | 是 |
| PCAP检索 | `940,138,360,24` | title | 是 |
| 资产检索 | `1370,138,360,24` | subtitle | 是 |
| 规则检索 | `80,180,360,24` | section | 是 |
| 脚本中心 | `510,180,360,24` | label | 是 |
| 帮助中心 | `940,180,360,24` | metric | 是 |
| 更多应用 | `1370,180,360,24` | button | 是 |
| 数据延迟 | `80,222,360,24` | status | 是 |
| 系统运行 | `510,222,360,24` | legend | 是 |
| 告警处理SLA | `940,222,360,24` | hint | 是 |
| 数据质量合格率 | `1370,222,360,24` | acceptance | 是 |
| 存储使用 | `80,264,360,24` | title | 是 |
| 带宽使用 | `510,264,360,24` | subtitle | 是 |
| 日志吞吐 | `940,264,360,24` | section | 是 |
| 页面标题 | `1370,264,360,24` | label | 是 |
| 筛选条件 | `80,306,360,24` | metric | 是 |
| 近24小时 | `510,306,360,24` | button | 是 |
| 高危告警 | `940,306,360,24` | status | 是 |
| 证据完整率 | `1370,306,360,24` | legend | 是 |
| 处置中 | `80,348,360,24` | hint | 是 |
| 已审计 | `510,348,360,24` | acceptance | 是 |
| 查看详情 | `940,348,360,24` | title | 是 |
| 导出证据 | `1370,348,360,24` | subtitle | 是 |
| 反馈学习 | `80,390,360,24` | section | 是 |
| Topics Apt Campaign 视觉校正项 34 | `510,390,360,24` | label | 是 |
| Topics Apt Campaign 视觉校正项 35 | `940,390,360,24` | metric | 否 |
| Topics Apt Campaign 视觉校正项 36 | `1370,390,360,24` | button | 否 |
| Topics Apt Campaign 视觉校正项 37 | `80,432,360,24` | status | 否 |
| Topics Apt Campaign 视觉校正项 38 | `510,432,360,24` | legend | 否 |
| Topics Apt Campaign 视觉校正项 39 | `940,432,360,24` | hint | 否 |
| Topics Apt Campaign 视觉校正项 40 | `1370,432,360,24` | acceptance | 否 |
| Topics Apt Campaign 视觉校正项 41 | `80,474,360,24` | title | 否 |
| Topics Apt Campaign 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`Topics Apt Campaign`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`topics-apt-campaign`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/topics`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`园区网络全流量采集与分析系统`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`综合态势`，类型 metric，来源 prompt/layer plus target visual ledger。
- 文本 06：`采集监测`，类型 button，来源 prompt/layer plus target visual ledger。
- 文本 07：`威胁分析`，类型 status，来源 prompt/layer plus target visual ledger。
- 文本 08：`资产图谱`，类型 legend，来源 prompt/layer plus target visual ledger。
- 文本 09：`检测运营`，类型 hint，来源 prompt/layer plus target visual ledger。
- 文本 10：`审计配置`，类型 acceptance，来源 prompt/layer plus target visual ledger。
- 文本 11：`PCAP检索`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 12：`资产检索`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 13：`规则检索`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 14：`脚本中心`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 15：`帮助中心`，类型 metric，来源 prompt/layer plus target visual ledger。
- 文本 16：`更多应用`，类型 button，来源 prompt/layer plus target visual ledger。
- 文本 17：`数据延迟`，类型 status，来源 prompt/layer plus target visual ledger。
- 文本 18：`系统运行`，类型 legend，来源 prompt/layer plus target visual ledger。
- 文本 19：`告警处理SLA`，类型 hint，来源 manual visual ledger。
- 文本 20：`数据质量合格率`，类型 acceptance，来源 manual visual ledger。
- 文本 21：`存储使用`，类型 title，来源 manual visual ledger。
- 文本 22：`带宽使用`，类型 subtitle，来源 manual visual ledger。
- 文本 23：`日志吞吐`，类型 section，来源 manual visual ledger。
- 文本 24：`页面标题`，类型 label，来源 manual visual ledger。
- 文本 25：`筛选条件`，类型 metric，来源 manual visual ledger。
- 文本 26：`近24小时`，类型 button，来源 manual visual ledger。
- 文本 27：`高危告警`，类型 status，来源 manual visual ledger。
- 文本 28：`证据完整率`，类型 legend，来源 manual visual ledger。
- 文本 29：`处置中`，类型 hint，来源 manual visual ledger。
- 文本 30：`已审计`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`查看详情`，类型 title，来源 manual visual ledger。
- 文本 32：`导出证据`，类型 subtitle，来源 manual visual ledger。
- 文本 33：`反馈学习`，类型 section，来源 manual visual ledger。
- 文本 34：`Topics Apt Campaign 视觉校正项 34`，类型 label，来源 manual visual ledger。
- 文本 35：`Topics Apt Campaign 视觉校正项 35`，类型 metric，来源 manual visual ledger。
- 文本 36：`Topics Apt Campaign 视觉校正项 36`，类型 button，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/topics-apt-campaign/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/topics-apt-campaign/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/topics-apt-campaign/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/topics-apt-campaign/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/topics-apt-campaign/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/topics-apt-campaign/text-ocr.txt`
...
- 生产路由：`http://10.0.5.8:30180/topics?topic=apt&tab=apt&__codex_ui_breakdown_production=1&windowsCdpEvidenceTs=1783237823914`
- 页面状态：`topic=apt`、`tab=apt`，可见标签复用，1920 x 1080，无横向/纵向滚动。
- 复现步骤：锁定 target.png，打开 30180 真实生产路由，使用 Windows Chrome CDP 截图，生成 diff.png 和 metrics.json，读取 verification.json。
...
- 2026-07-05 已完成可见 Windows Chrome CDP + 30180 真实生产路由重截。
- Alpha 验收口径：内容、布局、菜单、业务组件、交互状态一致；亮度、透明度、抗锯齿和密集微文本差异按 alpha 规则放宽。
- `metrics.json`：`status=pass`，`channel_tolerance=92`，`pixel_mismatch_ratio=0.0882957175925926`，阈值 `<=0.1`。
- `capture-meta.json`：`runtime_status=pass`，无 console/pageerror/requestfailed/bad response/forbidden target resource requests，无横向或纵向滚动。
- 主线程判定：`pixel-accepted`，证据见本目录 `implementation.png`、`diff.png`、`metrics.json`、`capture-meta.json`、`verification.json`。

## r245 业务约束回归

- 生产镜像：`traffic/web-ui:ui-topic-graphs-20260711-r245`；仅回修专题业务区域，公共顶部、左侧和底部壳层未作为像素回修对象。
- ECharts：加密隧道局部影响面、APT 战役攻击关系、APT 战役事件趋势和处置动作状态均由页面 API 契约或 typed fallback 数据驱动的 ECharts canvas 渲染。
- 交互：`interaction-r245.json` 已覆盖隧道、数据外传、APT 三个分支的图表 canvas、分析标签切换、表格第 2 页、表格滚动容器及确认 Drawer；`bad_responses`、`console_errors`、`page_errors` 均为空。
- 业务 ROI：`metrics-business-r245.json` 为 `0.10059123258314683`，低于 `0.125`；整图诊断 `metrics.json` 为 `0.09548707561728395`，同样通过。

## r259 拓扑 SVG 回退

- 当前生产镜像：`traffic/web-ui:ui-screen-original-svg-20260711-r259`。加密隧道局部影响面和 APT 战役攻击关系已由 ECharts graph 回退为 API/typed fallback 驱动 SVG；数据外传关系图、APT 趋势和处置分布等非拓扑图表继续使用 ECharts。
- `interaction-r259-topology-svg.json` 覆盖 tunnel、exfil、apt 三个专题状态，验证两张拓扑 SVG、数据外传 canvas、APT 趋势/处置 canvas、分析标签、第 2 页、滚动容器和动作确认 Drawer；运行时错误数组均为空。
- `metrics-business-r259.json` 的 APT 业务 ROI 为 `0.09128387903290155 <= 0.125`，diff 为 `diff-business-r259.png`，评分区域为 `content-root:198,80,1722,917`。
