# notifications.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：通知配置
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/notifications.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/notifications.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/notifications.json`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 通知配置 | `80,54,360,24` | title | 是 |
| notifications | `510,54,360,24` | subtitle | 是 |
| /notifications | `940,54,360,24` | section | 是 |
| 通知配置。 | `1370,54,360,24` | label | 是 |
| 配置告警、系统异常、验收缺口和运营任务的通知渠道与升级策略。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“通知配置”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：通知配置。 | `80,306,360,24` | metric | 是 |
| 路由：/notifications。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 通知配置。 | `940,306,360,24` | status | 是 |
| 页面重点：配置告警、系统异常、验收缺口和运营任务的通知渠道与升级策略。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 通知渠道：邮件、短信、Webhook、企业微信/钉钉、工单系统。 | `510,348,360,24` | acceptance | 是 |
| - 订阅规则：严重级别、告警类型、资产组、园区、时间窗、接收人。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`通知配置`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`notifications`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/notifications`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`通知配置。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`配置告警、系统异常、验收缺口和运营任务的通知渠道与升级策略。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“通知配置”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：通知配置。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/notifications。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：审计配置 / 通知配置。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：配置告警、系统异常、验收缺口和运营任务的通知渠道与升级策略。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 通知渠道：邮件、短信、Webhook、企业微信/钉钉、工单系统。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 订阅规则：严重级别、告警类型、资产组、园区、时间窗、接收人。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/notifications/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/notifications/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/notifications/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/notifications/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/notifications/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/notifications/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/notifications/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/notifications/verification.json`
...
## 2026-07-11 r254 业务实现与验证

- 生产业务实现：`web/ui/src/pages/NotificationConfigPage.tsx`；仅调整通知配置业务区域和业务样式，公共 topbar/sidebar/bottombar 未改动。
- 数据与接口：页面快照使用 `fetchPageSnapshot(route.id)`；渠道趋势由 typed fallback 仿真数据驱动的 ECharts canvas 呈现；设置、测试发送与静默规则动作预留在 `pageApiPlans.notifications`。
- 动态图：6 个渠道卡片各有一个 ECharts 送达趋势图；渠道开关会更新本地仿真状态和趋势数据，同时打开带接口与审计事件说明的确认 Drawer。
- 可用交互：订阅规则为 6 条/页受控分页，表格保留稳定的纵横滚动；条件构造、升级策略、发送历史筛选、模板和静默窗口分页，以及所有顶部业务动作均具有可观察状态或确认提交反馈。
- Windows Chrome 交互证据：`evidence/ui-image-breakdowns/pages/notifications/interaction-r254.json`，Chrome `150.0.7871.49`，验证 6 张 ECharts canvas、第 2 页、`overflow-y: auto`、渠道开关状态、新增渠道确认、模板编辑 Drawer 和静默规则端点映射；4xx/5xx、`requestfailed`、console error 与 page error 均为 0。
- 业务 ROI：`evidence/ui-image-breakdowns/pages/notifications/metrics-business-r254.json`，业务区域 `content-root (198,80,1722,917)` 的 mismatch ratio 为 `0.0748470306014791`，低于 `0.125`；整图诊断为 `0.07531684027777778`。
