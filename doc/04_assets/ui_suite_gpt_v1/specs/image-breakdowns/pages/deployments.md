# deployments.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：部署管理
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/deployments.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/deployments.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/deployments.json`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 部署管理 | `80,54,360,24` | title | 是 |
| deployments | `510,54,360,24` | subtitle | 是 |
| /deployments | `940,54,360,24` | section | 是 |
| 部署管理。 | `1370,54,360,24` | label | 是 |
| 统一管理规则、模型、采集策略、Flink 作业和配置的发布、灰度、回滚。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“部署管理”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：部署管理。 | `80,306,360,24` | metric | 是 |
| 路由：/deployments。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / 部署管理。 | `940,306,360,24` | status | 是 |
| 页面重点：统一管理规则、模型、采集策略、Flink 作业和配置的发布、灰度、回滚。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 发布清单：发布对象、版本、环境、状态、负责人、时间、影响范围。 | `510,348,360,24` | acceptance | 是 |
| - 灰度策略：按租户、园区、探针、资产组、流量比例灰度。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`部署管理`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`deployments`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/deployments`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`部署管理。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`统一管理规则、模型、采集策略、Flink 作业和配置的发布、灰度、回滚。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“部署管理”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：部署管理。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/deployments。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：检测运营 / 部署管理。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：统一管理规则、模型、采集策略、Flink 作业和配置的发布、灰度、回滚。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 发布清单：发布对象、版本、环境、状态、负责人、时间、影响范围。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 灰度策略：按租户、园区、探针、资产组、流量比例灰度。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/deployments/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/deployments/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/deployments/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/deployments/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/deployments/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/deployments/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/deployments/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/deployments/verification.json`
...
- 当前生产镜像：`traffic/web-ui:ui-screen-original-svg-20260711-r259`。部署管理业务页使用 6 张 API/typed fallback 驱动的 ECharts sparkline 展示发布健康，已修正子级图表受通用网格选择器影响而被裁切的问题。
- 发布列表提供 7 条/页的受控分页和纵向滚动；新建发布、行级操作和发布证据均产生可观察 Drawer，公共 AppShell 未修改。
- `interaction-r259.json` 验证 6 张 canvas、第 2 页、`overflow-y: scroll` 和三类动作，且无 4xx/5xx、请求失败、console error 或 page error。
- `metrics-business-r259.json` 的业务 ROI 为 `0.07075855849694188 <= 0.125`，diff 为 `diff-business-r259.png`，评分区域为 `content-root:198,80,1722,917`。
