# campaigns.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：战役列表
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/campaigns.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/campaigns.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/campaigns.json`
- 对应路由/宿主路由：`/campaigns`
- 当前状态：`business-pixel-accepted`
- 生产镜像：`traffic/web-ui:ui-campaigns-visual-20260710-r165`
- 生产路由：`http://10.0.5.8:30180/campaigns`
- 生产证据：`evidence/ui-image-breakdowns/pages/campaigns/implementation-r165-final.png`
- 验收边界：r165 生产 React 页面经 Windows Chrome CDP 真实路由截图、runtime、diff/metrics、智能体辅助审查和主线程判断通过；目标 PNG 与 live API 数值差异按真实数据差异接受。
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 战役列表 | `80,54,360,24` | title | 是 |
| campaigns | `510,54,360,24` | subtitle | 是 |
| /campaigns | `940,54,360,24` | section | 是 |
| 战役列表。 | `1370,54,360,24` | label | 是 |
| 把多条告警聚合成事件/战役，支持跨时间、跨资产、跨阶段调查。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“战役列表”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：战役列表。 | `80,306,360,24` | metric | 是 |
| 路由：/campaigns。 | `510,306,360,24` | button | 是 |
| 业务模块：威胁分析 / 战役列表。 | `940,306,360,24` | status | 是 |
| 页面重点：把多条告警聚合成事件/战役，支持跨时间、跨资产、跨阶段调查。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 战役总览：战役数量、活跃战役、影响资产、最高风险、持续时间。 | `510,348,360,24` | acceptance | 是 |
| - 战役列表：战役名称、阶段、风险、资产数、告警数、负责人、状态。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`战役列表`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`campaigns`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/campaigns`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`战役列表。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`把多条告警聚合成事件/战役，支持跨时间、跨资产、跨阶段调查。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“战役列表”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：战役列表。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/campaigns。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：威胁分析 / 战役列表。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：把多条告警聚合成事件/战役，支持跨时间、跨资产、跨阶段调查。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 战役总览：战役数量、活跃战役、影响资产、最高风险、持续时间。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 战役列表：战役名称、阶段、风险、资产数、告警数、负责人、状态。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/campaigns/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/campaigns/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/campaigns/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/campaigns/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/campaigns/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/campaigns/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/campaigns/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/campaigns/verification.json`
