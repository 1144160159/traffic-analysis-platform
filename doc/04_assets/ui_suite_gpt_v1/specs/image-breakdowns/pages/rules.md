...
- 分类：pages
- 标题：规则管理
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/rules.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/rules.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/rules.json`
- 对应路由/宿主路由：`/rules`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 规则管理 | `80,54,360,24` | title | 是 |
| rules | `510,54,360,24` | subtitle | 是 |
| /rules | `940,54,360,24` | section | 是 |
| 规则管理。 | `1370,54,360,24` | label | 是 |
| 管理检测规则生命周期，覆盖创建、测试、发布、命中、误报、回滚。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“规则管理”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：规则管理。 | `80,306,360,24` | metric | 是 |
| 路由：/rules。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / 规则管理。 | `940,306,360,24` | status | 是 |
| 页面重点：管理检测规则生命周期，覆盖创建、测试、发布、命中、误报、回滚。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 规则列表：规则名、类型、严重级别、状态、版本、命中数、误报率。 | `510,348,360,24` | acceptance | 是 |
| - 规则编辑：条件、特征、阈值、MITRE 阶段、适用范围、例外条件。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`规则管理`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`rules`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/rules`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`规则管理。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`管理检测规则生命周期，覆盖创建、测试、发布、命中、误报、回滚。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“规则管理”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：规则管理。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/rules。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：检测运营 / 规则管理。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：管理检测规则生命周期，覆盖创建、测试、发布、命中、误报、回滚。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 规则列表：规则名、类型、严重级别、状态、版本、命中数、误报率。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 规则编辑：条件、特征、阈值、MITRE 阶段、适用范围、例外条件。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/rules/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/rules/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/rules/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/rules/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/rules/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/rules/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/rules/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/rules/verification.json`
...
- 生产镜像：`traffic/web-ui:ui-rules-interactions-20260711-r250`；路由：`/rules`；浏览器：Windows Chrome `150.0.7871.49`，CDP `http://127.0.0.1:9224`。
- 业务图表：平均延时、P95 延时、CPU 占用和内存占用均为 `DataQualityKpiSparklineChart` ECharts canvas，使用规则页面 API 的同构 typed fallback 数据。
- 业务交互：`interaction-r250.json` 已验证第 2 页、列表滚动、新建规则确认、规则编辑 Tab、样本回放 Tab 和全量发布 Drawer；无坏响应、request failure、console error 或 page error。
- 业务区域评分：`content-root` bbox `198,80,1722,917` 的 mismatch ratio 为 `0.07719207586218252 <= 0.125`；公共 topbar/sidebar/bottombar 仅作为整图诊断，不作为业务区回修范围。
