# alert-detail-evidence-logs.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：告警详情。
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/alert-detail-evidence-logs.png`
- 源图尺寸：1920 x 1080
...
- 复刻等级：r146 已由真实生产路由、Windows Chrome CDP 截图、runtime 检查、diff/metrics 和主线程判定闭环。
- 验收边界：本图是告警详情证据链的日志证据面板态；目标图不包含 AppShell，业务内容由 React/CSS 和 typed API/fallback 数据驱动，不加载目标截图。
...
- type：`menu-state`
- parent：`alerts`
- target UI：`doc/04_assets/ui_suite_gpt_v1/screens/pages/alert-detail-evidence-logs.png`
- 生产 URL：`http://10.0.5.8:30180/alerts/AL-20260620-000123?__codex_ui_breakdown_production=1&__codex_page_id=alert-detail-evidence-logs&evidenceView=logs&__capture=r146-final`
- image tag：`traffic/web-ui:ui-alert-detail-evidence-logs-visual-20260709-r146`
- viewport：`1920 x 1080`
- runtime：无 console/pageerror/requestfailed/HTTP 4xx/5xx，无禁用资源，无水平溢出。
- diff：`metrics-r146.json` 与 `metrics-business-r146.json` 均为 `pass`，业务 mismatch ratio `0.08773582175925926 <= 0.12`。
- 主线程判断：`business-pixel-accepted`。

## 实际目标图结构

- 顶部：`证据链（6）` 标题与证据分类 Tab，`日志 1` 为 active。
- 表格：列为 `证据类型 / 日志文件 / 来源 / 命中字段 / 内容摘要 / 生成时间 / 状态 / 操作`。
- 日志行：`ids-20260620-000123.log`，来源 `IDS / 探针-07`，命中 `rule=C2_Tunnel_v3, ja3_score=0.91`，状态 `已生成`。
- 关键字段：`dst_ip=185.22.14.9`、`sni=null`、`bytes_out_p95=5.8MB`、`user_event=svc_backup login`。
- 来源标签：`设备日志`、`规则命中`、`用户事件`。
- 底部入口：`查看全部 日志 1 项 >`。

## 数据与组件映射

- 页面组件：`web/ui/src/pages/AlertDetailPage.tsx` 中 `AlertEvidenceLogsFocusView`。
- 数据来源：`web/ui/src/services/alertDetailApi.ts` 的 `AlertDetailEvidenceRow.logEvidence` typed API/fallback。
- 字段映射：`logFile`、`source`、`hitFields`、`contentSummary`、`generatedAt`、`status`、`highlightedFields`、`sourceTags`。
- 样式实现：`web/ui/src/styles/pages.css` 中 `.taf-alert-evidence-logs-*`。
- 动态图示分类：本页无业务动态图示；表格、字段 chip、标签和图标均由数据/React/SVG/AntD 组件生成，不使用整卡、整表、整图截图资源。
...
| alert-detail-evidence-logs | `510,54,360,24` | subtitle | 是 |
| /alerts/:alertId | `940,54,360,24` | section | 是 |
| 单条告警研判详情，保留告警上下文并完成证据、资产、响应、反馈闭环。 | `1370,54,360,24` | label | 是 |
...
| 左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“告警中心”。 | `940,138,360,24` | title | 是 |
...
| 页面名称：告警详情。 | `1370,180,360,24` | button | 是 |
| 路由：/alerts/:alertId。 | `80,222,360,24` | status | 是 |
| 业务模块：威胁分析 / 告警中心。 | `510,222,360,24` | legend | 是 |
| 页面重点：单条告警研判详情，保留告警上下文并完成证据、资产、响应、反馈闭环。 | `940,222,360,24` | hint | 是 |
| 必须包含的业务模块： | `1370,222,360,24` | acceptance | 是 |
| - 研判摘要：告警 ID、严重级别、风险评分、置信度、状态、责任人。 | `80,264,360,24` | title | 是 |
| - 资产上下文：源/目的 IP、主机、服务、业务系统、最近风险画像。 | `510,264,360,24` | subtitle | 是 |
...
- 文本 02：`alert-detail-evidence-logs`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/alerts/:alertId`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`单条告警研判详情，保留告警上下文并完成证据、资产、响应、反馈闭环。`，类型 label，来源 prompt/layer plus target visual ledger。
...
- 文本 11：`左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“告警中心”。`，类型 title，来源 prompt/layer plus target visual ledger。
...
- 文本 16：`页面名称：告警详情。`，类型 button，来源 prompt/layer plus target visual ledger。
- 文本 17：`路由：/alerts/:alertId。`，类型 status，来源 prompt/layer plus target visual ledger。
- 文本 18：`业务模块：威胁分析 / 告警中心。`，类型 legend，来源 prompt/layer plus target visual ledger。
- 文本 19：`页面重点：单条告警研判详情，保留告警上下文并完成证据、资产、响应、反馈闭环。`，类型 hint，来源 manual visual ledger。
- 文本 20：`必须包含的业务模块：`，类型 acceptance，来源 manual visual ledger。
- 文本 21：`- 研判摘要：告警 ID、严重级别、风险评分、置信度、状态、责任人。`，类型 title，来源 manual visual ledger。
- 文本 22：`- 资产上下文：源/目的 IP、主机、服务、业务系统、最近风险画像。`，类型 subtitle，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/alert-detail-evidence-logs/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/alert-detail-evidence-logs/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/alert-detail-evidence-logs/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/alert-detail-evidence-logs/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/alert-detail-evidence-logs/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/alert-detail-evidence-logs/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/alert-detail-evidence-logs/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/alert-detail-evidence-logs/verification.json`
