# whitelist-expiry-expired-unhandled.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：白名单。
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/whitelist-expiry-expired-unhandled.png`
- 源图尺寸：1920 x 1080
...
| whitelist-expiry-expired-unhandled | `510,54,360,24` | subtitle | 是 |
| /whitelist | `940,54,360,24` | section | 是 |
| 管理业务例外和误报沉淀，避免白名单变成不可控的检测盲区。 | `1370,54,360,24` | label | 是 |
...
| 左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“白名单”。 | `940,138,360,24` | title | 是 |
...
| 页面名称：白名单。 | `1370,180,360,24` | button | 是 |
| 路由：/whitelist。 | `80,222,360,24` | status | 是 |
| 业务模块：检测运营 / 白名单。 | `510,222,360,24` | legend | 是 |
| 页面重点：管理业务例外和误报沉淀，避免白名单变成不可控的检测盲区。 | `940,222,360,24` | hint | 是 |
| 必须包含的业务模块： | `1370,222,360,24` | acceptance | 是 |
| - 白名单列表：对象类型、匹配条件、生效范围、有效期、责任人、来源告警。 | `80,264,360,24` | title | 是 |
| - 新增白名单：IP、资产、账号、域名、规则、模型、时间窗、例外原因。 | `510,264,360,24` | subtitle | 是 |
...
- 文本 02：`whitelist-expiry-expired-unhandled`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/whitelist`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`管理业务例外和误报沉淀，避免白名单变成不可控的检测盲区。`，类型 label，来源 prompt/layer plus target visual ledger。
...
- 文本 11：`左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“白名单”。`，类型 title，来源 prompt/layer plus target visual ledger。
...
- 文本 16：`页面名称：白名单。`，类型 button，来源 prompt/layer plus target visual ledger。
- 文本 17：`路由：/whitelist。`，类型 status，来源 prompt/layer plus target visual ledger。
- 文本 18：`业务模块：检测运营 / 白名单。`，类型 legend，来源 prompt/layer plus target visual ledger。
- 文本 19：`页面重点：管理业务例外和误报沉淀，避免白名单变成不可控的检测盲区。`，类型 hint，来源 manual visual ledger。
- 文本 20：`必须包含的业务模块：`，类型 acceptance，来源 manual visual ledger。
- 文本 21：`- 白名单列表：对象类型、匹配条件、生效范围、有效期、责任人、来源告警。`，类型 title，来源 manual visual ledger。
- 文本 22：`- 新增白名单：IP、资产、账号、域名、规则、模型、时间窗、例外原因。`，类型 subtitle，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/whitelist-expiry-expired-unhandled/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/whitelist-expiry-expired-unhandled/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/whitelist-expiry-expired-unhandled/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/whitelist-expiry-expired-unhandled/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/whitelist-expiry-expired-unhandled/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/whitelist-expiry-expired-unhandled/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/whitelist-expiry-expired-unhandled/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/whitelist-expiry-expired-unhandled/verification.json`
