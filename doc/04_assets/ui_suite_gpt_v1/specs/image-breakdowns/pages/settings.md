...
- 分类：pages
- 标题：系统设置
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/settings.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/settings.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/settings.json`
- 对应路由/宿主路由：`/settings`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 系统设置 | `80,54,360,24` | title | 是 |
| settings | `510,54,360,24` | subtitle | 是 |
| /settings | `940,54,360,24` | section | 是 |
| 系统设置。 | `1370,54,360,24` | label | 是 |
| 管理租户、角色权限、令牌、数据留存、集成凭据和系统级参数。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“系统设置”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：系统设置。 | `80,306,360,24` | metric | 是 |
| 路由：/settings。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 系统设置。 | `940,306,360,24` | status | 是 |
| 页面重点：管理租户、角色权限、令牌、数据留存、集成凭据和系统级参数。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 租户与站点：租户、园区、部门、资产组、数据隔离范围。 | `510,348,360,24` | acceptance | 是 |
| - 角色权限：安全值班员、研判员、管理员、审计员、只读大屏账号。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`系统设置`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`settings`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/settings`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`系统设置。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`管理租户、角色权限、令牌、数据留存、集成凭据和系统级参数。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“系统设置”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：系统设置。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/settings。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：审计配置 / 系统设置。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：管理租户、角色权限、令牌、数据留存、集成凭据和系统级参数。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 租户与站点：租户、园区、部门、资产组、数据隔离范围。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 角色权限：安全值班员、研判员、管理员、审计员、只读大屏账号。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/settings/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/settings/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/settings/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/settings/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/settings/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/settings/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/settings/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/settings/verification.json`
