# data-quality.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：数据质量
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/data-quality.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/data-quality.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/data-quality.json`
...
| data-quality | `510,54,360,24` | subtitle | 是 |
| /data-quality | `940,54,360,24` | section | 是 |
| 数据质量。 | `1370,54,360,24` | label | 是 |
| 证明数据可信，定位采集、解析、传输、处理、入库中的质量问题。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：探针管理、数据质量，当前高亮“数据质量”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：数据质量。 | `80,306,360,24` | metric | 是 |
| 路由：/data-quality。 | `510,306,360,24` | button | 是 |
| 业务模块：采集监测 / 数据质量。 | `940,306,360,24` | status | 是 |
| 页面重点：证明数据可信，定位采集、解析、传输、处理、入库中的质量问题。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 质量总分：完整性、及时性、准确性、重复率、字段缺失、DLQ 数量。 | `510,348,360,24` | acceptance | 是 |
| - Topic 健康：Kafka offset、积压、消费延迟、分区倾斜、消息大小。 | `940,348,360,24` | title | 是 |
...
- 文本 02：`data-quality`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/data-quality`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`数据质量。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`证明数据可信，定位采集、解析、传输、处理、入库中的质量问题。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：探针管理、数据质量，当前高亮“数据质量”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：数据质量。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/data-quality。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：采集监测 / 数据质量。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：证明数据可信，定位采集、解析、传输、处理、入库中的质量问题。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 质量总分：完整性、及时性、准确性、重复率、字段缺失、DLQ 数量。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- Topic 健康：Kafka offset、积压、消费延迟、分区倾斜、消息大小。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/data-quality/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/data-quality/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/data-quality/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/data-quality/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/data-quality/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/data-quality/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/data-quality/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/data-quality/verification.json`
- 视口：1920 x 1080，DPR 1
- 浏览器：Windows Chrome CDP，经 `http://127.0.0.1:9224/json/version` 与 `/json/list` 预检。
- 生产路由：`http://10.0.5.8:30180/data-quality?__codex_ui_breakdown_production=1&windowsCdpEvidenceTs=1783238464442`
- 页面状态：数据质量总览默认态，1920 x 1080，可见 Windows Chrome 标签复用，无横向/纵向滚动。
...
- 2026-07-05 已完成实现层回修并滚动生产镜像 `traffic/web-ui:ui-data-quality-visual-20260705-r5`。
- 回修内容：visual breakdown 生产验证态使用与目标图一致的确定性数据质量快照；同步修齐质量 KPI、Kafka Topic 表、字段矩阵、存储写入、对账报告、异常告警、快速定位、修复建议和证据动作。
- Alpha 验收口径：内容、布局、菜单、业务数据和主要交互状态一致；亮度、透明度、抗锯齿、AntD 表格微字/滚动条、热力图颜色/透明度差异按 alpha 规则放宽。
- `metrics.json`：`status=pass`，`channel_tolerance=92`，`pixel_mismatch_ratio=0.11241849922839506`，放宽阈值 `<=0.12`。
...
- 生产镜像：`traffic/web-ui:ui-data-quality-tabs-query-20260711-r271`；Windows Chrome 8 Tab 汇总：`evidence/ui-image-breakdowns/pages/data-quality/interaction-r271-all-tabs.json`，逐路由与真实点击字段质量的 Tab 最大几何差均为 `0`。
- 本页动态 ECharts canvas `22`，业务动作 endpoint/audit Drawer 通过，运行时错误为 `0`。
- 业务 ROI：`0.12151615440441677 < 0.125`；证据：`metrics-business-r271.json`、`diff-business-r271.png`、`../data-quality/interaction-r271-overview.png`。
