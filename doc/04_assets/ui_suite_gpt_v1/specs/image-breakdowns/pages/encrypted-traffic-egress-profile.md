# encrypted-traffic-egress-profile.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：加密流量。
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/encrypted-traffic-egress-profile.png`
- 源图尺寸：1920 x 1080
...
- 当前状态：`pixel-accepted-r189`
- 复刻等级：逐图目标 PNG 已锁定；r189 已完成真实生产路由截图、ECharts 运行时检查、diff、服务端审计写入、真实目的地/趋势 API 契约与双服务部署证据。
- 验收边界：业务门禁同时检查真实 React 页面、动态数据绑定、Windows Chrome 截图、视觉 diff、运行时错误和写操作持久化；独立审查的 r188 问题已由本轮主线程复核关闭。

## r189 ECharts、真实目的地/趋势与 API 写入验证

- 路由状态：`/encrypted-traffic?tab=egress-profile`，激活标签为“外联画像”。
- React 实现：`web/ui/src/pages/EncryptedTrafficPage.tsx` 的 `EgressProfileContent` 组合外联目的地地图、域名画像卡、异常趋势、Top 外联目的地、实体关系图和处置栏。
- ECharts：全球外联流向使用 `WorldActivityMap`（custom land + scatter + lines），外联会话风险趋势使用 `ExfilStackedTrendChart`（五序列堆叠柱），实体关系使用 `ExfilGraphChart`（节点连线）；三者均由结构化 option/series 渲染，未嵌入页面截图。
- 数据边界：页面通过 `EncryptedTrafficVisuals` 接收 `/v1/encrypted-traffic/stats`、`sessions`、`ja3`、`tunnels`、`exfiltration` 的适配数据。`top_destinations` 的 `dst_ip` 是地图、排行、域名卡与图谱的唯一目的地标识；`top_sources.src_ip` 仅用于源资产统计。`trend` 直接提供目的地数、大流量会话、长会话、非标准端口与加密会话的 2 小时时间桶。只有目的地、路径和会话均为空时才使用类型化仿真样本，并在图表标题明确显示“仿真数据（API 空）”。
- 联动：目的地表、域名卡、地图散点/流线和实体图谱共用 `selectedEgressDestination`；Windows Chrome 点击 `104.16.12.34` 后，地图与实体图谱 aria 状态同步为该目的地。
- 写接口：`POST /v1/encrypted-traffic/egress-actions` 受 `alert:write` 保护，支持告警请求、证据查询、图谱下钻、审计写入与处置请求；成功提示只在 `audit_logs` 持久化成功后出现。告警请求表述为“待规则服务评估”，不伪称已创建线上告警。
- 回归守卫：`web/ui/src/routes/encryptedTrafficVisuals.test.ts` 锁定 ECharts 组件、地图节点点击、共享选择与 API 提交；`pageSnapshotAdapters.test.ts` 锁定 API 空数组时的显式仿真数据契约以及真实 `top_destinations`/`trend` 字段直通；`pageApiPlans.test.ts` 锁定五类写动作的 RBAC、审计和 APISIX 接口契约。
- 生产镜像：`traffic/web-ui:ui-encrypted-traffic-egress-profile-20260710-r189` 与 `traffic/alert-service:encrypted-egress-data-20260710-r188`，Kubernetes 两个 Deployment 均为 `1/1`，APISIX 路由返回 HTTP `200`。
- Windows Chrome：通过 `http://127.0.0.1:9224` 访问真实 APISIX 路由，浏览器为 `Chrome/150.0.7871.49`。
- 业务视觉门禁：1920 x 1080，容差 `64`，门限 `0.35`，像素差异比 `0.13109230324074075`，结果 `pass`。严格诊断仍单独保留，不能将业务门禁写成严格像素通过。
- 运行时门禁：无 4xx/5xx、无请求失败、无 console/page 错误、无横向溢出；Windows Chrome 实测到 5 个 canvas、地图/堆叠趋势/节点关系图各一张 ECharts 画布和仿真来源标签。点击 `104.16.12.34` 后三处选择状态联动；“写入审计”返回 HTTP `200`，并能按 action ID 读回 `ENCRYPTED_EGRESS_AUDIT_WRITTEN` 审计记录。
...
| encrypted-traffic-egress-profile | `510,54,360,24` | subtitle | 是 |
...
- 文本 02：`encrypted-traffic-egress-profile`，类型 subtitle，来源 prompt/layer plus target visual ledger。
...
- 实现模式：语义 React 生产页面，真实 API 优先、类型化回退补齐可选数据。
...
- 映射说明：`EgressProfileContent` 通过 `EncryptedTrafficVisuals` 映射 API 适配数据，ECharts/表格/地图连线/域名卡片均由结构化数据生成。
...
- 目标图：`evidence/ui-image-breakdowns/pages/encrypted-traffic-egress-profile/target.png`
- r189 真实路由截图：`evidence/ui-image-breakdowns/pages/encrypted-traffic-egress-profile/implementation.png`
- r189 业务 diff 图：`evidence/ui-image-breakdowns/pages/encrypted-traffic-egress-profile/diff.png`
- r189 ECharts、联动和审计写入：`evidence/ui-image-breakdowns/pages/encrypted-traffic-egress-profile/normal-route-r189-echarts-interactions-runtime.json`
- r189 外传 API：`evidence/ui-image-breakdowns/pages/encrypted-traffic-egress-profile/r189-exfiltration-api-runtime.json`
- r189 验收包：`doc/02_acceptance/02-regression/ui-visual-interaction/encrypted-traffic-egress-profile-r189/`
- regions overlay：`evidence/ui-image-breakdowns/pages/encrypted-traffic-egress-profile/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/encrypted-traffic-egress-profile/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/encrypted-traffic-egress-profile/text-ocr.txt`
- 业务 metrics：`evidence/ui-image-breakdowns/pages/encrypted-traffic-egress-profile/metrics.json`
- 真实路由运行时：`evidence/ui-image-breakdowns/pages/encrypted-traffic-egress-profile/production-route-report.json`
- verification：`evidence/ui-image-breakdowns/pages/encrypted-traffic-egress-profile/verification.json`
- 视口：1920 x 1080，DPR 1
- 浏览器：Windows Chrome CDP，经 `http://127.0.0.1:9224/json/version` 与 `/json/list` 预检。
- 复现步骤：构建 r189 Web UI，部署到 Kubernetes，打开真实 APISIX 路由，使用 Windows Chrome CDP 截图、点击目的地、提交“写入审计”，生成业务 diff/metrics 并读取 verification.json。

## 差异清单

| 类型 | 位置 | 当前 | 期望 | 状态 |
|---|---|---|---|---|
| business-visual-diff | full image | 业务门禁像素差异比 `0.13109230324074075` | `<= 0.35`，通道容差 `64` | pass |
| live-egress-data | `/v1/encrypted-traffic/exfiltration` | 当前四类外传集合均为空，页面明确显示类型化仿真数据 | 有数据时直通 `top_destinations` 和 `trend` | documented-nonblocking |
| semantic-scope | 生产 React 实现 | API 数据绑定与类型化回退已实现 | 图表、表格、地图和域名卡不依赖截图 | pass |
| independent-review | 独立审查 | r188 审查的真实目的地、真实趋势和写审计问题已关闭；陈旧验收记录和测试断言本轮已更新 | 明确的 accept/rework 结论 | closed |

## 结论

- 逐图拆解层已记录区域、文本、组件、图标、token、交互、实现映射和证据路径。
- r189 的构建、部署、Windows Chrome 生产路由截图、业务 diff、ECharts 联动、API 空数据仿真和服务端审计写入均已通过。
- 当前线上外传查询没有非空记录；这不是伪造实时数据，页面以“仿真数据（API 空）”明确标识，且真实字段直通由契约测试覆盖。
- 主线程已接受本页，`verification.json` 记录最终验收结论。
