# audit-log-operation-context.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：审计日志。
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/audit-log-operation-context.png`
- 源图尺寸：1920 x 1080
...
| audit-log-operation-context | `510,54,360,24` | subtitle | 是 |
| /audit-log | `940,54,360,24` | section | 是 |
| 追踪所有关键操作，满足安全运营、内部追责和验收取证要求。 | `1370,54,360,24` | label | 是 |
...
| 左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“审计日志”。 | `940,138,360,24` | title | 是 |
...
| 页面名称：审计日志。 | `1370,180,360,24` | button | 是 |
| 路由：/audit-log。 | `80,222,360,24` | status | 是 |
| 业务模块：审计配置 / 审计日志。 | `510,222,360,24` | legend | 是 |
| 页面重点：追踪所有关键操作，满足安全运营、内部追责和验收取证要求。 | `940,222,360,24` | hint | 是 |
| 必须包含的业务模块： | `1370,222,360,24` | acceptance | 是 |
| - 日志检索：用户、租户、时间、对象类型、动作类型、结果、请求 ID。 | `80,264,360,24` | title | 是 |
| - 操作详情：操作前后值、对象 ID、IP、User-Agent、trace_id、失败原因。 | `510,264,360,24` | subtitle | 是 |
...
- 文本 02：`audit-log-operation-context`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/audit-log`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`追踪所有关键操作，满足安全运营、内部追责和验收取证要求。`，类型 label，来源 prompt/layer plus target visual ledger。
...
- 文本 11：`左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“审计日志”。`，类型 title，来源 prompt/layer plus target visual ledger。
...
- 文本 16：`页面名称：审计日志。`，类型 button，来源 prompt/layer plus target visual ledger。
- 文本 17：`路由：/audit-log。`，类型 status，来源 prompt/layer plus target visual ledger。
- 文本 18：`业务模块：审计配置 / 审计日志。`，类型 legend，来源 prompt/layer plus target visual ledger。
- 文本 19：`页面重点：追踪所有关键操作，满足安全运营、内部追责和验收取证要求。`，类型 hint，来源 manual visual ledger。
- 文本 20：`必须包含的业务模块：`，类型 acceptance，来源 manual visual ledger。
- 文本 21：`- 日志检索：用户、租户、时间、对象类型、动作类型、结果、请求 ID。`，类型 title，来源 manual visual ledger。
- 文本 22：`- 操作详情：操作前后值、对象 ID、IP、User-Agent、trace_id、失败原因。`，类型 subtitle，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/audit-log-operation-context/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/audit-log-operation-context/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/audit-log-operation-context/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/audit-log-operation-context/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/audit-log-operation-context/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/audit-log-operation-context/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/audit-log-operation-context/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/audit-log-operation-context/verification.json`
