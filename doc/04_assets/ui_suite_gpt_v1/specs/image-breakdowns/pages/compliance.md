# compliance.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：合规审计
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/compliance.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/compliance.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/compliance.json`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 合规审计 | `80,54,360,24` | title | 是 |
| compliance | `510,54,360,24` | subtitle | 是 |
| /compliance | `940,54,360,24` | section | 是 |
| 合规审计。 | `1370,54,360,24` | label | 是 |
| 面向任务书、第三方评测和工程验收，证明系统能力、证据和运行状态可交付。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“合规审计”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：合规审计。 | `80,306,360,24` | metric | 是 |
| 路由：/compliance。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 合规审计。 | `940,306,360,24` | status | 是 |
| 页面重点：面向任务书、第三方评测和工程验收，证明系统能力、证据和运行状态可交付。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 验收门禁：采集覆盖、数据质量、告警链路、PCAP 证据、MLOps、审计留痕、部署基线。 | `510,348,360,24` | acceptance | 是 |
| - 指标映射：任务书指标、测试项、对应页面、对应数据源、责任人。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`合规审计`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`compliance`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/compliance`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`合规审计。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`面向任务书、第三方评测和工程验收，证明系统能力、证据和运行状态可交付。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“合规审计”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：合规审计。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/compliance。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：审计配置 / 合规审计。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：面向任务书、第三方评测和工程验收，证明系统能力、证据和运行状态可交付。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 验收门禁：采集覆盖、数据质量、告警链路、PCAP 证据、MLOps、审计留痕、部署基线。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 指标映射：任务书指标、测试项、对应页面、对应数据源、责任人。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/compliance/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/compliance/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/compliance/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/compliance/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/compliance/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/compliance/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/compliance/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/compliance/verification.json`
