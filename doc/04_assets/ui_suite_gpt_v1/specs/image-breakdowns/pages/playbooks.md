# playbooks.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：SOAR 剧本
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/playbooks.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/playbooks.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/playbooks.json`
- 对应路由/宿主路由：`/playbooks`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| SOAR 剧本 | `80,54,360,24` | title | 是 |
| playbooks | `510,54,360,24` | subtitle | 是 |
| /playbooks | `940,54,360,24` | section | 是 |
| SOAR 剧本。 | `1370,54,360,24` | label | 是 |
| 把人工研判后的响应动作产品化，形成可授权、可回滚、可审计的自动化处置。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“SOAR 剧本”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：SOAR 剧本。 | `80,306,360,24` | metric | 是 |
| 路由：/playbooks。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / SOAR 剧本。 | `940,306,360,24` | status | 是 |
| 页面重点：把人工研判后的响应动作产品化，形成可授权、可回滚、可审计的自动化处置。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 剧本列表：剧本名称、适用告警、动作类型、风险级别、启用状态、最近执行。 | `510,348,360,24` | acceptance | 是 |
| - 剧本编排：条件节点、人工确认节点、隔离/阻断/封禁/脚本节点、回滚节点。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`SOAR 剧本`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`playbooks`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/playbooks`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`SOAR 剧本。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`把人工研判后的响应动作产品化，形成可授权、可回滚、可审计的自动化处置。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“SOAR 剧本”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：SOAR 剧本。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/playbooks。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：检测运营 / SOAR 剧本。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：把人工研判后的响应动作产品化，形成可授权、可回滚、可审计的自动化处置。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 剧本列表：剧本名称、适用告警、动作类型、风险级别、启用状态、最近执行。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 剧本编排：条件节点、人工确认节点、隔离/阻断/封禁/脚本节点、回滚节点。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/playbooks/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/playbooks/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/playbooks/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/playbooks/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/playbooks/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/playbooks/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/playbooks/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/playbooks/verification.json`
