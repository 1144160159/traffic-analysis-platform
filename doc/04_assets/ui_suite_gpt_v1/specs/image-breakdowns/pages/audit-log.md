# audit-log.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：审计日志
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/audit-log.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/audit-log.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/audit-log.json`
- 对应路由/宿主路由：`/audit-log`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 审计日志 | `80,54,360,24` | title | 是 |
| audit-log | `510,54,360,24` | subtitle | 是 |
| /audit-log | `940,54,360,24` | section | 是 |
| 审计日志。 | `1370,54,360,24` | label | 是 |
| 追踪所有关键操作，满足安全运营、内部追责和验收取证要求。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“审计日志”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：审计日志。 | `80,306,360,24` | metric | 是 |
| 路由：/audit-log。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 审计日志。 | `940,306,360,24` | status | 是 |
| 页面重点：追踪所有关键操作，满足安全运营、内部追责和验收取证要求。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 日志检索：用户、租户、时间、对象类型、动作类型、结果、请求 ID。 | `510,348,360,24` | acceptance | 是 |
| - 操作详情：操作前后值、对象 ID、IP、User-Agent、trace_id、失败原因。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`审计日志`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`audit-log`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/audit-log`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`审计日志。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`追踪所有关键操作，满足安全运营、内部追责和验收取证要求。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“审计日志”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：审计日志。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/audit-log。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：审计配置 / 审计日志。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：追踪所有关键操作，满足安全运营、内部追责和验收取证要求。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 日志检索：用户、租户、时间、对象类型、动作类型、结果、请求 ID。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 操作详情：操作前后值、对象 ID、IP、User-Agent、trace_id、失败原因。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/audit-log/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/audit-log/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/audit-log/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/audit-log/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/audit-log/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/audit-log/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/audit-log/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/audit-log/verification.json`
...
- 生产镜像：`traffic/web-ui:ui-auditlog-interactions-20260711-r251`；路由：`/audit-log`；浏览器：Windows Chrome `150.0.7871.49`，CDP `http://127.0.0.1:9224`。
- 业务图表：日志留存、归档、完整性和脱敏状态均为 `DataQualityKpiSparklineChart` ECharts canvas，使用审计页面 API 的同构 typed fallback 数据。
- 业务交互：`interaction-r251.json` 已验证第 2 页、列表滚动、导出确认、操作上下文 Tab 和 CSV 格式切换；无坏响应、request failure、console error 或 page error。
- 业务区域评分：`content-root` bbox `198,80,1722,917` 的 mismatch ratio 为 `0.07454938780576464 <= 0.125`；公共 topbar/sidebar/bottombar 仅作为整图诊断，不作为业务区回修范围。
