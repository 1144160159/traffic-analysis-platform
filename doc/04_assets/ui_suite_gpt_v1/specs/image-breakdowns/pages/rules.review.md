...
- Scope: canonical target PNG plus r250 production-route business verification
- Evidence target: `evidence/ui-image-breakdowns/pages/rules/target.png`
...
- The target belongs to category `pages` and is handled as `规则管理`.
...
## r250 Production Review

| Check | Result | Evidence |
|---|---|---|
| Production image | pass | `traffic/web-ui:ui-rules-interactions-20260711-r250` rolled out through `deployments/kubernetes/applications/web-ui.yaml`. |
| Dynamic business charts | pass | `interaction-r250.json` reports `performance_canvas_count=4`; each performance card uses an ECharts sparkline. |
| Pagination and overflow | pass | The same interaction record reports page `2` and computed table vertical overflow `auto`. |
| Business actions | pass | New-rule confirmation, editor/sample Tab state and full-publish Drawer all respond to user input. |
| Windows Chrome runtime | pass | No bad response, console error, page error or request failure. |
| Business ROI | pass | `metrics-business-r250.json`: `0.07719207586218252 <= 0.125`; bbox `198,80,1722,917`. |

The visible Chrome rerun gate is closed by r250. Review scope is the business content area only: public topbar, sidebar and bottombar remained fixed and were not modified to influence the score.
