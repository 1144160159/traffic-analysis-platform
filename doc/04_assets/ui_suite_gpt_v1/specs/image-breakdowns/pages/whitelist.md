# whitelist.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：白名单
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/whitelist.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/whitelist.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/whitelist.json`
- 对应路由/宿主路由：`/whitelist`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 白名单 | `80,54,360,24` | title | 是 |
| whitelist | `510,54,360,24` | subtitle | 是 |
| /whitelist | `940,54,360,24` | section | 是 |
| 白名单。 | `1370,54,360,24` | label | 是 |
| 管理业务例外和误报沉淀，避免白名单变成不可控的检测盲区。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“白名单”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：白名单。 | `80,306,360,24` | metric | 是 |
| 路由：/whitelist。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / 白名单。 | `940,306,360,24` | status | 是 |
| 页面重点：管理业务例外和误报沉淀，避免白名单变成不可控的检测盲区。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 白名单列表：对象类型、匹配条件、生效范围、有效期、责任人、来源告警。 | `510,348,360,24` | acceptance | 是 |
| - 新增白名单：IP、资产、账号、域名、规则、模型、时间窗、例外原因。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`白名单`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`whitelist`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/whitelist`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`白名单。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`管理业务例外和误报沉淀，避免白名单变成不可控的检测盲区。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“白名单”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：白名单。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/whitelist。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：检测运营 / 白名单。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：管理业务例外和误报沉淀，避免白名单变成不可控的检测盲区。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 白名单列表：对象类型、匹配条件、生效范围、有效期、责任人、来源告警。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 新增白名单：IP、资产、账号、域名、规则、模型、时间窗、例外原因。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/whitelist/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/whitelist/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/whitelist/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/whitelist/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/whitelist/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/whitelist/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/whitelist/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/whitelist/verification.json`
...
- 生产镜像：`traffic/web-ui:ui-whitelist-interactions-20260711-r249`；路由：`/whitelist`；浏览器：Windows Chrome `150.0.7871.49`，CDP `http://127.0.0.1:9224`。
- 业务图表：`白名单命中趋势` 与 `白名单覆盖维度` 均为 ECharts canvas，数据由 `fetchPageSnapshot` 返回的页面快照或 typed fallback 驱动。
- 业务交互：`interaction-r249.json` 已验证第 2 页、列表横向滚动、创建和行级操作 Drawer、过期未处理 Tab 与审批展开；无坏响应、request failure、console error 或 page error。
- 业务区域评分：`content-root` bbox `198,80,1722,917` 的 mismatch ratio 为 `0.08112792687359807 <= 0.125`；公共 topbar/sidebar/bottombar 仅作为整图诊断，不作为业务区回修范围。
