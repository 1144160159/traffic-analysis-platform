...
- 分类：pages
- 标题：实体图谱
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/graph.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/graph.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/graph.json`
- 对应路由/宿主路由：`/graph`
...
| graph | `510,54,360,24` | subtitle | 是 |
| /graph | `940,54,360,24` | section | 是 |
| 实体图谱。 | `1370,54,360,24` | label | 是 |
| 展示 IP、主机、账号、服务、域名、告警、证据之间的实体关系。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：资产台账、实体图谱、数据融合、行为基准，当前高亮“实体图谱”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：实体图谱。 | `80,306,360,24` | metric | 是 |
| 路由：/graph。 | `510,306,360,24` | button | 是 |
| 业务模块：资产图谱 / 实体图谱。 | `940,306,360,24` | status | 是 |
| 页面重点：展示 IP、主机、账号、服务、域名、告警、证据之间的实体关系。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 实体搜索：IP、账号、主机、域名、服务、告警 ID、资产 ID。 | `510,348,360,24` | acceptance | 是 |
| - 邻居图谱：一跳/二跳邻居、关系类型、边权重、最近活跃时间。 | `940,348,360,24` | title | 是 |
...
- 文本 02：`graph`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/graph`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`实体图谱。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`展示 IP、主机、账号、服务、域名、告警、证据之间的实体关系。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：资产台账、实体图谱、数据融合、行为基准，当前高亮“实体图谱”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：实体图谱。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/graph。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：资产图谱 / 实体图谱。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：展示 IP、主机、账号、服务、域名、告警、证据之间的实体关系。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 实体搜索：IP、账号、主机、域名、服务、告警 ID、资产 ID。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 邻居图谱：一跳/二跳邻居、关系类型、边权重、最近活跃时间。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/graph/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/graph/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/graph/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/graph/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/graph/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/graph/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/graph/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/graph/verification.json`
