...
- 分类：pages
- 标题：MLOps 编排
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/mlops.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/mlops.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/mlops.json`
- 对应路由/宿主路由：`/mlops`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| MLOps 编排 | `80,54,360,24` | title | 是 |
| mlops | `510,54,360,24` | subtitle | 是 |
| /mlops | `940,54,360,24` | section | 是 |
| MLOps 编排。 | `1370,54,360,24` | label | 是 |
| 形成反馈样本 -> 标注 -> 训练 -> 评估 -> 注册 -> 发布 -> 效果回流的模型闭环。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“MLOps 编排”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：MLOps 编排。 | `80,306,360,24` | metric | 是 |
| 路由：/mlops。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / MLOps 编排。 | `940,306,360,24` | status | 是 |
| 页面重点：形成反馈样本 -> 标注 -> 训练 -> 评估 -> 注册 -> 发布 -> 效果回流的模型闭环。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 编排看板：训练任务、评估任务、注册任务、发布任务、失败任务。 | `510,348,360,24` | acceptance | 是 |
| - 反馈样本池：TP/FP、误报原因、标注人、来源告警、白名单建议。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`MLOps 编排`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`mlops`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/mlops`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`MLOps 编排。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`形成反馈样本 -> 标注 -> 训练 -> 评估 -> 注册 -> 发布 -> 效果回流的模型闭环。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“MLOps 编排”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：MLOps 编排。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/mlops。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：检测运营 / MLOps 编排。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：形成反馈样本 -> 标注 -> 训练 -> 评估 -> 注册 -> 发布 -> 效果回流的模型闭环。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 编排看板：训练任务、评估任务、注册任务、发布任务、失败任务。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 反馈样本池：TP/FP、误报原因、标注人、来源告警、白名单建议。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/mlops/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/mlops/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/mlops/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/mlops/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/mlops/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/mlops/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/mlops/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/mlops/verification.json`
...
- 生产镜像：`traffic/web-ui:ui-mlops-task-detail-20260712-r289`；真实路由：`http://10.0.5.8:30180/mlops`，Windows Chrome CDP。
- 6 条门禁 sparkline 和 1 张误报率/漂移趋势均为 typed simulation ECharts；Windows Chrome 逐图验证尺寸与 `content-root` 内可见性。
- 流水线、训练、重试、停止、标注、注册、任务详情、反馈详情、模型版本详情和三种行级操作均有前端 endpoint 契约、RBAC 声明、请求体、审计事件、异步回执和会话审计；这些记录不替代服务端 RBAC、任务持久化和数据库审计证明。
- 32 条任务均明确标记数据模式：前 6 条为 status API 派生仿真、触发条件行为 API 条件派生，其余为 `SIM-MLOPS-*` 仿真；6 条/页受控分页，第 3 至 6 页可见仿真标识。
- 状态筛选、刷新和部署管理导航均通过真实点击验证。静态 inventory：被动控件 `0`、SVG 图表候选 `0`、分页标记 `2`。
- 任务详情使用独立 `GET /v1/mlops/tasks/{id}`，Drawer 展示当前任务阶段、数据集、算法、特征、资源、状态和数据模式；交互截图在 `/mlops` 根节点可见时生成并通过文件有效性校验。
- 业务 ROI：`0.0748254989949806 < 0.125`；证据：`implementation-r289.png`、`interaction-r289.json`、`metrics-business-r289.json`、`diff-business-r289.png`。
- 全栈状态：待补真实后端动作、seeded 分页数据、持久化任务/审计和正常生产路由复验，当前不得标记 `full-stack-accepted`。
