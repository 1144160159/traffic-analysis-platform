# baselines-time-window.png review
...
- Evidence target: `evidence/ui-image-breakdowns/pages/baselines-time-window/target.png`
...
| Visual diff | pass | Canonical `content-root` ROI is `0.11912804593071635`, within `0.125` at channel tolerance `90` |
| Auxiliary review | requested | Independent subagent review has not been performed for this evidence refresh |
| Main-thread judgment | requested | Final acceptance remains pending after the evidence-stage visual diff pass |
...
|---|---|---|---|---|
| visual-diff | content-root | canonical production screenshot ROI is `0.11912804593071635` at tolerance `90` | mismatch ratio <= `0.125` | closed |
...
- Canonical result: `content-root` ROI `0.11912804593071635` passes threshold `0.125` at tolerance `90`.
- Verdict: `changes-required`; the screenshot remains on the asset baseline tab and does not prove the time-window baseline target state.
