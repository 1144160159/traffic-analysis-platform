# campaign-detail.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：战役详情
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/campaign-detail.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/campaign-detail.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/campaign-detail.json`
- 对应路由/宿主路由：`/campaigns/:campaignId`
- 当前状态：`business-pixel-accepted`
- 复刻等级：逐图目标 PNG 已锁定；生产 React 路由已通过 Windows Chrome CDP 截图、diff/metrics 与主线程判断。
- 验收边界：目标 PNG 作为拆解与视觉 diff 输入；最终验收以 `http://10.0.5.8:30180/campaigns/campaign-exfil-default-1782729598739-e1d2dc37` 生产路由、动态 API/typed fallback 数据、runtime 元数据和人工复核为准。
...
| campaign-detail | `510,54,360,24` | subtitle | 是 |
| /campaigns/:campaignId | `940,54,360,24` | section | 是 |
| 战役详情。 | `1370,54,360,24` | label | 是 |
| 战役画像、攻击时间轴、关联告警、影响范围、证据包、复盘结论。 | `80,96,360,24` | metric | 是 |
...
| 页面名称：战役详情。 | `80,306,360,24` | metric | 是 |
| 路由：/campaigns/:campaignId。 | `510,306,360,24` | button | 是 |
| 业务模块：威胁分析 / 战役列表。 | `940,306,360,24` | status | 是 |
| 页面重点：战役画像、攻击时间轴、关联告警、影响范围、证据包、复盘结论。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 战役画像：名称、风险、阶段、持续时间、负责人、当前状态。 | `510,348,360,24` | acceptance | 是 |
| - 攻击时间轴：关键阶段、关联告警、证据节点、处置节点。 | `940,348,360,24` | title | 是 |
...
- 文本 02：`campaign-detail`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/campaigns/:campaignId`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`战役详情。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`战役画像、攻击时间轴、关联告警、影响范围、证据包、复盘结论。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 25：`页面名称：战役详情。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/campaigns/:campaignId。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：威胁分析 / 战役列表。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：战役画像、攻击时间轴、关联告警、影响范围、证据包、复盘结论。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 战役画像：名称、风险、阶段、持续时间、负责人、当前状态。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 攻击时间轴：关键阶段、关联告警、证据节点、处置节点。`，类型 title，来源 manual visual ledger。
...
- 页面映射：`route:/campaigns/:campaignId`
...
- 目标图：`evidence/ui-image-breakdowns/pages/campaign-detail/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/campaign-detail/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/campaign-detail/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/campaign-detail/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/campaign-detail/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/campaign-detail/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/campaign-detail/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/campaign-detail/verification.json`
