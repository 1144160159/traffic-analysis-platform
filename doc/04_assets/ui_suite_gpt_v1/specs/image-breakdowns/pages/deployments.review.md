...
- Evidence target: `evidence/ui-image-breakdowns/pages/deployments/target.png`
...
- The target belongs to category `pages` and is handled as `部署管理`.
...
## r259 Production Regression Review

| Check | Result | Evidence |
|---|---|---|
| Dynamic health charts | pass | `interaction-r259.json` reports six ECharts canvases; screenshot review confirms all six cards are visible and no chart is cropped. |
| Buttons have observable outcomes | pass | New deployment, row action and evidence action each open a Drawer. |
| Release list usability | pass | Controlled page 2 is available and the list container reports `overflow-y: scroll`. |
