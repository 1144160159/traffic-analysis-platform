# campaign-detail-impact-account.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：战役详情。
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/campaign-detail-impact-account.png`
- 源图尺寸：1920 x 1080
...
- 对应路由/宿主路由：`/campaigns/:campaignId?impact=account` / `/campaigns/:campaignId`
- 当前状态：`business-pixel-accepted`
- 生产镜像：`traffic/web-ui:ui-campaign-impact-account-visual-20260710-r167`
- 复刻等级：真实 React/CSS 数据驱动实现，已完成 Windows Chrome 生产路由截图、diff、辅助审查、回修和主线程判断。
- 验收边界：目标图是战役详情中的“影响范围 / 账号”focus 状态；生产路由和普通战役详情共用 typed snapshot，不引用目标 PNG。

## 目标图观察

- 整体布局：影响范围独立面板，顶部六个对象 tab，中部账号风险环图与风险分布，下部 Top 5 账号表。
- 业务重点：账号风险数量、占比、账号类型、权限风险和登录链路均由 typed campaign snapshot 驱动。
- 当前页面/浮层状态：`账号` tab 激活；受影响账号 31，高/中/低风险分别为 8/14/9。
- 视觉基调：深海军蓝 SOC 指挥台，青蓝描边，低饱和面板，高密度文字与表格，状态色严格区分。
- 证据边界：`implementation.png` 来自 r167 真实 APISIX 路由和 Windows Chrome CDP；`target.png` 只参与拆解与 diff。
- 视觉读取方式：直接锁定目标 PNG，结合 prompt、layer JSON、manifest 与 Windows Chrome 截图证据校验。
- 坐标口径：所有 bbox 均以目标 PNG 左上角为原点，单位 px。

## 本轮生产实现校正

- `CampaignDetailPage.tsx` 使用 query `impact=account` 表达可访问、可复现的账号状态；视觉验收模式只隐藏 AppShell，不改变业务组件和数据结构。
- `campaignDetailApi.ts` 新增 `CampaignDetailImpactAccount`、`CampaignDetailImpactRiskRow`、`CampaignDetailAccountRow`，真实接口入口保持 `/v1/campaigns/{campaignId}`。
- API 未返回账号影响字段时使用 typed fallback：总数 31，风险分布 8/14/9，Top 5 为 `svc_backup`、`temp_admin`、`li.ming`、`ops_reader`、`svc_deploy`。
- 风险环使用 React/CSS `conic-gradient` 根据 typed count 计算角度；表格直接消费 snapshot rows，不是图片或目标图回放。
- r166 首拍 metrics 为 `0.10694733796296296`；辅助审查发现环图应为横向椭圆，回修 r167 后为 `0.09737268518518519`。
- r167 runtime：无 console/page error、requestfailed、HTTP 4xx/5xx、禁止资源请求或横向溢出；5 行账号和 3 行风险分布均存在。
...
| campaign-detail-impact-account | `510,54,360,24` | subtitle | 是 |
...
- 文本 02：`campaign-detail-impact-account`，类型 subtitle，来源 prompt/layer plus target visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/campaign-detail-impact-account/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/campaign-detail-impact-account/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/campaign-detail-impact-account/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/campaign-detail-impact-account/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/campaign-detail-impact-account/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/campaign-detail-impact-account/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/campaign-detail-impact-account/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/campaign-detail-impact-account/verification.json`
