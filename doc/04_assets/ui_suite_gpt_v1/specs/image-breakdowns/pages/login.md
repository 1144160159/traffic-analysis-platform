...
- 分类：pages
- 标题：登录
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/login.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/login.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/login.json`
- 对应路由/宿主路由：`/login`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 登录 | `80,54,360,24` | title | 是 |
| login | `510,54,360,24` | subtitle | 是 |
| /login | `940,54,360,24` | section | 是 |
| 登录。 | `1370,54,360,24` | label | 是 |
| 统一身份认证、验证码、租户/站点选择、账号安全提示、OIDC/SSO 入口、认证失败提示。 | `80,96,360,24` | metric | 是 |
...
| 登录页不展示常规 AppShell | `510,138,360,24` | acceptance | 是 |
...
| 登录/认证类页面不显示左侧业务导航和二级菜单，但视觉风格必须与主系统一致。 | `1370,222,360,24` | acceptance | 是 |
| 登录/认证类页面不显示业务顶部 KPI 条，可展示系统名称、租户/站点、安全提示和能力摘要。 | `80,264,360,24` | title | 是 |
...
| 页面名称：登录。 | `80,306,360,24` | metric | 是 |
| 路由：/login。 | `510,306,360,24` | button | 是 |
| 业务模块：登录入口 / 身份认证。 | `940,306,360,24` | status | 是 |
| 页面重点：统一身份认证、验证码、租户/站点选择、账号安全提示、OIDC/SSO 入口、认证失败提示。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 登录表单：账号、密码、验证码、租户/站点选择、记住登录、提交按钮。 | `510,348,360,24` | acceptance | 是 |
| - 身份入口：账号密码登录、OIDC/SSO 登录、只读演示入口。 | `940,348,360,24` | title | 是 |
...
下列条目来自目标图逐字复核，用于同步结构化 JSON 的字符级账本；它们不是新增界面文案。

| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 中 | `452,320,70,82` | brand-glyph | 是 |
| 7 | `1518,606,25,42` | captcha-character | 是 |
| K | `1547,606,28,42` | captcha-character | 是 |
| 3 | `1580,606,25,42` | captcha-character | 是 |
| 8 | `1610,606,25,42` | captcha-character | 是 |
| 分隔符一 | `1414,692,8,25` | link-separator | 是 |
| 分隔符二 | `1534,692,8,25` | link-separator | 是 |

### 文本人工校正说明

- 文本 01：`登录`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`login`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/login`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`登录。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`统一身份认证、验证码、租户/站点选择、账号安全提示、OIDC/SSO 入口、认证失败提示。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 10：`登录页不展示常规 AppShell`，类型 acceptance，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`登录/认证类页面不显示左侧业务导航和二级菜单，但视觉风格必须与主系统一致。`，类型 acceptance，来源 manual visual ledger。
- 文本 21：`登录/认证类页面不显示业务顶部 KPI 条，可展示系统名称、租户/站点、安全提示和能力摘要。`，类型 title，来源 manual visual ledger。
...
- 文本 25：`页面名称：登录。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/login。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：登录入口 / 身份认证。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：统一身份认证、验证码、租户/站点选择、账号安全提示、OIDC/SSO 入口、认证失败提示。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 登录表单：账号、密码、验证码、租户/站点选择、记住登录、提交按钮。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 身份入口：账号密码登录、OIDC/SSO 登录、只读演示入口。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/login/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/login/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/login/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/login/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/login/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/login/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/login/metrics.json`
