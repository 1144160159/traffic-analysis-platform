# forensics.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：取证分析
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/forensics.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/forensics.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/forensics.json`
- 对应路由/宿主路由：`/forensics`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 取证分析 | `80,54,360,24` | title | 是 |
| forensics | `510,54,360,24` | subtitle | 是 |
| /forensics | `940,54,360,24` | section | 是 |
| 取证分析。 | `1370,54,360,24` | label | 是 |
| 围绕告警、资产、时间窗完成 PCAP、Session、日志证据检索、切片、下载和审计。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“取证分析”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：取证分析。 | `80,306,360,24` | metric | 是 |
| 路由：/forensics。 | `510,306,360,24` | button | 是 |
| 业务模块：威胁分析 / 取证分析。 | `940,306,360,24` | status | 是 |
| 页面重点：围绕告警、资产、时间窗完成 PCAP、Session、日志证据检索、切片、下载和审计。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 取证任务：告警 ID、资产、五元组、时间窗、任务状态、责任人。 | `510,348,360,24` | acceptance | 是 |
| - PCAP 索引：pcap_index、对象路径、大小、hash、时间窗、协议。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`取证分析`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`forensics`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/forensics`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`取证分析。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`围绕告警、资产、时间窗完成 PCAP、Session、日志证据检索、切片、下载和审计。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“取证分析”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：取证分析。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/forensics。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：威胁分析 / 取证分析。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：围绕告警、资产、时间窗完成 PCAP、Session、日志证据检索、切片、下载和审计。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 取证任务：告警 ID、资产、五元组、时间窗、任务状态、责任人。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- PCAP 索引：pcap_index、对象路径、大小、hash、时间窗、协议。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/forensics/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/forensics/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/forensics/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/forensics/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/forensics/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/forensics/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/forensics/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/forensics/verification.json`
...
- 生产镜像：`traffic/web-ui:ui-forensics-interactions-20260711-r252`；路由：`/forensics`；浏览器：Windows Chrome `150.0.7871.49`，CDP `http://127.0.0.1:9224`。
- 业务图表：会话数据包趋势为 `DataQualityKpiSparklineChart` ECharts canvas，使用取证任务和统计 API 的同构 typed fallback 数据。
- 业务交互：`interaction-r252.json` 已验证第 2 页、列表滚动、新建任务确认与会话复放 Drawer；无坏响应、request failure、console error 或 page error。
- 业务区域评分：`content-root` bbox `198,80,1722,917` 的 mismatch ratio 为 `0.08618595455311151 <= 0.125`；公共 topbar/sidebar/bottombar 仅作为整图诊断，不作为业务区回修范围。
- 本段取代上文 reference-raster 的历史实施描述，当前 `implementation.png` 为真实 APISIX/Web UI 生产路由截图，不是目标 PNG 回放。
