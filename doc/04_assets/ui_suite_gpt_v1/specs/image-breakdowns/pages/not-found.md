# not-found.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：404 异常页
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/not-found.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/not-found.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/not-found.json`
- 对应路由/宿主路由：`*`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 404 异常页 | `80,54,360,24` | title | 是 |
| not-found | `510,54,360,24` | subtitle | 是 |
| * | `940,54,360,24` | section | 是 |
| 404 异常页。 | `1370,54,360,24` | label | 是 |
| 页面不存在、返回工作流、查看审计日志、联系管理员，不能暴露敏感路径。 | `80,96,360,24` | metric | 是 |
...
| 当前页面不显示常规二级菜单。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：404 异常页。 | `80,306,360,24` | metric | 是 |
| 路由：*。 | `510,306,360,24` | button | 是 |
| 业务模块：异常状态 / 404 异常页。 | `940,306,360,24` | status | 是 |
| 页面重点：页面不存在、返回工作流、查看审计日志、联系管理员，不能暴露敏感路径。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 错误摘要：页面不存在、追踪 ID、时间、当前租户/站点。 | `510,348,360,24` | acceptance | 是 |
| - 返回入口：返回仪表盘、态势大屏、告警中心或上一页。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`404 异常页`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`not-found`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`*`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`404 异常页。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`页面不存在、返回工作流、查看审计日志、联系管理员，不能暴露敏感路径。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`当前页面不显示常规二级菜单。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：404 异常页。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：*。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：异常状态 / 404 异常页。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：页面不存在、返回工作流、查看审计日志、联系管理员，不能暴露敏感路径。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 错误摘要：页面不存在、追踪 ID、时间、当前租户/站点。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 返回入口：返回仪表盘、态势大屏、告警中心或上一页。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/not-found/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/not-found/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/not-found/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/not-found/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/not-found/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/not-found/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/not-found/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/not-found/verification.json`
