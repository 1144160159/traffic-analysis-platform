# assets-detail-network-interface.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：资产台账。
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/assets-detail-network-interface.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/assets.prompt.txt`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 资产台账。 | `80,54,360,24` | title | 是 |
| assets-detail-network-interface | `510,54,360,24` | subtitle | 是 |
| /assets | `940,54,360,24` | section | 是 |
| 管理和查看园区终端、服务器、网络设备、业务系统和风险画像。 | `1370,54,360,24` | label | 是 |
...
| 左侧二级菜单显示：资产台账、实体图谱、数据融合、行为基准，当前高亮“资产台账”。 | `940,138,360,24` | title | 是 |
...
| 页面名称：资产台账。 | `1370,180,360,24` | button | 是 |
| 路由：/assets。 | `80,222,360,24` | status | 是 |
| 业务模块：资产图谱 / 资产台账。 | `510,222,360,24` | legend | 是 |
| 页面重点：管理和查看园区终端、服务器、网络设备、业务系统和风险画像。 | `940,222,360,24` | hint | 是 |
| 必须包含的业务模块： | `1370,222,360,24` | acceptance | 是 |
| - 资产列表：IP、MAC、主机名、类型、部门、园区、操作系统、重要性。 | `80,264,360,24` | title | 是 |
| - 风险画像：最近告警、暴露端口、异常行为、漏洞、弱口令、外联。 | `510,264,360,24` | subtitle | 是 |
...
- 文本 01：`资产台账。`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`assets-detail-network-interface`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/assets`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`管理和查看园区终端、服务器、网络设备、业务系统和风险画像。`，类型 label，来源 prompt/layer plus target visual ledger。
...
- 文本 11：`左侧二级菜单显示：资产台账、实体图谱、数据融合、行为基准，当前高亮“资产台账”。`，类型 title，来源 prompt/layer plus target visual ledger。
...
- 文本 16：`页面名称：资产台账。`，类型 button，来源 prompt/layer plus target visual ledger。
- 文本 17：`路由：/assets。`，类型 status，来源 prompt/layer plus target visual ledger。
- 文本 18：`业务模块：资产图谱 / 资产台账。`，类型 legend，来源 prompt/layer plus target visual ledger。
- 文本 19：`页面重点：管理和查看园区终端、服务器、网络设备、业务系统和风险画像。`，类型 hint，来源 manual visual ledger。
- 文本 20：`必须包含的业务模块：`，类型 acceptance，来源 manual visual ledger。
- 文本 21：`- 资产列表：IP、MAC、主机名、类型、部门、园区、操作系统、重要性。`，类型 title，来源 manual visual ledger。
- 文本 22：`- 风险画像：最近告警、暴露端口、异常行为、漏洞、弱口令、外联。`，类型 subtitle，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/assets-detail-network-interface/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/assets-detail-network-interface/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/assets-detail-network-interface/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/assets-detail-network-interface/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/assets-detail-network-interface/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/assets-detail-network-interface/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/assets-detail-network-interface/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/assets-detail-network-interface/verification.json`
