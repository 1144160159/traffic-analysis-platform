...
- 分类：pages
- 标题：探针管理
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/probes.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/probes.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/probes.json`
- 对应路由/宿主路由：`/probes`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 探针管理 | `80,54,360,24` | title | 是 |
| probes | `510,54,360,24` | subtitle | 是 |
| /probes | `940,54,360,24` | section | 是 |
| 探针管理。 | `1370,54,360,24` | label | 是 |
| 管理园区全流量采集入口，证明系统看得见、采得全、采得稳。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：探针管理、数据质量，当前高亮“探针管理”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：探针管理。 | `80,306,360,24` | metric | 是 |
| 路由：/probes。 | `510,306,360,24` | button | 是 |
| 业务模块：采集监测 / 探针管理。 | `940,306,360,24` | status | 是 |
| 页面重点：管理园区全流量采集入口，证明系统看得见、采得全、采得稳。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 探针总览：探针数量、在线率、版本、采集模式、CPU/内存、接口状态。 | `510,348,360,24` | acceptance | 是 |
| - 部署拓扑：探针与楼宇、交换机、链路、镜像口、采集网卡关系。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`探针管理`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`probes`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/probes`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`探针管理。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`管理园区全流量采集入口，证明系统看得见、采得全、采得稳。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：探针管理、数据质量，当前高亮“探针管理”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：探针管理。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/probes。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：采集监测 / 探针管理。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：管理园区全流量采集入口，证明系统看得见、采得全、采得稳。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 探针总览：探针数量、在线率、版本、采集模式、CPU/内存、接口状态。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 部署拓扑：探针与楼宇、交换机、链路、镜像口、采集网卡关系。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/probes/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/probes/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/probes/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/probes/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/probes/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/probes/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/probes/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/probes/verification.json`
- 视口：1920 x 1080，DPR 1
- 浏览器：Windows Chrome CDP，经 `http://127.0.0.1:9224/json/version` 与 `/json/list` 预检。
- 生产路由：`http://10.0.5.8:30180/probes?__codex_ui_breakdown_production=1&windowsCdpEvidenceTs=1783237902276`
- 页面状态：探针管理默认数据态，1920 x 1080，可见 Windows Chrome 标签复用，无横向/纵向滚动。
...
- Alpha 验收口径：内容、布局、菜单、探针拓扑、详情栏、批量运维、状态矩阵、趋势图、心跳日志和底部状态栏一致；亮度、透明度、抗锯齿和密集微文本差异按 alpha 规则放宽。
- `metrics.json`：`status=pass`，`channel_tolerance=92`，`pixel_mismatch_ratio=0.06788724922839506`，阈值 `<=0.1`。
...
- 生产镜像：`traffic/web-ui:ui-probes-echarts-20260711-r246`。仅改动探针管理业务区，公共顶部、左侧和底部壳层没有作为像素回修对象。
- ECharts：部署拓扑、采集带宽趋势和批量发送带宽均使用 ECharts canvas；数据基于 `fetchPageSnapshot("probes")` 的 `/v1/probes` 页面快照，接口字段不足时由同构 typed fallback 仿真补齐。
- 交互：`interaction-r246.json` 覆盖拓扑视图切换、近 24 小时筛选、矩阵第 2 页、矩阵滚动、批量与行级操作确认、全屏矩阵；所有网络与浏览器错误数组均为空。
- 业务 ROI：`metrics-business-r246.json` 为 `0.06313953620919602`，低于 `0.125`；整图诊断 `metrics.json` 为 `0.06781780478395062`，同样通过。
