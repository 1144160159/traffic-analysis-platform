...
- 分类：pages
- 标题：资产台账
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/assets.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/assets.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/assets.json`
- 对应路由/宿主路由：`/assets`
...
| assets | `510,54,360,24` | subtitle | 是 |
| /assets | `940,54,360,24` | section | 是 |
| 资产台账。 | `1370,54,360,24` | label | 是 |
| 管理和查看园区终端、服务器、网络设备、业务系统和风险画像。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：资产台账、实体图谱、数据融合、行为基准，当前高亮“资产台账”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：资产台账。 | `80,306,360,24` | metric | 是 |
| 路由：/assets。 | `510,306,360,24` | button | 是 |
| 业务模块：资产图谱 / 资产台账。 | `940,306,360,24` | status | 是 |
| 页面重点：管理和查看园区终端、服务器、网络设备、业务系统和风险画像。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 资产列表：IP、MAC、主机名、类型、部门、园区、操作系统、重要性。 | `510,348,360,24` | acceptance | 是 |
| - 风险画像：最近告警、暴露端口、异常行为、漏洞、弱口令、外联。 | `940,348,360,24` | title | 是 |
...
- 文本 02：`assets`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/assets`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`资产台账。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`管理和查看园区终端、服务器、网络设备、业务系统和风险画像。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：资产台账、实体图谱、数据融合、行为基准，当前高亮“资产台账”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：资产台账。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/assets。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：资产图谱 / 资产台账。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：管理和查看园区终端、服务器、网络设备、业务系统和风险画像。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 资产列表：IP、MAC、主机名、类型、部门、园区、操作系统、重要性。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 风险画像：最近告警、暴露端口、异常行为、漏洞、弱口令、外联。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/assets/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/assets/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/assets/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/assets/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/assets/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/assets/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/assets/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/assets/verification.json`
