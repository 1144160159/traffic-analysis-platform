# baselines-time-window.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：行为基准。
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/baselines-time-window.png`
- 源图尺寸：1920 x 1080
...
| baselines-time-window | `510,54,360,24` | subtitle | 是 |
| /baselines | `940,54,360,24` | section | 是 |
| 沉淀资产、账号、端口、协议、时间段的正常行为基线，用于异常检测和解释。 | `1370,54,360,24` | label | 是 |
...
| 左侧二级菜单显示：资产台账、实体图谱、数据融合、行为基准，当前高亮“行为基准”。 | `940,138,360,24` | title | 是 |
...
| 页面名称：行为基准。 | `1370,180,360,24` | button | 是 |
| 路由：/baselines。 | `80,222,360,24` | status | 是 |
| 业务模块：资产图谱 / 行为基准。 | `510,222,360,24` | legend | 是 |
| 页面重点：沉淀资产、账号、端口、协议、时间段的正常行为基线，用于异常检测和解释。 | `940,222,360,24` | hint | 是 |
| 必须包含的业务模块： | `1370,222,360,24` | acceptance | 是 |
| - 基线总览：资产基线、账号基线、端口基线、流量基线、覆盖率。 | `80,264,360,24` | title | 是 |
| - 偏离检测：新目的地、新端口、异常时间、异常流量、异常会话长度。 | `510,264,360,24` | subtitle | 是 |
...
- 文本 02：`baselines-time-window`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/baselines`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`沉淀资产、账号、端口、协议、时间段的正常行为基线，用于异常检测和解释。`，类型 label，来源 prompt/layer plus target visual ledger。
...
- 文本 11：`左侧二级菜单显示：资产台账、实体图谱、数据融合、行为基准，当前高亮“行为基准”。`，类型 title，来源 prompt/layer plus target visual ledger。
...
- 文本 16：`页面名称：行为基准。`，类型 button，来源 prompt/layer plus target visual ledger。
- 文本 17：`路由：/baselines。`，类型 status，来源 prompt/layer plus target visual ledger。
- 文本 18：`业务模块：资产图谱 / 行为基准。`，类型 legend，来源 prompt/layer plus target visual ledger。
- 文本 19：`页面重点：沉淀资产、账号、端口、协议、时间段的正常行为基线，用于异常检测和解释。`，类型 hint，来源 manual visual ledger。
- 文本 20：`必须包含的业务模块：`，类型 acceptance，来源 manual visual ledger。
- 文本 21：`- 基线总览：资产基线、账号基线、端口基线、流量基线、覆盖率。`，类型 title，来源 manual visual ledger。
- 文本 22：`- 偏离检测：新目的地、新端口、异常时间、异常流量、异常会话长度。`，类型 subtitle，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/baselines-time-window/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/baselines-time-window/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/baselines-time-window/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/baselines-time-window/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/baselines-time-window/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/baselines-time-window/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/baselines-time-window/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/baselines-time-window/verification.json`
