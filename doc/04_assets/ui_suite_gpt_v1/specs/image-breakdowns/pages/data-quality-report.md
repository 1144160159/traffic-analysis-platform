# data-quality-report.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：数据质量。
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/data-quality-report.png`
- 源图尺寸：1920 x 1080
...
- 复刻等级：逐图目标 PNG 已锁定；r132 生产路由截图、overlay、diff、runtime 和 verification 已由本轮闭环生成。
...
- 生产 URL：`http://10.0.5.8:30180/data-quality?tab=report&__codex_ui_breakdown_production=1&__capture=r132-final`
- 视口：`1920 x 1080`
- 镜像：`traffic/web-ui:ui-data-quality-report-visual-20260709-r132`
- CDP 预检：`evidence/ui-image-breakdowns/pages/data-quality-report/cdp-version-r132-final-pre-capture.txt`、`evidence/ui-image-breakdowns/pages/data-quality-report/cdp-list-r132-final-pre-capture.txt`
- 最终截图：`evidence/ui-image-breakdowns/pages/data-quality-report/implementation-r132-final.png`，同步别名 `implementation.png`
- runtime：0 console errors、0 page errors、0 requestfailed、0 HTTP 4xx/5xx、0 overflow、0 clipped text without title
- full diff：`0.10627555941358025 <= 0.12`，`channel_tolerance=92`
- business diff：`0.11604332665853533 <= 0.12`，`channel_tolerance=92`
- 主线程判断：报告 KPI、报告预览、章节、异常归因、导出记录、验收审批与右侧闭环栏完整可见；差异集中于字体抗锯齿、公共指标实时值、报告预览 SVG/文字密度和表格微排版。
...
| data-quality-report | `510,54,360,24` | subtitle | 是 |
...
- 文本 02：`data-quality-report`，类型 subtitle，来源 prompt/layer plus target visual ledger。
...
- 映射说明：The evidence screenshot is captured from the production React route through Windows Chrome CDP; the breakdown arrays map the target image to semantic frontend units.
...
- 目标图：`evidence/ui-image-breakdowns/pages/data-quality-report/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/data-quality-report/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/data-quality-report/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/data-quality-report/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/data-quality-report/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/data-quality-report/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/data-quality-report/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/data-quality-report/verification.json`
- 视口：1920 x 1080，DPR 1
- 浏览器：Windows Chrome CDP，经 `http://127.0.0.1:9224/json/version` 与 `/json/list` 预检。
- 生产 URL：`http://10.0.5.8:30180/data-quality?tab=report&__codex_ui_breakdown_production=1`
- 页面状态：`route_state.tab=report`
- 复现步骤：先检查 Windows Chrome CDP `/json/version` 与 `/json/list`，再通过 `connectOverCDP("http://127.0.0.1:9224")` 打开生产路由，1920x1080 截取 implementation.png，生成 diff.png 和 metrics.json，读取 verification.json。

## 生产路由验收记录

- 验收时间：2026-07-05 16:29:29 +08:00
- 真实路由：`http://10.0.5.8:30180/data-quality?tab=report&__codex_ui_breakdown_production=1&windowsCdpEvidenceTs=1783240163165`
- 视口：1920 x 1080，DPR 1
- 浏览器：Windows Chrome `Chrome/149.0.7827.201`，CDP `http://127.0.0.1:9224`
- 截图证据：`evidence/ui-image-breakdowns/pages/data-quality-report/implementation.png`
- diff 证据：`evidence/ui-image-breakdowns/pages/data-quality-report/diff.png`
- metrics：`pixel_mismatch_ratio=0.11669367283950617`，`channel_tolerance=92`，`max_pixel_ratio=0.12`
- runtime：pass；无 console/pageerror/requestfailed/bad responses/forbidden target resource requests。
- 主线程判断：`pixel-accepted`。内容与布局在 alpha 口径下通过；残余差异集中在报表预览微细节、亮度和透明度，不影响当前验收门。

## 差异清单

| 类型 | 位置 | 当前 | 期望 | 状态 |
|---|---|---|---|---|
| visual-diff | full image | Windows Chrome 生产路由截图已与 locked target PNG 对比。 | pixel mismatch ratio <= 0.12 under alpha gate | accepted |
| semantic-scope | production React implementation | React/CSS/SVG 代码确定性实现 report tab；未把目标整图作为页面资源。 | semantic React components follow this record | accepted |

## 结论

- 逐图拆解层已记录区域、文本、组件、图标、token、交互、实现映射和证据路径。
- Windows Chrome 生产路由截图和视觉 diff 已由自动闭环脚本写入。
- 通过口径为：target.png、implementation.png、regions-overlay.png、diff.png、metrics.json、verification.json 齐备，且 metrics 状态为 pass。
- 主线程判定覆盖当前 `data-quality-report` 目标图的 alpha 像素复刻与生产路由可见状态，不扩大到未处理页面。

## r271 最新生产验收

- Windows Chrome canvas `5`，业务动作 endpoint/audit Drawer 通过，运行时错误为 `0`。
- 逐路由与真实点击字段质量的 Tab 几何差均为 `0`；业务 ROI：`0.10563152835142621 < 0.125`；证据：`metrics-business-r271.json`、`diff-business-r271.png`、`../data-quality/interaction-r271-report.png`。
