...
- 分类：pages
- 标题：模型管理
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/models.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/models.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/models.json`
- 对应路由/宿主路由：`/models`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 模型管理 | `80,54,360,24` | title | 是 |
| models | `510,54,360,24` | subtitle | 是 |
| /models | `940,54,360,24` | section | 是 |
| 模型管理。 | `1370,54,360,24` | label | 是 |
| 管理 AI/行为检测模型的版本、指标、数据集、解释和在线状态。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“模型管理”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：模型管理。 | `80,306,360,24` | metric | 是 |
| 路由：/models。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / 模型管理。 | `940,306,360,24` | status | 是 |
| 页面重点：管理 AI/行为检测模型的版本、指标、数据集、解释和在线状态。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 模型列表：模型名、类型、版本、状态、线上版本、训练时间、负责人。 | `510,348,360,24` | acceptance | 是 |
| - 模型指标：准确率、召回率、F1、AUC、误报率、漂移、置信区间。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`模型管理`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`models`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/models`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`模型管理。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`管理 AI/行为检测模型的版本、指标、数据集、解释和在线状态。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“模型管理”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：模型管理。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/models。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：检测运营 / 模型管理。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：管理 AI/行为检测模型的版本、指标、数据集、解释和在线状态。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 模型列表：模型名、类型、版本、状态、线上版本、训练时间、负责人。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 模型指标：准确率、召回率、F1、AUC、误报率、漂移、置信区间。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/models/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/models/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/models/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/models/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/models/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/models/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/models/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/models/verification.json`
...
- 生产镜像：`traffic/web-ui:ui-models-page-contract-20260711-r283`；真实路由：`http://10.0.5.8:30180/models`，Windows Chrome CDP。
- 动态图表：7 张模型指标趋势、1 张特征贡献、1 张样本分布，共 9 张 ECharts。真实行读取 API hidden metrics；补足目标容量的行使用明确的 `SIM-MODEL-*` typed simulation catalog，图表由选中模型 ID/指标驱动，切换后 canvas 发生变化。
- 交互：模型筛选、28 条受控分页、解释 Tab、导入/反馈/重训/评估/激活/停用/回滚/MLOps 上下文动作均有可观察结果。确认动作由异步仿真 service 返回任务 ID、请求体、API 契约和会话审计事件；这些仅证明前端交互，不证明服务端动作、RBAC 或数据库审计完成。
- 数据链路：真实 API 返回行保留 `model_id -> __model_id` lineage；仿真补充行明确标记 `__data_mode=simulated`，不伪装为后端 ID。Windows Chrome 捕获 `/api/v1/models?page=1&limit=8&offset=0` 和 `page=2&limit=8&offset=8`。
- 滚动：模型列表 `overflowY=auto`，`scrollHeight=429 > clientHeight=349`；第二页通过真实点击验证。
- 当前页组装规则由纯函数测试证明：API 页记录保持在前，仿真 catalog 只补剩余槽位；同一模型 ID 仅 hidden metrics 更新时，7 点趋势数组也会变化。
- 业务 ROI：`0.07570132875343398 < 0.125`；证据：`implementation-r283.png`、`interaction-r283.json`、`metrics-business-r283.json`、`diff-business-r283.png`。
- 全栈状态：待补后端动作、seeded 第二页数据、持久化审计和正常生产路由复验，当前不得标记 `full-stack-accepted`。
