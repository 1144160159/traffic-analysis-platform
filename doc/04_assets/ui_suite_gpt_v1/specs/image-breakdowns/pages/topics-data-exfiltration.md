# topics-data-exfiltration.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：Topics Data Exfiltration
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/topics-data-exfiltration.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：无直接 prompt
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json`
- 对应路由/宿主路由：`/topics`
- 当前状态：`pixel-accepted`
- 复刻等级：逐图目标 PNG 已锁定；alpha 视觉门禁已通过，内容与布局一致，亮度/透明度/抗锯齿和图表渲染差异按当前 alpha 规则放宽。
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| Topics Data Exfiltration | `80,54,360,24` | title | 是 |
| topics-data-exfiltration | `510,54,360,24` | subtitle | 是 |
...
| Topics Data Exfiltration 视觉校正项 34 | `510,390,360,24` | label | 是 |
| Topics Data Exfiltration 视觉校正项 35 | `940,390,360,24` | metric | 否 |
| Topics Data Exfiltration 视觉校正项 36 | `1370,390,360,24` | button | 否 |
| Topics Data Exfiltration 视觉校正项 37 | `80,432,360,24` | status | 否 |
| Topics Data Exfiltration 视觉校正项 38 | `510,432,360,24` | legend | 否 |
| Topics Data Exfiltration 视觉校正项 39 | `940,432,360,24` | hint | 否 |
| Topics Data Exfiltration 视觉校正项 40 | `1370,432,360,24` | acceptance | 否 |
| Topics Data Exfiltration 视觉校正项 41 | `80,474,360,24` | title | 否 |
| Topics Data Exfiltration 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`Topics Data Exfiltration`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`topics-data-exfiltration`，类型 subtitle，来源 prompt/layer plus target visual ledger。
...
- 文本 34：`Topics Data Exfiltration 视觉校正项 34`，类型 label，来源 manual visual ledger。
- 文本 35：`Topics Data Exfiltration 视觉校正项 35`，类型 metric，来源 manual visual ledger。
- 文本 36：`Topics Data Exfiltration 视觉校正项 36`，类型 button，来源 manual visual ledger。
...
- 映射说明：The evidence screenshot is captured from the real `/topics?topic=exfil&tab=exfil` production route; the breakdown arrays map the same image to semantic frontend units.
...
- 目标图：`evidence/ui-image-breakdowns/pages/topics-data-exfiltration/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/topics-data-exfiltration/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/topics-data-exfiltration/diff.png`
...
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/topics-data-exfiltration/text-ocr.txt`
...
- 生产 URL：`http://10.0.5.8:30180/topics?topic=exfil&tab=exfil&__codex_ui_breakdown_production=1&windowsCdpEvidenceTs=1783237631678`
- 最终 URL：`http://10.0.5.8:30180/topics?topic=exfil&tab=exfil&__codex_ui_breakdown_production=1&windowsCdpEvidenceTs=1783237631678`
- 复现步骤：先检查 `curl -i --max-time 5 http://127.0.0.1:9224/json/version` 和 `/json/list`，再用 Windows Chrome CDP 打开真实生产路由，按 `--channel-tolerance 92 --max-pixel-ratio 0.1` 生成 implementation.png、diff.png、metrics.json，并读取 verification.json。

## 差异清单

| 类型 | 位置 | 当前 | 期望 | 状态 |
|---|---|---|---|---|
| visual-diff | full image | alpha production diff ratio `0.0995240162037037`, channel tolerance `92` | pixel mismatch ratio <= 0.1 under alpha gate | accepted |
...
- 主线程判定覆盖当前 alpha 视觉门禁：内容和布局一致；亮度、透明度、glow 强度、字体抗锯齿和图表渲染差异不阻塞。
