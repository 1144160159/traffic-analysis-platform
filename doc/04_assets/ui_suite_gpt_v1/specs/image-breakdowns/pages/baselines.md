# baselines.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：行为基准
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/baselines.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/baselines.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/baselines.json`
- 对应路由/宿主路由：`/baselines`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 行为基准 | `80,54,360,24` | title | 是 |
| baselines | `510,54,360,24` | subtitle | 是 |
| /baselines | `940,54,360,24` | section | 是 |
| 行为基准。 | `1370,54,360,24` | label | 是 |
| 沉淀资产、账号、端口、协议、时间段的正常行为基线，用于异常检测和解释。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：资产台账、实体图谱、数据融合、行为基准，当前高亮“行为基准”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：行为基准。 | `80,306,360,24` | metric | 是 |
| 路由：/baselines。 | `510,306,360,24` | button | 是 |
| 业务模块：资产图谱 / 行为基准。 | `940,306,360,24` | status | 是 |
| 页面重点：沉淀资产、账号、端口、协议、时间段的正常行为基线，用于异常检测和解释。 | `1370,306,360,24` | legend | 是 |
| 必须包含的业务模块： | `80,348,360,24` | hint | 是 |
| - 基线总览：资产基线、账号基线、端口基线、流量基线、覆盖率。 | `510,348,360,24` | acceptance | 是 |
| - 偏离检测：新目的地、新端口、异常时间、异常流量、异常会话长度。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`行为基准`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`baselines`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/baselines`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`行为基准。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`沉淀资产、账号、端口、协议、时间段的正常行为基线，用于异常检测和解释。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：资产台账、实体图谱、数据融合、行为基准，当前高亮“行为基准”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：行为基准。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/baselines。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：资产图谱 / 行为基准。`，类型 status，来源 manual visual ledger。
- 文本 28：`页面重点：沉淀资产、账号、端口、协议、时间段的正常行为基线，用于异常检测和解释。`，类型 legend，来源 manual visual ledger。
- 文本 29：`必须包含的业务模块：`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 基线总览：资产基线、账号基线、端口基线、流量基线、覆盖率。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 偏离检测：新目的地、新端口、异常时间、异常流量、异常会话长度。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/baselines/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/baselines/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/baselines/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/baselines/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/baselines/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/baselines/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/baselines/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/baselines/verification.json`
