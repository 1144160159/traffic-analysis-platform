# data-quality-topic-health.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：数据质量。
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/data-quality-topic-health.png`
- 源图尺寸：1920 x 1080
...
| data-quality-topic-health | `510,54,360,24` | subtitle | 是 |
...
- 文本 02：`data-quality-topic-health`，类型 subtitle，来源 prompt/layer plus target visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/data-quality-topic-health/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/data-quality-topic-health/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/data-quality-topic-health/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/pages/data-quality-topic-health/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/pages/data-quality-topic-health/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/data-quality-topic-health/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/data-quality-topic-health/metrics.json`
- verification：`evidence/ui-image-breakdowns/pages/data-quality-topic-health/verification.json`
- 视口：1920 x 1080，DPR 1
- 浏览器：Windows Chrome CDP，经 `http://127.0.0.1:9224/json/version` 与 `/json/list` 预检。
- 生产路由：`http://10.0.5.8:30180/data-quality?tab=topic-health&__codex_ui_breakdown_production=1&windowsCdpEvidenceTs=1783238572693`
- 页面状态：`tab=topic-health`，1920 x 1080，可见 Windows Chrome 标签复用，无横向/纵向滚动。
...
- Alpha 验收口径：Topic 健康 tab、KPI、Kafka Topic 表、消费延迟趋势、分区倾斜热力图、Consumer Group、消息大小分布、异常分区队列、右侧闭环栏和底部状态栏一致；亮度、透明度、抗锯齿和密集表格/图表差异按 alpha 规则放宽。
- `metrics.json`：`status=pass`，`channel_tolerance=92`，`pixel_mismatch_ratio=0.11644434799382716`，放宽阈值 `<=0.12`。
...
- Topic 延迟趋势已恢复为占满面板的动态 ECharts，右侧栏恢复 274px，Topic 表为 7 条/页受控分页；公共 AppShell 未修改。
- Windows Chrome：canvas `7`，业务动作 endpoint/audit Drawer 通过，运行时错误为 `0`。
- 逐路由与真实点击字段质量的 Tab 几何差均为 `0`；业务 ROI：`0.11704137994799484 < 0.125`；证据：`metrics-business-r271.json`、`diff-business-r271.png`、`../data-quality/interaction-r271-topic-health.png`。
