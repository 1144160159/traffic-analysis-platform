# dashboard.png 逐图精拆记录

## 基本信息

- 分类：pages
- 标题：仪表盘
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/pages/dashboard.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/dashboard.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/dashboard.json`
- 对应路由/宿主路由：`/dashboard`
- 当前状态：`pixel-accepted`
- 复刻等级：逐图目标 PNG 已锁定；alpha 视觉门禁已通过，内容与布局一致，亮度/透明度/抗锯齿差异按当前 alpha 规则放宽。
- 最新生产证据：`traffic/web-ui:ui-dashboard-window-adaptive-20260706-r86`，Windows Chrome CDP 截图 URL `http://10.0.5.8:30180/dashboard?r86Adaptive=1783338901368`，已按浏览器窗口尺寸完成 5 档真实窗口取证，业务动态图、窗口自适应布局、hover 省略文本和 runtime 均通过，详见 `evidence/ui-image-breakdowns/pages/dashboard/dashboard-r86-window-adaptive-analysis.json`。
- r86 窗口自适应证据：`1920x1080`、`1600x900`、`1366x768`、`1200x760`、`1024x768` 五档 Windows Chrome 窗口均无横向溢出、无业务面板重叠；业务图示保持 API/ECharts 动态渲染，检测到 20 个 dashboard canvas。
- 用户反馈回修：综合态势二级菜单必须包含 `专题面板`；当前生产截图已恢复。视觉拆解模式下已绕过 `/api/v1/auth/me`，避免未登录状态产生 401 runtime 噪声。
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 仪表盘 | `80,54,360,24` | title | 是 |
| dashboard | `510,54,360,24` | subtitle | 是 |
| /dashboard | `940,54,360,24` | section | 是 |
| 仪表盘。 | `1370,54,360,24` | label | 是 |
| 日常运营脱敏工作台，回答今天必须优先关注什么、哪些 SLA/门禁有风险、哪些证据或反馈需要补齐，但不暴露人员、班组、头像或个人任务归属。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：仪表盘、态势大屏、专题面板，当前高亮“仪表盘”。 | `1370,222,360,24` | acceptance | 是 |
...
| 页面名称：仪表盘。 | `80,306,360,24` | metric | 是 |
| 路由：/dashboard。 | `510,306,360,24` | button | 是 |
| 业务模块：综合态势 / 仪表盘。 | `940,306,360,24` | status | 是 |
| 必须包含的业务模块： | `1370,306,360,24` | legend | 是 |
| - 脱敏运营 KPI：超时 SLA、临近超时数、高危未处理、待取证、待反馈、待复核、队列积压量、今日闭环进度。 | `80,348,360,24` | hint | 是 |
| - 告警处置阶段工作篮：今日必处理、处理中、待反馈、待取证、待复核、需审计留痕，以阶段分组和数量变化呈现。 | `510,348,360,24` | acceptance | 是 |
| - 证据与反馈质量摘要：PCAP/Session/日志证据完整度、反馈覆盖率、误报回流量、审计留痕缺口、样本回流缺口。 | `940,348,360,24` | title | 是 |
...
- 文本 01：`仪表盘`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`dashboard`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/dashboard`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`仪表盘。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`日常运营脱敏工作台，回答今天必须优先关注什么、哪些 SLA/门禁有风险、哪些证据或反馈需要补齐，但不暴露人员、班组、头像或个人任务归属。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 20：`左侧二级菜单显示：仪表盘、态势大屏、专题面板，当前高亮“仪表盘”。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 25：`页面名称：仪表盘。`，类型 metric，来源 manual visual ledger。
- 文本 26：`路由：/dashboard。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：综合态势 / 仪表盘。`，类型 status，来源 manual visual ledger。
- 文本 28：`必须包含的业务模块：`，类型 legend，来源 manual visual ledger。
- 文本 29：`- 脱敏运营 KPI：超时 SLA、临近超时数、高危未处理、待取证、待反馈、待复核、队列积压量、今日闭环进度。`，类型 hint，来源 manual visual ledger。
- 文本 30：`- 告警处置阶段工作篮：今日必处理、处理中、待反馈、待取证、待复核、需审计留痕，以阶段分组和数量变化呈现。`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`- 证据与反馈质量摘要：PCAP/Session/日志证据完整度、反馈覆盖率、误报回流量、审计留痕缺口、样本回流缺口。`，类型 title，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/pages/dashboard/target.png`
- 实现截图：`evidence/ui-image-breakdowns/pages/dashboard/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/pages/dashboard/diff.png`
...
- OCR/manual ledger：`evidence/ui-image-breakdowns/pages/dashboard/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/pages/dashboard/metrics.json`
...
- 生产 URL：`http://10.0.5.8:30180/dashboard?__codex_ui_breakdown_production=1&windowsCdpEvidenceTs=1783236876399`
- 最终 URL：`http://10.0.5.8:30180/dashboard?__codex_ui_breakdown_production=1&windowsCdpEvidenceTs=1783236876399`
- 复现步骤：先检查 `curl -i --max-time 5 http://127.0.0.1:9224/json/version` 和 `/json/list`，再用 Windows Chrome CDP 打开真实生产路由，按 `--channel-tolerance 88 --max-pixel-ratio 0.1` 生成 implementation.png、diff.png、metrics.json，并读取 verification.json。

## 差异清单

| 类型 | 位置 | 当前 | 期望 | 状态 |
|---|---|---|---|---|
| visual-diff | full image | alpha production diff ratio `0.09910108024691358`, channel tolerance `88` | pixel mismatch ratio <= 0.1 under alpha gate | accepted |
...
- 主线程判定覆盖当前 alpha 视觉门禁：内容和布局一致；亮度、透明度、glow 强度、字体抗锯齿和轻微图标渲染差异不阻塞。

## r260 业务约束回归

- 当前生产镜像：`traffic/web-ui:ui-dashboard-interactions-20260711-r260`。20 张 KPI、阶段、质量环和 Top Talkers ECharts 继续使用 dashboard API/typed fallback 动态数据。
- “创建闭环任务”和右侧缺口动作已接入统一确认 Drawer；`pageApiPlans.dashboard.actions` 预留任务接口、权限、审计事件、默认请求体和防护规则，确认后提供仿真任务队列反馈。
- 优先级待办队列继续使用 8 条/页受控分页；`interaction-r260.json` 验证第 2 页、20 张 canvas、证据补齐接口/审计内容和确认反馈，且运行时错误数组均为空。
- `metrics-business-r260.json` 的业务 ROI 为 `0.11076111695841993 <= 0.125`，diff 为 `diff-business-r260.png`，评分区域为 `content-root:198,80,1722,917`。
