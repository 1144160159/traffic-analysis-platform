# drawer-asset-detail.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：资产详情
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-asset-detail.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-asset-detail.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-asset-detail.json`
- 对应路由/宿主路由：`/assets`
...
| 抽屉容器 | `980,48,900,984` | 3 | 资产详情 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 资产详情 | `80,54,360,24` | title | 是 |
| drawer-asset-detail | `510,54,360,24` | subtitle | 是 |
| /assets | `940,54,360,24` | section | 是 |
| 资产详情。 | `1370,54,360,24` | label | 是 |
| 资产详情、端口服务、风险评分、告警历史、基线偏离 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：资产详情。 | `1370,264,360,24` | label | 是 |
| 基准页面：资产台账。 | `80,306,360,24` | metric | 是 |
| 基准路由：/assets。 | `510,306,360,24` | button | 是 |
| 业务模块：资产图谱 / 资产台账。 | `940,306,360,24` | status | 是 |
| 浮层重点：资产详情、端口服务、风险评分、告警历史、基线偏离 | `1370,306,360,24` | legend | 是 |
| 浮层布局：从右侧展开资产详情 Drawer，保留资产表格背景。 | `80,348,360,24` | hint | 是 |
...
| 资产详情 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 资产详情 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`资产详情`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-asset-detail`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/assets`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`资产详情。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`资产详情、端口服务、风险评分、告警历史、基线偏离`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：资产详情。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：资产详情、端口服务、风险评分、告警历史、基线偏离`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：从右侧展开资产详情 Drawer，保留资产表格背景。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-asset-detail/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-asset-detail/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-asset-detail/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-asset-detail/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-asset-detail/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-asset-detail/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-asset-detail/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-asset-detail/verification.json`
