# drawer-topic-subscription.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：专题订阅配置
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-topic-subscription.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-topic-subscription.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-topic-subscription.json`
- 对应路由/宿主路由：`/topics`
...
| 抽屉容器 | `980,48,900,984` | 3 | 专题订阅配置 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 专题订阅配置 | `80,54,360,24` | title | 是 |
| drawer-topic-subscription | `510,54,360,24` | subtitle | 是 |
| /topics | `940,54,360,24` | section | 是 |
| 专题订阅配置。 | `1370,54,360,24` | label | 是 |
| 配置专题订阅、静默、免打扰、阈值触发、通知渠道和审计留痕 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：专题订阅配置。 | `1370,264,360,24` | label | 是 |
| 基准页面：专题面板。 | `80,306,360,24` | metric | 是 |
| 基准路由：/topics。 | `510,306,360,24` | button | 是 |
| 业务模块：综合态势 / 专题面板。 | `940,306,360,24` | status | 是 |
| 浮层重点：配置专题订阅、静默、免打扰、阈值触发、通知渠道和审计留痕 | `1370,306,360,24` | legend | 是 |
| 浮层布局：右侧 Drawer，只展示专题订阅容器；包含订阅对象、触发条件、静默窗口、通知渠道、模板预览和生效范围。 | `80,348,360,24` | hint | 是 |
...
| 专题订阅配置 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 专题订阅配置 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`专题订阅配置`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-topic-subscription`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/topics`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`专题订阅配置。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`配置专题订阅、静默、免打扰、阈值触发、通知渠道和审计留痕`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：专题订阅配置。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：配置专题订阅、静默、免打扰、阈值触发、通知渠道和审计留痕`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：右侧 Drawer，只展示专题订阅容器；包含订阅对象、触发条件、静默窗口、通知渠道、模板预览和生效范围。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-topic-subscription/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-topic-subscription/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-topic-subscription/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-topic-subscription/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-topic-subscription/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-topic-subscription/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-topic-subscription/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-topic-subscription/verification.json`
