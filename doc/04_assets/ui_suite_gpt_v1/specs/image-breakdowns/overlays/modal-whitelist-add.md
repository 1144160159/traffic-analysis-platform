# modal-whitelist-add.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：添加白名单
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-whitelist-add.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-whitelist-add.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-whitelist-add.json`
- 对应路由/宿主路由：`/whitelist`
...
| Modal 容器 | `360,170,1200,740` | 3 | 添加白名单 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 添加白名单 | `80,54,360,24` | title | 是 |
| modal-whitelist-add | `510,54,360,24` | subtitle | 是 |
| /whitelist | `940,54,360,24` | section | 是 |
| 添加白名单。 | `1370,54,360,24` | label | 是 |
| 添加白名单，填写对象、规则、理由、有效期和审批链 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“白名单”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：添加白名单。 | `1370,264,360,24` | label | 是 |
| 基准页面：白名单。 | `80,306,360,24` | metric | 是 |
| 基准路由：/whitelist。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / 白名单。 | `940,306,360,24` | status | 是 |
| 浮层重点：添加白名单，填写对象、规则、理由、有效期和审批链 | `1370,306,360,24` | legend | 是 |
| 浮层布局：表单 Modal，右侧显示影响评估。 | `80,348,360,24` | hint | 是 |
...
| 添加白名单 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 添加白名单 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`添加白名单`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-whitelist-add`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/whitelist`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`添加白名单。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`添加白名单，填写对象、规则、理由、有效期和审批链`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“白名单”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：添加白名单。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：白名单。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/whitelist。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：检测运营 / 白名单。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：添加白名单，填写对象、规则、理由、有效期和审批链`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：表单 Modal，右侧显示影响评估。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-whitelist-add/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-whitelist-add/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-whitelist-add/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-whitelist-add/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-whitelist-add/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-whitelist-add/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-whitelist-add/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-whitelist-add/verification.json`
