# modal-topic-save-view.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：专题保存视图
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-topic-save-view.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-topic-save-view.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-topic-save-view.json`
- 对应路由/宿主路由：`/topics`
...
| Modal 容器 | `360,170,1200,740` | 3 | 专题保存视图 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 专题保存视图 | `80,54,360,24` | title | 是 |
| modal-topic-save-view | `510,54,360,24` | subtitle | 是 |
| /topics | `940,54,360,24` | section | 是 |
| 专题保存视图。 | `1370,54,360,24` | label | 是 |
| 固化当前专题模式、筛选范围、时间窗、资产范围和证据门禁，保存为可复用视图 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：专题保存视图。 | `1370,264,360,24` | label | 是 |
| 基准页面：专题面板。 | `80,306,360,24` | metric | 是 |
| 基准路由：/topics。 | `510,306,360,24` | button | 是 |
| 业务模块：综合态势 / 专题面板。 | `940,306,360,24` | status | 是 |
| 浮层重点：固化当前专题模式、筛选范围、时间窗、资产范围和证据门禁，保存为可复用视图 | `1370,306,360,24` | legend | 是 |
| 浮层布局：居中 Modal，只展示专题保存业务容器本体；包含视图名称、专题模式、筛选摘要、可见范围、刷新策略、权限边界和保存审计提示。 | `80,348,360,24` | hint | 是 |
...
| 专题保存视图 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 专题保存视图 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`专题保存视图`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-topic-save-view`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/topics`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`专题保存视图。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`固化当前专题模式、筛选范围、时间窗、资产范围和证据门禁，保存为可复用视图`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：专题保存视图。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：固化当前专题模式、筛选范围、时间窗、资产范围和证据门禁，保存为可复用视图`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：居中 Modal，只展示专题保存业务容器本体；包含视图名称、专题模式、筛选摘要、可见范围、刷新策略、权限边界和保存审计提示。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-topic-save-view/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-topic-save-view/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-topic-save-view/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-topic-save-view/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-topic-save-view/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-topic-save-view/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-topic-save-view/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-topic-save-view/verification.json`
