# modal-login-error-captcha.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：登录异常与验证码状态
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-login-error-captcha.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-login-error-captcha.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-login-error-captcha.json`
- 对应路由/宿主路由：`/login`
...
| Modal 容器 | `360,170,1200,740` | 3 | 登录异常与验证码状态 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 登录异常与验证码状态 | `80,54,360,24` | title | 是 |
| modal-login-error-captcha | `510,54,360,24` | subtitle | 是 |
| /login | `940,54,360,24` | section | 是 |
| 登录异常与验证码状态。 | `1370,54,360,24` | label | 是 |
| 登录异常与验证码状态，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。 | `80,96,360,24` | metric | 是 |
...
| 登录/认证类页面不显示左侧业务导航和二级菜单，但视觉风格必须与主系统一致。 | `940,222,360,24` | hint | 是 |
| 登录/认证类页面不显示业务顶部 KPI 条，可展示系统名称、租户/站点、安全提示和能力摘要。 | `1370,222,360,24` | acceptance | 是 |
...
| 浮层名称：登录异常与验证码状态。 | `1370,264,360,24` | label | 是 |
| 基准页面：登录。 | `80,306,360,24` | metric | 是 |
| 基准路由：/login。 | `510,306,360,24` | button | 是 |
| 业务模块：登录入口 / 身份认证。 | `940,306,360,24` | status | 是 |
| 浮层重点：登录异常与验证码状态，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。 | `1370,306,360,24` | legend | 是 |
| 浮层布局：在对应业务页面上展示清晰的业务浮层状态，背景轻微压暗；浮层内容按深色高密度表单、详情、状态机或确认提示组织。 | `80,348,360,24` | hint | 是 |
...
| 登录异常与验证码状态 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 登录异常与验证码状态 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`登录异常与验证码状态`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-login-error-captcha`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/login`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`登录异常与验证码状态。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`登录异常与验证码状态，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`登录/认证类页面不显示左侧业务导航和二级菜单，但视觉风格必须与主系统一致。`，类型 hint，来源 manual visual ledger。
- 文本 20：`登录/认证类页面不显示业务顶部 KPI 条，可展示系统名称、租户/站点、安全提示和能力摘要。`，类型 acceptance，来源 manual visual ledger。
...
- 文本 24：`浮层名称：登录异常与验证码状态。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：登录。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/login。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：登录入口 / 身份认证。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：登录异常与验证码状态，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：在对应业务页面上展示清晰的业务浮层状态，背景轻微压暗；浮层内容按深色高密度表单、详情、状态机或确认提示组织。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-login-error-captcha/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-login-error-captcha/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-login-error-captcha/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-login-error-captcha/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-login-error-captcha/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-login-error-captcha/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-login-error-captcha/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-login-error-captcha/verification.json`
