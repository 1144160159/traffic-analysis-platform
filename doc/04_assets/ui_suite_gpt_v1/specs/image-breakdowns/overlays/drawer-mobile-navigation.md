# drawer-mobile-navigation.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：移动端侧滑菜单
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-mobile-navigation.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-mobile-navigation.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-mobile-navigation.json`
- 对应路由/宿主路由：`/dashboard`
...
| 抽屉容器 | `980,48,900,984` | 3 | 移动端侧滑菜单 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 移动端侧滑菜单 | `80,54,360,24` | title | 是 |
| drawer-mobile-navigation | `510,54,360,24` | subtitle | 是 |
| /dashboard | `940,54,360,24` | section | 是 |
| 移动端侧滑菜单。 | `1370,54,360,24` | label | 是 |
| 移动端侧滑菜单，展示一级菜单、二级菜单、用户状态和站点选择 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：移动端侧滑菜单。 | `1370,264,360,24` | label | 是 |
| 基准页面：仪表盘。 | `80,306,360,24` | metric | 是 |
| 基准路由：/dashboard。 | `510,306,360,24` | button | 是 |
| 业务模块：综合态势 / 仪表盘。 | `940,306,360,24` | status | 是 |
| 浮层重点：移动端侧滑菜单，展示一级菜单、二级菜单、用户状态和站点选择 | `1370,306,360,24` | legend | 是 |
| 浮层布局：模拟窄屏移动端，左侧 Drawer 展开，背景内容压暗。 | `80,348,360,24` | hint | 是 |
...
| 移动端侧滑菜单 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 移动端侧滑菜单 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`移动端侧滑菜单`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-mobile-navigation`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/dashboard`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`移动端侧滑菜单。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`移动端侧滑菜单，展示一级菜单、二级菜单、用户状态和站点选择`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：移动端侧滑菜单。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：移动端侧滑菜单，展示一级菜单、二级菜单、用户状态和站点选择`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：模拟窄屏移动端，左侧 Drawer 展开，背景内容压暗。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-mobile-navigation/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-mobile-navigation/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-mobile-navigation/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-mobile-navigation/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-mobile-navigation/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-mobile-navigation/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-mobile-navigation/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-mobile-navigation/verification.json`
