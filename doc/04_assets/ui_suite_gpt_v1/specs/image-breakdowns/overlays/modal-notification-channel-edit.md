# modal-notification-channel-edit.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：通知渠道编辑
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-notification-channel-edit.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-notification-channel-edit.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-notification-channel-edit.json`
...
| Modal 容器 | `360,170,1200,740` | 3 | 通知渠道编辑 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 通知渠道编辑 | `80,54,360,24` | title | 是 |
| modal-notification-channel-edit | `510,54,360,24` | subtitle | 是 |
| /notifications | `940,54,360,24` | section | 是 |
| 通知渠道编辑。 | `1370,54,360,24` | label | 是 |
| 新增或编辑通知渠道，配置 Webhook、邮件、短信、IM、认证方式、路由规则和测试状态 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：通知渠道编辑。 | `1370,264,360,24` | label | 是 |
| 基准页面：通知配置。 | `80,306,360,24` | metric | 是 |
| 基准路由：/notifications。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 通知配置。 | `940,306,360,24` | status | 是 |
| 浮层重点：新增或编辑通知渠道，配置 Webhook、邮件、短信、IM、认证方式、路由规则和测试状态 | `1370,306,360,24` | legend | 是 |
| 浮层布局：居中宽 Modal，只展示通知渠道编辑容器；包含渠道类型、连接参数、凭证脱敏、路由条件、健康检查和保存审计提示。 | `80,348,360,24` | hint | 是 |
...
| 通知渠道编辑 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 通知渠道编辑 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`通知渠道编辑`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-notification-channel-edit`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/notifications`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`通知渠道编辑。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`新增或编辑通知渠道，配置 Webhook、邮件、短信、IM、认证方式、路由规则和测试状态`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：通知渠道编辑。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：新增或编辑通知渠道，配置 Webhook、邮件、短信、IM、认证方式、路由规则和测试状态`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：居中宽 Modal，只展示通知渠道编辑容器；包含渠道类型、连接参数、凭证脱敏、路由条件、健康检查和保存审计提示。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-notification-channel-edit/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-notification-channel-edit/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-notification-channel-edit/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-notification-channel-edit/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-notification-channel-edit/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-notification-channel-edit/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-notification-channel-edit/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-notification-channel-edit/verification.json`
