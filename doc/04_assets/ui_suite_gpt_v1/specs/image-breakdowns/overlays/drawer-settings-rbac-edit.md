# drawer-settings-rbac-edit.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：RBAC 权限编辑
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-settings-rbac-edit.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-settings-rbac-edit.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-settings-rbac-edit.json`
- 对应路由/宿主路由：`/settings`
...
| 抽屉容器 | `980,48,900,984` | 3 | RBAC 权限编辑 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| RBAC 权限编辑 | `80,54,360,24` | title | 是 |
| drawer-settings-rbac-edit | `510,54,360,24` | subtitle | 是 |
| /settings | `940,54,360,24` | section | 是 |
| RBAC 权限编辑。 | `1370,54,360,24` | label | 是 |
| RBAC 权限矩阵编辑，展示角色、资源、动作、租户边界、影响范围和审批提示 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：RBAC 权限编辑。 | `1370,264,360,24` | label | 是 |
| 基准页面：系统设置。 | `80,306,360,24` | metric | 是 |
| 基准路由：/settings。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 系统设置。 | `940,306,360,24` | status | 是 |
| 浮层重点：RBAC 权限矩阵编辑，展示角色、资源、动作、租户边界、影响范围和审批提示 | `1370,306,360,24` | legend | 是 |
| 浮层布局：右侧宽 Drawer，只展示 RBAC 编辑容器；包含角色选择、权限矩阵、资源范围、策略冲突、影响用户数、审批门禁和审计留痕。 | `80,348,360,24` | hint | 是 |
...
| RBAC 权限编辑 视觉校正项 41 | `80,474,360,24` | title | 否 |
| RBAC 权限编辑 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`RBAC 权限编辑`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-settings-rbac-edit`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/settings`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`RBAC 权限编辑。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`RBAC 权限矩阵编辑，展示角色、资源、动作、租户边界、影响范围和审批提示`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：RBAC 权限编辑。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：RBAC 权限矩阵编辑，展示角色、资源、动作、租户边界、影响范围和审批提示`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：右侧宽 Drawer，只展示 RBAC 编辑容器；包含角色选择、权限矩阵、资源范围、策略冲突、影响用户数、审批门禁和审计留痕。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-settings-rbac-edit/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-settings-rbac-edit/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-settings-rbac-edit/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-settings-rbac-edit/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-settings-rbac-edit/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-settings-rbac-edit/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-settings-rbac-edit/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-settings-rbac-edit/verification.json`
