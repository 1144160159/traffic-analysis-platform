# popconfirm-delete.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：规则删除确认
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/popconfirm-delete.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/popconfirm-delete.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/popconfirm-delete.json`
- 对应路由/宿主路由：`/rules`
...
| 确认气泡 | `1120,164,560,500` | 3 | 规则删除确认 | 容器阴影、描边和圆角一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 规则删除确认 | `80,54,360,24` | title | 是 |
| popconfirm-delete | `510,54,360,24` | subtitle | 是 |
| /rules | `940,54,360,24` | section | 是 |
| 规则删除确认。 | `1370,54,360,24` | label | 是 |
| 规则删除确认，显示规则名、影响范围和审计记录 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：规则删除确认。 | `1370,264,360,24` | label | 是 |
| 基准页面：规则管理。 | `80,306,360,24` | metric | 是 |
| 基准路由：/rules。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / 规则管理。 | `940,306,360,24` | status | 是 |
| 浮层重点：规则删除确认，显示规则名、影响范围和审计记录 | `1370,306,360,24` | legend | 是 |
| 浮层布局：小型 Popconfirm 锚定规则表格删除按钮，危险按钮为红色。 | `80,348,360,24` | hint | 是 |
...
| 规则删除确认 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 规则删除确认 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`规则删除确认`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`popconfirm-delete`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/rules`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`规则删除确认。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`规则删除确认，显示规则名、影响范围和审计记录`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：规则删除确认。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：规则删除确认，显示规则名、影响范围和审计记录`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：小型 Popconfirm 锚定规则表格删除按钮，危险按钮为红色。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/popconfirm-delete/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/popconfirm-delete/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/popconfirm-delete/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/popconfirm-delete/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/popconfirm-delete/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/popconfirm-delete/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/popconfirm-delete/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/popconfirm-delete/verification.json`
