# drawer-field-quality-sample.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：字段质量样例
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-field-quality-sample.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-field-quality-sample.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-field-quality-sample.json`
...
| 抽屉容器 | `980,48,900,984` | 3 | 字段质量样例 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 字段质量样例 | `80,54,360,24` | title | 是 |
| drawer-field-quality-sample | `510,54,360,24` | subtitle | 是 |
| /data-quality | `940,54,360,24` | section | 是 |
| 字段质量样例。 | `1370,54,360,24` | label | 是 |
| 字段质量样例，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。 | `80,96,360,24` | metric | 是 |
...
| 字段质量样例 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 字段质量样例 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`字段质量样例`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-field-quality-sample`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/data-quality`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`字段质量样例。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`字段质量样例，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：字段质量样例。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：字段质量样例，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 legend，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-field-quality-sample/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-field-quality-sample/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-field-quality-sample/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-field-quality-sample/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-field-quality-sample/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-field-quality-sample/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-field-quality-sample/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-field-quality-sample/verification.json`
