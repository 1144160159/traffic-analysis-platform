# drawer-encrypted-fingerprint.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：加密指纹详情
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-encrypted-fingerprint.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-encrypted-fingerprint.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-encrypted-fingerprint.json`
...
| 抽屉容器 | `980,48,900,984` | 3 | 加密指纹详情 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 加密指纹详情 | `80,54,360,24` | title | 是 |
| drawer-encrypted-fingerprint | `510,54,360,24` | subtitle | 是 |
| /encrypted-traffic | `940,54,360,24` | section | 是 |
| 加密指纹详情。 | `1370,54,360,24` | label | 是 |
| 加密指纹详情，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“加密流量”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：加密指纹详情。 | `1370,264,360,24` | label | 是 |
| 基准页面：加密流量。 | `80,306,360,24` | metric | 是 |
| 基准路由：/encrypted-traffic。 | `510,306,360,24` | button | 是 |
| 业务模块：威胁分析 / 加密流量。 | `940,306,360,24` | status | 是 |
| 浮层重点：加密指纹详情，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。 | `1370,306,360,24` | legend | 是 |
...
| 加密指纹详情 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 加密指纹详情 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`加密指纹详情`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-encrypted-fingerprint`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/encrypted-traffic`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`加密指纹详情。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`加密指纹详情，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“加密流量”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：加密指纹详情。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：加密流量。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/encrypted-traffic。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：威胁分析 / 加密流量。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：加密指纹详情，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 legend，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-encrypted-fingerprint/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-encrypted-fingerprint/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-encrypted-fingerprint/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-encrypted-fingerprint/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-encrypted-fingerprint/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-encrypted-fingerprint/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-encrypted-fingerprint/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-encrypted-fingerprint/verification.json`
