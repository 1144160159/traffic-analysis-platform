# modal-audit-export.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：审计材料导出
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-audit-export.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-audit-export.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-audit-export.json`
- 对应路由/宿主路由：`/audit-log`
...
| Modal 容器 | `360,170,1200,740` | 3 | 审计材料导出 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 审计材料导出 | `80,54,360,24` | title | 是 |
| modal-audit-export | `510,54,360,24` | subtitle | 是 |
| /audit-log | `940,54,360,24` | section | 是 |
| 审计材料导出。 | `1370,54,360,24` | label | 是 |
| 导出审计取证材料并做权限确认，选择日志范围、字段脱敏、签名校验和审批门禁 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“审计日志”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：审计材料导出。 | `1370,264,360,24` | label | 是 |
| 基准页面：审计日志。 | `80,306,360,24` | metric | 是 |
| 基准路由：/audit-log。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 审计日志。 | `940,306,360,24` | status | 是 |
| 浮层重点：导出审计取证材料并做权限确认，选择日志范围、字段脱敏、签名校验和审批门禁 | `1370,306,360,24` | legend | 是 |
| 浮层布局：居中 Modal，只展示审计导出容器；包含时间窗、操作类型、对象范围、字段选择、脱敏预览、hash 校验和权限确认。 | `80,348,360,24` | hint | 是 |
...
| 审计材料导出 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 审计材料导出 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`审计材料导出`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-audit-export`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/audit-log`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`审计材料导出。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`导出审计取证材料并做权限确认，选择日志范围、字段脱敏、签名校验和审批门禁`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“审计日志”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：审计材料导出。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：审计日志。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/audit-log。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：审计配置 / 审计日志。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：导出审计取证材料并做权限确认，选择日志范围、字段脱敏、签名校验和审批门禁`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：居中 Modal，只展示审计导出容器；包含时间窗、操作类型、对象范围、字段选择、脱敏预览、hash 校验和权限确认。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-audit-export/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-audit-export/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-audit-export/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-audit-export/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-audit-export/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-audit-export/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-audit-export/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-audit-export/verification.json`
