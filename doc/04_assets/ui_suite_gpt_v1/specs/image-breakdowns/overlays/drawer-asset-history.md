# drawer-asset-history.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：资产历史
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-asset-history.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-asset-history.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-asset-history.json`
- 对应路由/宿主路由：`/assets`
...
| 抽屉容器 | `980,48,900,984` | 3 | 资产历史 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 资产历史 | `80,54,360,24` | title | 是 |
| drawer-asset-history | `510,54,360,24` | subtitle | 是 |
| /assets | `940,54,360,24` | section | 是 |
| 资产历史。 | `1370,54,360,24` | label | 是 |
| 资产历史，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。 | `80,96,360,24` | metric | 是 |
...
| 资产历史 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 资产历史 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`资产历史`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-asset-history`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/assets`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`资产历史。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`资产历史，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：资产历史。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：资产历史，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 legend，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-asset-history/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-asset-history/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-asset-history/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-asset-history/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-asset-history/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-asset-history/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-asset-history/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-asset-history/verification.json`
