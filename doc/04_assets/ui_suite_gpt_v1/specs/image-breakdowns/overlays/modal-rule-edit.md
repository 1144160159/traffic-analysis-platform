# modal-rule-edit.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：新建/编辑规则
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-rule-edit.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-rule-edit.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-rule-edit.json`
- 对应路由/宿主路由：`/rules`
...
| Modal 容器 | `360,170,1200,740` | 3 | 新建/编辑规则 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 新建/编辑规则 | `80,54,360,24` | title | 是 |
| modal-rule-edit | `510,54,360,24` | subtitle | 是 |
| /rules | `940,54,360,24` | section | 是 |
| 新建/编辑规则。 | `1370,54,360,24` | label | 是 |
| 新建或编辑检测规则，包含 DSL、条件、严重级别、灰度范围 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：新建/编辑规则。 | `1370,264,360,24` | label | 是 |
| 基准页面：规则管理。 | `80,306,360,24` | metric | 是 |
| 基准路由：/rules。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / 规则管理。 | `940,306,360,24` | status | 是 |
| 浮层重点：新建或编辑检测规则，包含 DSL、条件、严重级别、灰度范围 | `1370,306,360,24` | legend | 是 |
| 浮层布局：大 Modal，左侧表单，右侧规则预览和测试结果。 | `80,348,360,24` | hint | 是 |
...
| 新建/编辑规则 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 新建/编辑规则 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`新建/编辑规则`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-rule-edit`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/rules`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`新建/编辑规则。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`新建或编辑检测规则，包含 DSL、条件、严重级别、灰度范围`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：新建/编辑规则。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：新建或编辑检测规则，包含 DSL、条件、严重级别、灰度范围`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：大 Modal，左侧表单，右侧规则预览和测试结果。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-rule-edit/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-rule-edit/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-rule-edit/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-rule-edit/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-rule-edit/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-rule-edit/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-rule-edit/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-rule-edit/verification.json`
