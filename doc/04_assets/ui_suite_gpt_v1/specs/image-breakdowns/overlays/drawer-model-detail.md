# drawer-model-detail.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：模型详情
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-model-detail.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-model-detail.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-model-detail.json`
- 对应路由/宿主路由：`/models`
...
| 抽屉容器 | `980,48,900,984` | 3 | 模型详情 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 模型详情 | `80,54,360,24` | title | 是 |
| drawer-model-detail | `510,54,360,24` | subtitle | 是 |
| /models | `940,54,360,24` | section | 是 |
| 模型详情。 | `1370,54,360,24` | label | 是 |
| 模型详情、版本、训练数据、评估指标、在线表现和回滚入口 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“模型管理”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：模型详情。 | `1370,264,360,24` | label | 是 |
| 基准页面：模型管理。 | `80,306,360,24` | metric | 是 |
| 基准路由：/models。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / 模型管理。 | `940,306,360,24` | status | 是 |
| 浮层重点：模型详情、版本、训练数据、评估指标、在线表现和回滚入口 | `1370,306,360,24` | legend | 是 |
| 浮层布局：右侧 Drawer，底部固定上线/回滚动作。 | `80,348,360,24` | hint | 是 |
...
| 模型详情 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 模型详情 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`模型详情`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-model-detail`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/models`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`模型详情。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`模型详情、版本、训练数据、评估指标、在线表现和回滚入口`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“模型管理”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：模型详情。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：模型管理。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/models。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：检测运营 / 模型管理。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：模型详情、版本、训练数据、评估指标、在线表现和回滚入口`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：右侧 Drawer，底部固定上线/回滚动作。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-model-detail/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-model-detail/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-model-detail/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-model-detail/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-model-detail/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-model-detail/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-model-detail/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-model-detail/verification.json`
