# modal-topic-report-export.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：专题报告导出
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-topic-report-export.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-topic-report-export.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-topic-report-export.json`
- 对应路由/宿主路由：`/topics`
...
| Modal 容器 | `360,170,1200,740` | 3 | 专题报告导出 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 专题报告导出 | `80,54,360,24` | title | 是 |
| modal-topic-report-export | `510,54,360,24` | subtitle | 是 |
| /topics | `940,54,360,24` | section | 是 |
| 专题报告导出。 | `1370,54,360,24` | label | 是 |
| 导出专题报告 PDF/Word，选择报告模板、脱敏级别、证据范围和审批门禁 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：专题报告导出。 | `1370,264,360,24` | label | 是 |
| 基准页面：专题面板。 | `80,306,360,24` | metric | 是 |
| 基准路由：/topics。 | `510,306,360,24` | button | 是 |
| 业务模块：综合态势 / 专题面板。 | `940,306,360,24` | status | 是 |
| 浮层重点：导出专题报告 PDF/Word，选择报告模板、脱敏级别、证据范围和审批门禁 | `1370,306,360,24` | legend | 是 |
| 浮层布局：居中宽 Modal，只展示专题报告导出容器；包含报告格式、章节选择、专题时间窗、附件清单、脱敏策略、审批提示和导出动作。 | `80,348,360,24` | hint | 是 |
...
| 专题报告导出 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 专题报告导出 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`专题报告导出`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-topic-report-export`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/topics`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`专题报告导出。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`导出专题报告 PDF/Word，选择报告模板、脱敏级别、证据范围和审批门禁`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：专题报告导出。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：导出专题报告 PDF/Word，选择报告模板、脱敏级别、证据范围和审批门禁`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：居中宽 Modal，只展示专题报告导出容器；包含报告格式、章节选择、专题时间窗、附件清单、脱敏策略、审批提示和导出动作。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-topic-report-export/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-topic-report-export/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-topic-report-export/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-topic-report-export/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-topic-report-export/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-topic-report-export/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-topic-report-export/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-topic-report-export/verification.json`
