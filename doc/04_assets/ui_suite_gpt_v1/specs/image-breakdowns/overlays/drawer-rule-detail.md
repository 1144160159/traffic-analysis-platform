# drawer-rule-detail.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：规则详情
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-rule-detail.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-rule-detail.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-rule-detail.json`
- 对应路由/宿主路由：`/rules`
...
| 抽屉容器 | `980,48,900,984` | 3 | 规则详情 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 规则详情 | `80,54,360,24` | title | 是 |
| drawer-rule-detail | `510,54,360,24` | subtitle | 是 |
| /rules | `940,54,360,24` | section | 是 |
| 规则详情。 | `1370,54,360,24` | label | 是 |
| 规则详情、版本历史、命中趋势、发布记录、关联模型 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：规则详情。 | `1370,264,360,24` | label | 是 |
| 基准页面：规则管理。 | `80,306,360,24` | metric | 是 |
| 基准路由：/rules。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / 规则管理。 | `940,306,360,24` | status | 是 |
| 浮层重点：规则详情、版本历史、命中趋势、发布记录、关联模型 | `1370,306,360,24` | legend | 是 |
| 浮层布局：右侧 Drawer 展示规则生命周期。 | `80,348,360,24` | hint | 是 |
...
| 规则详情 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 规则详情 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`规则详情`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-rule-detail`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/rules`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`规则详情。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`规则详情、版本历史、命中趋势、发布记录、关联模型`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：规则详情。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：规则详情、版本历史、命中趋势、发布记录、关联模型`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：右侧 Drawer 展示规则生命周期。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-rule-detail/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-rule-detail/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-rule-detail/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-rule-detail/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-rule-detail/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-rule-detail/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-rule-detail/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-rule-detail/verification.json`
