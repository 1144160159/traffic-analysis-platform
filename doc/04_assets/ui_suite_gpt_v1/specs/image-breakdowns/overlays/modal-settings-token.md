# modal-settings-token.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：创建 API 令牌
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-settings-token.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-settings-token.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-settings-token.json`
- 对应路由/宿主路由：`/settings`
...
| Modal 容器 | `360,170,1200,740` | 3 | 创建 API 令牌 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 创建 API 令牌 | `80,54,360,24` | title | 是 |
| modal-settings-token | `510,54,360,24` | subtitle | 是 |
| /settings | `940,54,360,24` | section | 是 |
| 创建 API 令牌。 | `1370,54,360,24` | label | 是 |
| 创建 API 令牌，设置权限、有效期、访问范围和安全提醒 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“系统设置”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：创建 API 令牌。 | `1370,264,360,24` | label | 是 |
| 基准页面：系统设置。 | `80,306,360,24` | metric | 是 |
| 基准路由：/settings。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 系统设置。 | `940,306,360,24` | status | 是 |
| 浮层重点：创建 API 令牌，设置权限、有效期、访问范围和安全提醒 | `1370,306,360,24` | legend | 是 |
| 浮层布局：设置页上打开 Modal，令牌值区域带一次性展示提示。 | `80,348,360,24` | hint | 是 |
...
| 创建 API 令牌 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 创建 API 令牌 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`创建 API 令牌`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-settings-token`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/settings`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`创建 API 令牌。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`创建 API 令牌，设置权限、有效期、访问范围和安全提醒`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“系统设置”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：创建 API 令牌。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：系统设置。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/settings。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：审计配置 / 系统设置。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：创建 API 令牌，设置权限、有效期、访问范围和安全提醒`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：设置页上打开 Modal，令牌值区域带一次性展示提示。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-settings-token/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-settings-token/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-settings-token/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-settings-token/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-settings-token/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-settings-token/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-settings-token/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-settings-token/verification.json`
