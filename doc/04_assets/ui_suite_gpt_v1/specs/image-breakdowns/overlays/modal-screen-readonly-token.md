# modal-screen-readonly-token.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：态势大屏只读令牌/脱敏配置
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-screen-readonly-token.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-screen-readonly-token.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-screen-readonly-token.json`
- 对应路由/宿主路由：`/screen`
...
| Modal 容器 | `360,170,1200,740` | 3 | 态势大屏只读令牌/脱敏配置 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 态势大屏只读令牌/脱敏配置 | `80,54,360,24` | title | 是 |
| modal-screen-readonly-token | `510,54,360,24` | subtitle | 是 |
| /screen | `940,54,360,24` | section | 是 |
| 态势大屏只读令牌/脱敏配置。 | `1370,54,360,24` | label | 是 |
| 态势大屏只读令牌/脱敏配置，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：仪表盘、态势大屏、专题面板，当前高亮“态势大屏”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：态势大屏只读令牌/脱敏配置。 | `1370,264,360,24` | label | 是 |
| 基准页面：态势大屏。 | `80,306,360,24` | metric | 是 |
| 基准路由：/screen。 | `510,306,360,24` | button | 是 |
| 业务模块：综合态势 / 态势大屏。 | `940,306,360,24` | status | 是 |
| 浮层重点：态势大屏只读令牌/脱敏配置，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。 | `1370,306,360,24` | legend | 是 |
...
| 态势大屏只读令牌/脱敏配置 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 态势大屏只读令牌/脱敏配置 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`态势大屏只读令牌/脱敏配置`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-screen-readonly-token`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/screen`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`态势大屏只读令牌/脱敏配置。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`态势大屏只读令牌/脱敏配置，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：仪表盘、态势大屏、专题面板，当前高亮“态势大屏”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：态势大屏只读令牌/脱敏配置。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：态势大屏。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/screen。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：综合态势 / 态势大屏。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：态势大屏只读令牌/脱敏配置，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 legend，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-screen-readonly-token/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-screen-readonly-token/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-screen-readonly-token/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-screen-readonly-token/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-screen-readonly-token/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-screen-readonly-token/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-screen-readonly-token/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-screen-readonly-token/verification.json`
