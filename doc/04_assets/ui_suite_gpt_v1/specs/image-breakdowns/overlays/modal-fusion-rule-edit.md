# modal-fusion-rule-edit.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：融合规则编辑
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-fusion-rule-edit.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-fusion-rule-edit.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-fusion-rule-edit.json`
- 对应路由/宿主路由：`/fusion`
...
| Modal 容器 | `360,170,1200,740` | 3 | 融合规则编辑 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 融合规则编辑 | `80,54,360,24` | title | 是 |
| modal-fusion-rule-edit | `510,54,360,24` | subtitle | 是 |
| /fusion | `940,54,360,24` | section | 是 |
| 融合规则编辑。 | `1370,54,360,24` | label | 是 |
| 融合规则编辑，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：资产台账、实体图谱、数据融合、行为基准，当前高亮“数据融合”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：融合规则编辑。 | `1370,264,360,24` | label | 是 |
| 基准页面：数据融合。 | `80,306,360,24` | metric | 是 |
| 基准路由：/fusion。 | `510,306,360,24` | button | 是 |
| 业务模块：资产图谱 / 数据融合。 | `940,306,360,24` | status | 是 |
| 浮层重点：融合规则编辑，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。 | `1370,306,360,24` | legend | 是 |
...
| 融合规则编辑 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 融合规则编辑 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`融合规则编辑`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-fusion-rule-edit`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/fusion`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`融合规则编辑。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`融合规则编辑，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：资产台账、实体图谱、数据融合、行为基准，当前高亮“数据融合”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：融合规则编辑。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：数据融合。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/fusion。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：资产图谱 / 数据融合。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：融合规则编辑，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 legend，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-fusion-rule-edit/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-fusion-rule-edit/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-fusion-rule-edit/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-fusion-rule-edit/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-fusion-rule-edit/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-fusion-rule-edit/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-fusion-rule-edit/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-fusion-rule-edit/verification.json`
