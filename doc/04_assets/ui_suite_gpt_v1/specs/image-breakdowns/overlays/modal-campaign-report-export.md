# modal-campaign-report-export.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：战役报告导出
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-campaign-report-export.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-campaign-report-export.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-campaign-report-export.json`
- 对应路由/宿主路由：`/campaigns/:campaignId`
...
| Modal 容器 | `360,170,1200,740` | 3 | 战役报告导出 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 战役报告导出 | `80,54,360,24` | title | 是 |
| modal-campaign-report-export | `510,54,360,24` | subtitle | 是 |
| /campaigns/:campaignId | `940,54,360,24` | section | 是 |
| 战役报告导出。 | `1370,54,360,24` | label | 是 |
| 战役详情生成战役报告，选择攻击阶段、影响范围、证据包、处置结论和复盘建议 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“战役列表”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：战役报告导出。 | `1370,264,360,24` | label | 是 |
| 基准页面：战役详情。 | `80,306,360,24` | metric | 是 |
| 基准路由：/campaigns/:campaignId。 | `510,306,360,24` | button | 是 |
| 业务模块：威胁分析 / 战役列表。 | `940,306,360,24` | status | 是 |
| 浮层重点：战役详情生成战役报告，选择攻击阶段、影响范围、证据包、处置结论和复盘建议 | `1370,306,360,24` | legend | 是 |
| 浮层布局：居中宽 Modal，只展示战役报告生成容器；包含报告模板、阶段时间轴、证据完整度、影响范围、脱敏策略、审批门禁和导出动作。 | `80,348,360,24` | hint | 是 |
...
| 战役报告导出 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 战役报告导出 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`战役报告导出`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-campaign-report-export`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/campaigns/:campaignId`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`战役报告导出。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`战役详情生成战役报告，选择攻击阶段、影响范围、证据包、处置结论和复盘建议`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“战役列表”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：战役报告导出。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：战役详情。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/campaigns/:campaignId。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：威胁分析 / 战役列表。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：战役详情生成战役报告，选择攻击阶段、影响范围、证据包、处置结论和复盘建议`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：居中宽 Modal，只展示战役报告生成容器；包含报告模板、阶段时间轴、证据完整度、影响范围、脱敏策略、审批门禁和导出动作。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-campaign-report-export/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-campaign-report-export/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-campaign-report-export/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-campaign-report-export/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-campaign-report-export/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-campaign-report-export/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-campaign-report-export/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-campaign-report-export/verification.json`
