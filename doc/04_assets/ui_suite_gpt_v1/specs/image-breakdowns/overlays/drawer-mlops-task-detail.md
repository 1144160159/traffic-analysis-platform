# drawer-mlops-task-detail.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：MLOps 任务详情
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-mlops-task-detail.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-mlops-task-detail.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-mlops-task-detail.json`
- 对应路由/宿主路由：`/mlops`
...
| 抽屉容器 | `980,48,900,984` | 3 | MLOps 任务详情 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| MLOps 任务详情 | `80,54,360,24` | title | 是 |
| drawer-mlops-task-detail | `510,54,360,24` | subtitle | 是 |
| /mlops | `940,54,360,24` | section | 是 |
| MLOps 任务详情。 | `1370,54,360,24` | label | 是 |
| MLOps 任务详情，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“MLOps 编排”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：MLOps 任务详情。 | `1370,264,360,24` | label | 是 |
| 基准页面：MLOps 编排。 | `80,306,360,24` | metric | 是 |
| 基准路由：/mlops。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / MLOps 编排。 | `940,306,360,24` | status | 是 |
| 浮层重点：MLOps 任务详情，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。 | `1370,306,360,24` | legend | 是 |
...
| MLOps 任务详情 视觉校正项 41 | `80,474,360,24` | title | 否 |
| MLOps 任务详情 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`MLOps 任务详情`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-mlops-task-detail`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/mlops`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`MLOps 任务详情。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`MLOps 任务详情，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“MLOps 编排”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：MLOps 任务详情。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：MLOps 编排。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/mlops。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：检测运营 / MLOps 编排。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：MLOps 任务详情，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 legend，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-mlops-task-detail/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-mlops-task-detail/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-mlops-task-detail/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-mlops-task-detail/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-mlops-task-detail/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-mlops-task-detail/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-mlops-task-detail/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-mlops-task-detail/verification.json`
