# modal-playbook-trigger.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：从告警触发剧本
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-playbook-trigger.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-playbook-trigger.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-playbook-trigger.json`
...
| Modal 容器 | `360,170,1200,740` | 3 | 从告警触发剧本 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 从告警触发剧本 | `80,54,360,24` | title | 是 |
| modal-playbook-trigger | `510,54,360,24` | subtitle | 是 |
| /alerts/:alertId | `940,54,360,24` | section | 是 |
| 从告警触发剧本。 | `1370,54,360,24` | label | 是 |
| 从告警触发剧本，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。 | `80,96,360,24` | metric | 是 |
...
| 从告警触发剧本 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 从告警触发剧本 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`从告警触发剧本`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-playbook-trigger`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/alerts/:alertId`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`从告警触发剧本。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`从告警触发剧本，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：从告警触发剧本。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：从告警触发剧本，必须体现权限、影响范围、状态解释、下一步动作和审计留痕。`，类型 legend，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-playbook-trigger/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-playbook-trigger/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-playbook-trigger/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-playbook-trigger/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-playbook-trigger/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-playbook-trigger/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-playbook-trigger/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-playbook-trigger/verification.json`
