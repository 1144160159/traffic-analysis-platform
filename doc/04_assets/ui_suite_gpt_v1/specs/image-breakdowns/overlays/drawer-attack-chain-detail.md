# drawer-attack-chain-detail.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：攻击链详情抽屉
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-attack-chain-detail.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-attack-chain-detail.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-attack-chain-detail.json`
- 对应路由/宿主路由：`/attack-chains`
...
| 抽屉容器 | `980,48,900,984` | 3 | 攻击链详情抽屉 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 攻击链详情抽屉 | `80,54,360,24` | title | 是 |
| drawer-attack-chain-detail | `510,54,360,24` | subtitle | 是 |
| /attack-chains | `940,54,360,24` | section | 是 |
| 攻击链详情抽屉。 | `1370,54,360,24` | label | 是 |
| 攻击链节点详情、证据、命中规则、关联资产和下一步调查 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“攻击链分析”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：攻击链详情抽屉。 | `1370,264,360,24` | label | 是 |
| 基准页面：攻击链分析。 | `80,306,360,24` | metric | 是 |
| 基准路由：/attack-chains。 | `510,306,360,24` | button | 是 |
| 业务模块：威胁分析 / 攻击链分析。 | `940,306,360,24` | status | 是 |
| 浮层重点：攻击链节点详情、证据、命中规则、关联资产和下一步调查 | `1370,306,360,24` | legend | 是 |
| 浮层布局：右侧 Drawer 覆盖攻击链画布，节点高亮不变。 | `80,348,360,24` | hint | 是 |
...
| 攻击链详情抽屉 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 攻击链详情抽屉 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`攻击链详情抽屉`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-attack-chain-detail`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/attack-chains`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`攻击链详情抽屉。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`攻击链节点详情、证据、命中规则、关联资产和下一步调查`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“攻击链分析”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：攻击链详情抽屉。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：攻击链分析。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/attack-chains。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：威胁分析 / 攻击链分析。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：攻击链节点详情、证据、命中规则、关联资产和下一步调查`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：右侧 Drawer 覆盖攻击链画布，节点高亮不变。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-attack-chain-detail/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-attack-chain-detail/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-attack-chain-detail/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-attack-chain-detail/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-attack-chain-detail/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-attack-chain-detail/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-attack-chain-detail/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-attack-chain-detail/verification.json`
