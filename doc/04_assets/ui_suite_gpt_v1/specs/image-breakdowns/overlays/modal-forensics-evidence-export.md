# modal-forensics-evidence-export.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：取证证据导出
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-forensics-evidence-export.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-forensics-evidence-export.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-forensics-evidence-export.json`
- 对应路由/宿主路由：`/forensics`
...
| Modal 容器 | `360,170,1200,740` | 3 | 取证证据导出 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 取证证据导出 | `80,54,360,24` | title | 是 |
| modal-forensics-evidence-export | `510,54,360,24` | subtitle | 是 |
| /forensics | `940,54,360,24` | section | 是 |
| 取证证据导出。 | `1370,54,360,24` | label | 是 |
| 导出 PCAP、CSV、日志、图谱路径和取证材料，体现 hash 校验、签名 URL、有效期和下载审计 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“取证分析”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：取证证据导出。 | `1370,264,360,24` | label | 是 |
| 基准页面：取证分析。 | `80,306,360,24` | metric | 是 |
| 基准路由：/forensics。 | `510,306,360,24` | button | 是 |
| 业务模块：威胁分析 / 取证分析。 | `940,306,360,24` | status | 是 |
| 浮层重点：导出 PCAP、CSV、日志、图谱路径和取证材料，体现 hash 校验、签名 URL、有效期和下载审计 | `1370,306,360,24` | legend | 是 |
| 浮层布局：居中宽 Modal，只展示取证导出容器；包含证据类型选择、时间窗、对象路径、hash 校验、签名链接、脱敏级别和权限确认。 | `80,348,360,24` | hint | 是 |
...
| 取证证据导出 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 取证证据导出 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`取证证据导出`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-forensics-evidence-export`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/forensics`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`取证证据导出。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`导出 PCAP、CSV、日志、图谱路径和取证材料，体现 hash 校验、签名 URL、有效期和下载审计`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：告警中心、战役列表、攻击链分析、加密流量、取证分析，当前高亮“取证分析”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：取证证据导出。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：取证分析。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/forensics。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：威胁分析 / 取证分析。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：导出 PCAP、CSV、日志、图谱路径和取证材料，体现 hash 校验、签名 URL、有效期和下载审计`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：居中宽 Modal，只展示取证导出容器；包含证据类型选择、时间窗、对象路径、hash 校验、签名链接、脱敏级别和权限确认。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-forensics-evidence-export/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-forensics-evidence-export/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-forensics-evidence-export/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-forensics-evidence-export/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-forensics-evidence-export/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-forensics-evidence-export/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-forensics-evidence-export/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-forensics-evidence-export/verification.json`
