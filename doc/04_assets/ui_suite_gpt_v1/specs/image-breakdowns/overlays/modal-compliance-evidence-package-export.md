# modal-compliance-evidence-package-export.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：合规证据包导出
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-compliance-evidence-package-export.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-compliance-evidence-package-export.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-compliance-evidence-package-export.json`
- 对应路由/宿主路由：`/compliance`
...
| Modal 容器 | `360,170,1200,740` | 3 | 合规证据包导出 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 合规证据包导出 | `80,54,360,24` | title | 是 |
| modal-compliance-evidence-package-export | `510,54,360,24` | subtitle | 是 |
| /compliance | `940,54,360,24` | section | 是 |
| 合规证据包导出。 | `1370,54,360,24` | label | 是 |
| 导出合规证据包，选择门禁项、证据类型、时间窗、脱敏级别、hash 校验和审批门禁 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“合规审计”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：合规证据包导出。 | `1370,264,360,24` | label | 是 |
| 基准页面：合规审计。 | `80,306,360,24` | metric | 是 |
| 基准路由：/compliance。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 合规审计。 | `940,306,360,24` | status | 是 |
| 浮层重点：导出合规证据包，选择门禁项、证据类型、时间窗、脱敏级别、hash 校验和审批门禁 | `1370,306,360,24` | legend | 是 |
| 浮层布局：居中宽 Modal，只展示合规证据包导出容器；包含证据清单、完整性校验、导出格式、签名 URL、有效期和审计提示。 | `80,348,360,24` | hint | 是 |
...
| 合规证据包导出 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 合规证据包导出 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`合规证据包导出`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-compliance-evidence-package-export`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/compliance`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`合规证据包导出。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`导出合规证据包，选择门禁项、证据类型、时间窗、脱敏级别、hash 校验和审批门禁`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“合规审计”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：合规证据包导出。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：合规审计。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/compliance。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：审计配置 / 合规审计。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：导出合规证据包，选择门禁项、证据类型、时间窗、脱敏级别、hash 校验和审批门禁`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：居中宽 Modal，只展示合规证据包导出容器；包含证据清单、完整性校验、导出格式、签名 URL、有效期和审计提示。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-compliance-evidence-package-export/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-compliance-evidence-package-export/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-compliance-evidence-package-export/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-compliance-evidence-package-export/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-compliance-evidence-package-export/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-compliance-evidence-package-export/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-compliance-evidence-package-export/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-compliance-evidence-package-export/verification.json`
