# drawer-campaign-detail.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：战役详情抽屉
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-campaign-detail.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-campaign-detail.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-campaign-detail.json`
- 对应路由/宿主路由：`/campaigns`
...
| 抽屉容器 | `980,48,900,984` | 3 | 战役详情抽屉 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 战役详情抽屉 | `80,54,360,24` | title | 是 |
| drawer-campaign-detail | `510,54,360,24` | subtitle | 是 |
| /campaigns | `940,54,360,24` | section | 是 |
| 战役详情抽屉。 | `1370,54,360,24` | label | 是 |
| 从列表右侧滑出战役详情，展示聚类原因、阶段、证据和处置建议 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：战役详情抽屉。 | `1370,264,360,24` | label | 是 |
| 基准页面：战役列表。 | `80,306,360,24` | metric | 是 |
| 基准路由：/campaigns。 | `510,306,360,24` | button | 是 |
| 业务模块：威胁分析 / 战役列表。 | `940,306,360,24` | status | 是 |
| 浮层重点：从列表右侧滑出战役详情，展示聚类原因、阶段、证据和处置建议 | `1370,306,360,24` | legend | 是 |
| 浮层布局：右侧 Drawer 宽约 38%，底层战役列表保持可见。 | `80,348,360,24` | hint | 是 |
...
| 战役详情抽屉 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 战役详情抽屉 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`战役详情抽屉`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-campaign-detail`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/campaigns`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`战役详情抽屉。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`从列表右侧滑出战役详情，展示聚类原因、阶段、证据和处置建议`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：战役详情抽屉。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：战役列表。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/campaigns。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：威胁分析 / 战役列表。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：从列表右侧滑出战役详情，展示聚类原因、阶段、证据和处置建议`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：右侧 Drawer 宽约 38%，底层战役列表保持可见。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-campaign-detail/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-campaign-detail/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-campaign-detail/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-campaign-detail/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-campaign-detail/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-campaign-detail/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-campaign-detail/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-campaign-detail/verification.json`
