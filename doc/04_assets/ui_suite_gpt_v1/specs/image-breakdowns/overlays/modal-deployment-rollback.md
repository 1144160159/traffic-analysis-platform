# modal-deployment-rollback.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：回滚部署确认
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-deployment-rollback.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-deployment-rollback.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-deployment-rollback.json`
- 对应路由/宿主路由：`/deployments`
...
| Modal 容器 | `360,170,1200,740` | 3 | 回滚部署确认 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 回滚部署确认 | `80,54,360,24` | title | 是 |
| modal-deployment-rollback | `510,54,360,24` | subtitle | 是 |
| /deployments | `940,54,360,24` | section | 是 |
| 回滚部署确认。 | `1370,54,360,24` | label | 是 |
| 回滚确认，展示影响范围、最近版本、风险和确认输入 | `80,96,360,24` | metric | 是 |
...
| 左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“部署管理”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：回滚部署确认。 | `1370,264,360,24` | label | 是 |
| 基准页面：部署管理。 | `80,306,360,24` | metric | 是 |
| 基准路由：/deployments。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / 部署管理。 | `940,306,360,24` | status | 是 |
| 浮层重点：回滚确认，展示影响范围、最近版本、风险和确认输入 | `1370,306,360,24` | legend | 是 |
| 浮层布局：危险操作确认 Modal，红色风险提示明确但不夸张。 | `80,348,360,24` | hint | 是 |
...
| 回滚部署确认 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 回滚部署确认 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`回滚部署确认`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-deployment-rollback`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/deployments`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`回滚部署确认。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`回滚确认，展示影响范围、最近版本、风险和确认输入`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：规则管理、部署管理、模型管理、MLOps 编排、SOAR 剧本、白名单，当前高亮“部署管理”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：回滚部署确认。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：部署管理。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/deployments。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：检测运营 / 部署管理。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：回滚确认，展示影响范围、最近版本、风险和确认输入`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：危险操作确认 Modal，红色风险提示明确但不夸张。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-deployment-rollback/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-deployment-rollback/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-deployment-rollback/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-deployment-rollback/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-deployment-rollback/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-deployment-rollback/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-deployment-rollback/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-deployment-rollback/verification.json`
