# popconfirm-settings-token-revoke.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：API 令牌吊销确认
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/popconfirm-settings-token-revoke.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/popconfirm-settings-token-revoke.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/popconfirm-settings-token-revoke.json`
- 对应路由/宿主路由：`/settings`
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| API 令牌吊销确认 | `80,54,360,24` | title | 是 |
| popconfirm-settings-token-revoke | `510,54,360,24` | subtitle | 是 |
| /settings | `940,54,360,24` | section | 是 |
| API 令牌吊销确认。 | `1370,54,360,24` | label | 是 |
| 撤销 API 令牌确认，展示调用方、最近使用时间和影响范围 | `80,96,360,24` | metric | 是 |
...
| 必须实现为 Popconfirm 或等价语义组件 | `510,138,360,24` | acceptance | 是 |
...
| 浮层名称：API 令牌吊销确认。 | `1370,264,360,24` | label | 是 |
| 基准页面：系统设置。 | `80,306,360,24` | metric | 是 |
| 基准路由：/settings。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 系统设置。 | `940,306,360,24` | status | 是 |
| 浮层重点：撤销 API 令牌确认，展示调用方、最近使用时间和影响范围 | `1370,306,360,24` | legend | 是 |
| 浮层布局：小型危险 Popconfirm，锚定令牌列表行按钮。 | `80,348,360,24` | hint | 是 |
...
| API 令牌吊销确认 视觉校正项 41 | `80,474,360,24` | title | 否 |
| API 令牌吊销确认 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`API 令牌吊销确认`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`popconfirm-settings-token-revoke`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/settings`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`API 令牌吊销确认。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`撤销 API 令牌确认，展示调用方、最近使用时间和影响范围`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 10：`必须实现为 Popconfirm 或等价语义组件`，类型 acceptance，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：API 令牌吊销确认。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：撤销 API 令牌确认，展示调用方、最近使用时间和影响范围`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：小型危险 Popconfirm，锚定令牌列表行按钮。`，类型 hint，来源 manual visual ledger。
...
| 区域 | 组件/元素 | 实现方式 | 状态 | 备注 |
|---|---|---|---|---|
| `canvas` | `Popconfirm` | web/ui/src/components/Popconfirm.tsx | default | Popconfirm must keep stable dimensions and match the recorded bbox/token mapping. |
...
- 目标图：`evidence/ui-image-breakdowns/overlays/popconfirm-settings-token-revoke/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/popconfirm-settings-token-revoke/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/popconfirm-settings-token-revoke/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/popconfirm-settings-token-revoke/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/popconfirm-settings-token-revoke/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/popconfirm-settings-token-revoke/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/popconfirm-settings-token-revoke/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/popconfirm-settings-token-revoke/verification.json`
