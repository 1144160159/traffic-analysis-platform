# modal-deployment-create.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：新建部署
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-deployment-create.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-deployment-create.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-deployment-create.json`
...
| Modal 容器 | `360,170,1200,740` | 3 | 新建部署 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 新建部署 | `80,54,360,24` | title | 是 |
| modal-deployment-create | `510,54,360,24` | subtitle | 是 |
| /deployments | `940,54,360,24` | section | 是 |
| 新建部署。 | `1370,54,360,24` | label | 是 |
| 新建部署计划，选择对象、环境、窗口、回滚策略和审批人 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：新建部署。 | `1370,264,360,24` | label | 是 |
| 基准页面：部署管理。 | `80,306,360,24` | metric | 是 |
| 基准路由：/deployments。 | `510,306,360,24` | button | 是 |
| 业务模块：检测运营 / 部署管理。 | `940,306,360,24` | status | 是 |
| 浮层重点：新建部署计划，选择对象、环境、窗口、回滚策略和审批人 | `1370,306,360,24` | legend | 是 |
| 浮层布局：分步骤 Modal，底层部署流水线压暗。 | `80,348,360,24` | hint | 是 |
...
| 新建部署 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 新建部署 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`新建部署`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-deployment-create`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/deployments`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`新建部署。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`新建部署计划，选择对象、环境、窗口、回滚策略和审批人`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：新建部署。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：新建部署计划，选择对象、环境、窗口、回滚策略和审批人`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：分步骤 Modal，底层部署流水线压暗。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-deployment-create/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-deployment-create/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-deployment-create/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-deployment-create/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-deployment-create/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-deployment-create/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-deployment-create/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-deployment-create/verification.json`
