# drawer-notification-silence-rule.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：通知静默规则
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-notification-silence-rule.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-notification-silence-rule.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-notification-silence-rule.json`
...
| 抽屉容器 | `980,48,900,984` | 3 | 通知静默规则 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 通知静默规则 | `80,54,360,24` | title | 是 |
| drawer-notification-silence-rule | `510,54,360,24` | subtitle | 是 |
| /notifications | `940,54,360,24` | section | 是 |
| 通知静默规则。 | `1370,54,360,24` | label | 是 |
| 抑制、静默和维护窗口规则，配置作用对象、条件、时间窗、例外项和冲突提示 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：通知静默规则。 | `1370,264,360,24` | label | 是 |
| 基准页面：通知配置。 | `80,306,360,24` | metric | 是 |
| 基准路由：/notifications。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 通知配置。 | `940,306,360,24` | status | 是 |
| 浮层重点：抑制、静默和维护窗口规则，配置作用对象、条件、时间窗、例外项和冲突提示 | `1370,306,360,24` | legend | 是 |
| 浮层布局：右侧宽 Drawer，只展示通知静默规则容器；包含规则条件、维护窗口、受影响通知、冲突检测、审批门禁和生效审计。 | `80,348,360,24` | hint | 是 |
...
| 通知静默规则 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 通知静默规则 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`通知静默规则`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-notification-silence-rule`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/notifications`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`通知静默规则。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`抑制、静默和维护窗口规则，配置作用对象、条件、时间窗、例外项和冲突提示`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：通知静默规则。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：抑制、静默和维护窗口规则，配置作用对象、条件、时间窗、例外项和冲突提示`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：右侧宽 Drawer，只展示通知静默规则容器；包含规则条件、维护窗口、受影响通知、冲突检测、审批门禁和生效审计。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-notification-silence-rule/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-notification-silence-rule/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-notification-silence-rule/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-notification-silence-rule/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-notification-silence-rule/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-notification-silence-rule/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-notification-silence-rule/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-notification-silence-rule/verification.json`
