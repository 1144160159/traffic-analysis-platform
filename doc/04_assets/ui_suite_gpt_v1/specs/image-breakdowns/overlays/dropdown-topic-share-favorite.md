# dropdown-topic-share-favorite.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：专题分享收藏菜单
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/dropdown-topic-share-favorite.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/dropdown-topic-share-favorite.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/dropdown-topic-share-favorite.json`
- 对应路由/宿主路由：`/topics`
...
| 下拉菜单 | `1120,164,560,500` | 3 | 专题分享收藏菜单 | 容器阴影、描边和圆角一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 专题分享收藏菜单 | `80,54,360,24` | title | 是 |
| dropdown-topic-share-favorite | `510,54,360,24` | subtitle | 是 |
| /topics | `940,54,360,24` | section | 是 |
| 专题分享收藏菜单。 | `1370,54,360,24` | label | 是 |
| 专题分享、收藏、复制链接、生成只读链接和权限边界提示 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：专题分享收藏菜单。 | `1370,264,360,24` | label | 是 |
| 基准页面：专题面板。 | `80,306,360,24` | metric | 是 |
| 基准路由：/topics。 | `510,306,360,24` | button | 是 |
| 业务模块：综合态势 / 专题面板。 | `940,306,360,24` | status | 是 |
| 浮层重点：专题分享、收藏、复制链接、生成只读链接和权限边界提示 | `1370,306,360,24` | legend | 是 |
| 浮层布局：紧凑 Dropdown，只展示从专题工具按钮展开的菜单容器；包含收藏、复制链接、分享给角色、生成只读链接和审计提示。 | `80,348,360,24` | hint | 是 |
...
| 专题分享收藏菜单 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 专题分享收藏菜单 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`专题分享收藏菜单`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`dropdown-topic-share-favorite`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/topics`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`专题分享收藏菜单。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`专题分享、收藏、复制链接、生成只读链接和权限边界提示`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：专题分享收藏菜单。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：专题分享、收藏、复制链接、生成只读链接和权限边界提示`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：紧凑 Dropdown，只展示从专题工具按钮展开的菜单容器；包含收藏、复制链接、分享给角色、生成只读链接和审计提示。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/dropdown-topic-share-favorite/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/dropdown-topic-share-favorite/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/dropdown-topic-share-favorite/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/dropdown-topic-share-favorite/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/dropdown-topic-share-favorite/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/dropdown-topic-share-favorite/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/dropdown-topic-share-favorite/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/dropdown-topic-share-favorite/verification.json`
