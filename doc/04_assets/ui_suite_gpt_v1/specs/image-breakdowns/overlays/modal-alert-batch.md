# modal-alert-batch.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：告警批量操作确认
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-alert-batch.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-alert-batch.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-alert-batch.json`
- 对应路由/宿主路由：`/alerts`
...
| Modal 容器 | `360,170,1200,740` | 3 | 告警批量操作确认 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 告警批量操作确认 | `80,54,360,24` | title | 是 |
| modal-alert-batch | `510,54,360,24` | subtitle | 是 |
| /alerts | `940,54,360,24` | section | 是 |
| 告警批量操作确认。 | `1370,54,360,24` | label | 是 |
| 批量确认告警、批量分派、批量忽略前的影响提示 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：告警批量操作确认。 | `1370,264,360,24` | label | 是 |
| 基准页面：告警中心。 | `80,306,360,24` | metric | 是 |
| 基准路由：/alerts。 | `510,306,360,24` | button | 是 |
| 业务模块：威胁分析 / 告警中心。 | `940,306,360,24` | status | 是 |
| 浮层重点：批量确认告警、批量分派、批量忽略前的影响提示 | `1370,306,360,24` | legend | 是 |
| 浮层布局：在告警中心页面上方显示居中 Modal，背景轻微压暗，底部有取消和确认按钮。 | `80,348,360,24` | hint | 是 |
...
| 告警批量操作确认 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 告警批量操作确认 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`告警批量操作确认`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-alert-batch`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/alerts`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`告警批量操作确认。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`批量确认告警、批量分派、批量忽略前的影响提示`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：告警批量操作确认。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：批量确认告警、批量分派、批量忽略前的影响提示`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：在告警中心页面上方显示居中 Modal，背景轻微压暗，底部有取消和确认按钮。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-alert-batch/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-alert-batch/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-alert-batch/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-alert-batch/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-alert-batch/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-alert-batch/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-alert-batch/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-alert-batch/verification.json`
