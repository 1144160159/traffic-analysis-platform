# dropdown-alert-row-actions.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：告警行操作下拉
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/dropdown-alert-row-actions.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/dropdown-alert-row-actions.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/dropdown-alert-row-actions.json`
- 对应路由/宿主路由：`/alerts`
...
| 下拉菜单 | `1120,164,560,500` | 3 | 告警行操作下拉 | 容器阴影、描边和圆角一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 告警行操作下拉 | `80,54,360,24` | title | 是 |
| dropdown-alert-row-actions | `510,54,360,24` | subtitle | 是 |
| /alerts | `940,54,360,24` | section | 是 |
| 告警行操作下拉。 | `1370,54,360,24` | label | 是 |
| 单条告警行操作，包含查看详情、生成 PCAP、创建工单、加入战役 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：告警行操作下拉。 | `1370,264,360,24` | label | 是 |
| 基准页面：告警中心。 | `80,306,360,24` | metric | 是 |
| 基准路由：/alerts。 | `510,306,360,24` | button | 是 |
| 业务模块：威胁分析 / 告警中心。 | `940,306,360,24` | status | 是 |
| 浮层重点：单条告警行操作，包含查看详情、生成 PCAP、创建工单、加入战役 | `1370,306,360,24` | legend | 是 |
| 浮层布局：在告警表格行尾展开小型菜单，当前行高亮。 | `80,348,360,24` | hint | 是 |
...
| 告警行操作下拉 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 告警行操作下拉 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`告警行操作下拉`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`dropdown-alert-row-actions`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/alerts`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`告警行操作下拉。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`单条告警行操作，包含查看详情、生成 PCAP、创建工单、加入战役`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：告警行操作下拉。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：单条告警行操作，包含查看详情、生成 PCAP、创建工单、加入战役`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：在告警表格行尾展开小型菜单，当前行高亮。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/dropdown-alert-row-actions/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/dropdown-alert-row-actions/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/dropdown-alert-row-actions/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/dropdown-alert-row-actions/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/dropdown-alert-row-actions/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/dropdown-alert-row-actions/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/dropdown-alert-row-actions/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/dropdown-alert-row-actions/verification.json`
