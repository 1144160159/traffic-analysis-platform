# dropdown-user-menu.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：用户下拉菜单
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/dropdown-user-menu.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/dropdown-user-menu.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/dropdown-user-menu.json`
- 对应路由/宿主路由：`/settings`
...
| 下拉菜单 | `1120,164,560,500` | 3 | 用户下拉菜单 | 容器阴影、描边和圆角一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 用户下拉菜单 | `80,54,360,24` | title | 是 |
| dropdown-user-menu | `510,54,360,24` | subtitle | 是 |
| /settings | `940,54,360,24` | section | 是 |
| 用户下拉菜单。 | `1370,54,360,24` | label | 是 |
| 用户菜单，包含个人信息、切换角色、安全设置、退出登录 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：用户下拉菜单。 | `1370,264,360,24` | label | 是 |
| 基准页面：系统设置。 | `80,306,360,24` | metric | 是 |
| 基准路由：/settings。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 系统设置。 | `940,306,360,24` | status | 是 |
| 浮层重点：用户菜单，包含个人信息、切换角色、安全设置、退出登录 | `1370,306,360,24` | legend | 是 |
| 浮层布局：从左侧底部用户卡触发的用户菜单浮层本体，向右展开到业务内容区；不绘制顶部头像入口、通知用户组或完整公共 AppShell。 | `80,348,360,24` | hint | 是 |
...
| 用户下拉菜单 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 用户下拉菜单 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`用户下拉菜单`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`dropdown-user-menu`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/settings`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`用户下拉菜单。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`用户菜单，包含个人信息、切换角色、安全设置、退出登录`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：用户下拉菜单。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：用户菜单，包含个人信息、切换角色、安全设置、退出登录`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：从左侧底部用户卡触发的用户菜单浮层本体，向右展开到业务内容区；不绘制顶部头像入口、通知用户组或完整公共 AppShell。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/dropdown-user-menu/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/dropdown-user-menu/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/dropdown-user-menu/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/dropdown-user-menu/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/dropdown-user-menu/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/dropdown-user-menu/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/dropdown-user-menu/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/dropdown-user-menu/verification.json`
