# modal-forensics-task.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：取证任务详情
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-forensics-task.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-forensics-task.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-forensics-task.json`
- 对应路由/宿主路由：`/forensics`
...
| Modal 容器 | `360,170,1200,740` | 3 | 取证任务详情 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 取证任务详情 | `80,54,360,24` | title | 是 |
| modal-forensics-task | `510,54,360,24` | subtitle | 是 |
| /forensics | `940,54,360,24` | section | 是 |
| 取证任务详情。 | `1370,54,360,24` | label | 是 |
| 取证任务详情、切片范围、存储位置、处理状态、失败重试 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：取证任务详情。 | `1370,264,360,24` | label | 是 |
| 基准页面：取证分析。 | `80,306,360,24` | metric | 是 |
| 基准路由：/forensics。 | `510,306,360,24` | button | 是 |
| 业务模块：威胁分析 / 取证分析。 | `940,306,360,24` | status | 是 |
| 浮层重点：取证任务详情、切片范围、存储位置、处理状态、失败重试 | `1370,306,360,24` | legend | 是 |
| 浮层布局：任务详情 Modal 覆盖取证工作台，突出进度和输出文件。 | `80,348,360,24` | hint | 是 |
...
| 取证任务详情 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 取证任务详情 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`取证任务详情`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-forensics-task`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/forensics`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`取证任务详情。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`取证任务详情、切片范围、存储位置、处理状态、失败重试`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：取证任务详情。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：取证任务详情、切片范围、存储位置、处理状态、失败重试`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：任务详情 Modal 覆盖取证工作台，突出进度和输出文件。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-forensics-task/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-forensics-task/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-forensics-task/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-forensics-task/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-forensics-task/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-forensics-task/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-forensics-task/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-forensics-task/verification.json`
