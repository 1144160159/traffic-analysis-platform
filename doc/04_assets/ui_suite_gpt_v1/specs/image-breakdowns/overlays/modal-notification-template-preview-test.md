# modal-notification-template-preview-test.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：通知模板预览测试
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-notification-template-preview-test.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-notification-template-preview-test.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-notification-template-preview-test.json`
- 对应路由/宿主路由：`/notifications`
...
| 区域 | bbox | 层级 | 说明 | 复刻要点 |
|---|---:|---:|---|---|
| 画布 | `0,0,1920,1080` | 0 | 居中 Modal 状态图 | 单屏完整展示 |
| 宿主页面上下文 | `0,0,1920,1080` | 1 | 被遮罩的业务页面 | 只作为上下文 |
| 全屏遮罩 | `0,0,1920,1080` | 2 | 压暗背景 | 透明度不遮蔽浮层文字 |
| Modal 容器 | `360,170,1200,740` | 3 | 通知模板预览测试 | 居中、圆角、描边一致 |
| Modal 标题栏 | `400,204,1120,70` | 4 | 标题、状态、关闭按钮 | 右上关闭按钮 |
| 摘要区 | `400,286,1120,100` | 4 | 关键对象和影响范围 | 摘要不挤压正文 |
| 表单/详情区 | `400,400,540,350` | 4 | 输入、选择、条件或详情字段 | 字段标签对齐 |
| 预览/证据区 | `960,400,560,350` | 4 | 表格、日志、证据或策略预览 | 高密度列表不越界 |
| 权限与审计提示 | `400,766,1120,72` | 4 | 权限、影响范围、审计 trace | 危险提示使用 danger token |
| 底部操作区 | `960,858,560,52` | 4 | 取消、确认、提交 | 右对齐，主次动作明确 |
| 焦点边界 | `398,398,544,354` | 5 | 当前焦点表单区 | focus 态不改变布局尺寸 |
| 关闭热区 | `1480,210,40,40` | 5 | 关闭按钮热区 | 热区不小于图标本体 |
...
- 区域 `host-context`：位置 `0,0,1920,1080`。
  ...
  组件：HostPage
  视觉要求：只作为上下文
- 区域 `modal-mask`：位置 `0,0,1920,1080`。
  用途：压暗背景
  组件：Mask
  视觉要求：透明度不遮蔽浮层文字
- 区域 `modal-surface`：位置 `360,170,1200,740`。
  用途：通知模板预览测试
  组件：Modal / WorkPanel
  视觉要求：居中、圆角、描边一致
- 区域 `modal-header`：位置 `400,204,1120,70`。
  用途：标题、状态、关闭按钮
  组件：ModalHeader / IconButton
  视觉要求：右上关闭按钮
- 区域 `modal-summary`：位置 `400,286,1120,100`。
  用途：关键对象和影响范围
  组件：DescriptionList / StatusTag
  视觉要求：摘要不挤压正文
- 区域 `modal-form`：位置 `400,400,540,350`。
  用途：输入、选择、条件或详情字段
  组件：Form / Input / Select / ConditionBuilder
  视觉要求：字段标签对齐
- 区域 `modal-preview`：位置 `960,400,560,350`。
  用途：表格、日志、证据或策略预览
  组件：DataTable / LogList / EvidenceFileCard
  视觉要求：高密度列表不越界
- 区域 `modal-alert`：位置 `400,766,1120,72`。
  用途：权限、影响范围、审计 trace
  组件：Alert / AuditTrail
  视觉要求：危险提示使用 danger token
- 区域 `modal-footer`：位置 `960,858,560,52`。
  用途：取消、确认、提交
  组件：Button
  视觉要求：右对齐，主次动作明确
- 区域 `focus-ring`：位置 `398,398,544,354`。
  用途：当前焦点表单区
  组件：FocusRing
  视觉要求：focus 态不改变布局尺寸
- 区域 `close-hit-area`：位置 `1480,210,40,40`。
  用途：关闭按钮热区
  组件：IconButton
  视觉要求：热区不小于图标本体
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 通知模板预览测试 | `80,54,360,24` | title | 是 |
| modal-notification-template-preview-test | `510,54,360,24` | subtitle | 是 |
| /notifications | `940,54,360,24` | section | 是 |
| 通知模板预览测试。 | `1370,54,360,24` | label | 是 |
| 通知模板预览和测试发送，展示变量映射、样例消息、目标渠道、测试结果和审计提示 | `80,96,360,24` | metric | 是 |
...
| 必须实现为 Modal 或等价语义组件 | `510,138,360,24` | acceptance | 是 |
...
| 左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“通知配置”。 | `940,222,360,24` | hint | 是 |
...
| 浮层名称：通知模板预览测试。 | `1370,264,360,24` | label | 是 |
| 基准页面：通知配置。 | `80,306,360,24` | metric | 是 |
| 基准路由：/notifications。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 通知配置。 | `940,306,360,24` | status | 是 |
| 浮层重点：通知模板预览和测试发送，展示变量映射、样例消息、目标渠道、测试结果和审计提示 | `1370,306,360,24` | legend | 是 |
| 浮层布局：居中宽 Modal，只展示模板预览与测试发送容器；左侧为模板变量和样例数据，右侧为预览消息、测试目标、发送结果和失败原因。 | `80,348,360,24` | hint | 是 |
...
| 通知模板预览测试 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 通知模板预览测试 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`通知模板预览测试`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-notification-template-preview-test`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/notifications`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`通知模板预览测试。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`通知模板预览和测试发送，展示变量映射、样例消息、目标渠道、测试结果和审计提示`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 10：`必须实现为 Modal 或等价语义组件`，类型 acceptance，来源 prompt/layer plus target visual ledger。
...
- 文本 19：`左侧二级菜单显示：合规审计、审计日志、通知配置、系统设置，当前高亮“通知配置”。`，类型 hint，来源 manual visual ledger。
...
- 文本 24：`浮层名称：通知模板预览测试。`，类型 label，来源 manual visual ledger。
- 文本 25：`基准页面：通知配置。`，类型 metric，来源 manual visual ledger。
- 文本 26：`基准路由：/notifications。`，类型 button，来源 manual visual ledger。
- 文本 27：`业务模块：审计配置 / 通知配置。`，类型 status，来源 manual visual ledger。
- 文本 28：`浮层重点：通知模板预览和测试发送，展示变量映射、样例消息、目标渠道、测试结果和审计提示`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：居中宽 Modal，只展示模板预览与测试发送容器；左侧为模板变量和样例数据，右侧为预览消息、测试目标、发送结果和失败原因。`，类型 hint，来源 manual visual ledger。
...
| 区域 | 组件/元素 | 实现方式 | 状态 | 备注 |
|---|---|---|---|---|
| `canvas` | `Modal` | web/ui/src/components/Modal.tsx | default | Modal must keep stable dimensions and match the recorded bbox/token mapping. |
...
| `state-section` | `Drawer` | web/ui/src/components/Drawer.tsx | data-ready | Drawer must keep stable dimensions and match the recorded bbox/token mapping. |
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-notification-template-preview-test/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-notification-template-preview-test/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-notification-template-preview-test/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-notification-template-preview-test/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-notification-template-preview-test/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-notification-template-preview-test/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-notification-template-preview-test/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-notification-template-preview-test/verification.json`
