# drawer-audit-operation-detail.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：审计操作详情
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-audit-operation-detail.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-audit-operation-detail.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-audit-operation-detail.json`
- 对应路由/宿主路由：`/audit-log`
...
| 抽屉容器 | `980,48,900,984` | 3 | 审计操作详情 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 审计操作详情 | `80,54,360,24` | title | 是 |
| drawer-audit-operation-detail | `510,54,360,24` | subtitle | 是 |
| /audit-log | `940,54,360,24` | section | 是 |
| 审计操作详情。 | `1370,54,360,24` | label | 是 |
| 审计操作详情抽屉，承载字段 Diff、请求上下文、关联链路、Trace ID 和取证动作 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：审计操作详情。 | `1370,264,360,24` | label | 是 |
| 基准页面：审计日志。 | `80,306,360,24` | metric | 是 |
| 基准路由：/audit-log。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 审计日志。 | `940,306,360,24` | status | 是 |
| 浮层重点：审计操作详情抽屉，承载字段 Diff、请求上下文、关联链路、Trace ID 和取证动作 | `1370,306,360,24` | legend | 是 |
| 浮层布局：右侧宽 Drawer，只展示审计操作详情容器；包含操作摘要、字段变更 Diff、请求上下文、关联告警/资产/证据链和导出审计材料动作。 | `80,348,360,24` | hint | 是 |
...
| 审计操作详情 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 审计操作详情 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`审计操作详情`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-audit-operation-detail`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/audit-log`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`审计操作详情。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`审计操作详情抽屉，承载字段 Diff、请求上下文、关联链路、Trace ID 和取证动作`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：审计操作详情。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：审计操作详情抽屉，承载字段 Diff、请求上下文、关联链路、Trace ID 和取证动作`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：右侧宽 Drawer，只展示审计操作详情容器；包含操作摘要、字段变更 Diff、请求上下文、关联告警/资产/证据链和导出审计材料动作。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-audit-operation-detail/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-audit-operation-detail/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-audit-operation-detail/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-audit-operation-detail/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-audit-operation-detail/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-audit-operation-detail/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-audit-operation-detail/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-audit-operation-detail/verification.json`
