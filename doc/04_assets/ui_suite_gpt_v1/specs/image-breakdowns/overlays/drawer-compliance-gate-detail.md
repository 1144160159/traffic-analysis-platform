# drawer-compliance-gate-detail.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：合规门禁详情
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/drawer-compliance-gate-detail.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/drawer-compliance-gate-detail.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/drawer-compliance-gate-detail.json`
...
| 抽屉容器 | `980,48,900,984` | 3 | 合规门禁详情 | 右侧固定宽度，圆角与描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 合规门禁详情 | `80,54,360,24` | title | 是 |
| drawer-compliance-gate-detail | `510,54,360,24` | subtitle | 是 |
| /compliance | `940,54,360,24` | section | 是 |
| 合规门禁详情。 | `1370,54,360,24` | label | 是 |
| 验收门禁未通过项下钻，展示门禁规则、失败原因、影响范围、证据缺口和整改动作 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：合规门禁详情。 | `1370,264,360,24` | label | 是 |
| 基准页面：合规审计。 | `80,306,360,24` | metric | 是 |
| 基准路由：/compliance。 | `510,306,360,24` | button | 是 |
| 业务模块：审计配置 / 合规审计。 | `940,306,360,24` | status | 是 |
| 浮层重点：验收门禁未通过项下钻，展示门禁规则、失败原因、影响范围、证据缺口和整改动作 | `1370,306,360,24` | legend | 是 |
| 浮层布局：右侧宽 Drawer，只展示合规门禁详情容器；包含门禁摘要、失败项矩阵、证据缺口、责任域、整改建议、复验入口和审计编号。 | `80,348,360,24` | hint | 是 |
...
| 合规门禁详情 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 合规门禁详情 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`合规门禁详情`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`drawer-compliance-gate-detail`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/compliance`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`合规门禁详情。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`验收门禁未通过项下钻，展示门禁规则、失败原因、影响范围、证据缺口和整改动作`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：合规门禁详情。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：验收门禁未通过项下钻，展示门禁规则、失败原因、影响范围、证据缺口和整改动作`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：右侧宽 Drawer，只展示合规门禁详情容器；包含门禁摘要、失败项矩阵、证据缺口、责任域、整改建议、复验入口和审计编号。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/drawer-compliance-gate-detail/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/drawer-compliance-gate-detail/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/drawer-compliance-gate-detail/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/drawer-compliance-gate-detail/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/drawer-compliance-gate-detail/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/drawer-compliance-gate-detail/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/drawer-compliance-gate-detail/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/drawer-compliance-gate-detail/verification.json`
