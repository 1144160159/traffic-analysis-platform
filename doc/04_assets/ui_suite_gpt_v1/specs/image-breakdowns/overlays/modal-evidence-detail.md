# modal-evidence-detail.png 逐图精拆记录

## 基本信息

- 分类：overlays
- 标题：证据详情
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/overlays/modal-evidence-detail.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/modal-evidence-detail.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/modal-evidence-detail.json`
...
| Modal 容器 | `360,170,1200,740` | 3 | 证据详情 | 居中、圆角、描边一致 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 证据详情 | `80,54,360,24` | title | 是 |
| modal-evidence-detail | `510,54,360,24` | subtitle | 是 |
| /alerts/:alertId | `940,54,360,24` | section | 是 |
| 证据详情。 | `1370,54,360,24` | label | 是 |
| 查看单条 PCAP/会话/日志证据的元数据、十六进制摘要和下载动作 | `80,96,360,24` | metric | 是 |
...
| 浮层名称：证据详情。 | `1370,264,360,24` | label | 是 |
| 基准页面：告警详情。 | `80,306,360,24` | metric | 是 |
| 基准路由：/alerts/:alertId。 | `510,306,360,24` | button | 是 |
| 业务模块：威胁分析 / 告警中心。 | `940,306,360,24` | status | 是 |
| 浮层重点：查看单条 PCAP/会话/日志证据的元数据、十六进制摘要和下载动作 | `1370,306,360,24` | legend | 是 |
| 浮层布局：宽 Modal，左侧证据属性，右侧数据预览和链路完整性。 | `80,348,360,24` | hint | 是 |
...
| 证据详情 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 证据详情 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`证据详情`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`modal-evidence-detail`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`/alerts/:alertId`，类型 section，来源 prompt/layer plus target visual ledger。
- 文本 04：`证据详情。`，类型 label，来源 prompt/layer plus target visual ledger。
- 文本 05：`查看单条 PCAP/会话/日志证据的元数据、十六进制摘要和下载动作`，类型 metric，来源 prompt/layer plus target visual ledger。
...
- 文本 24：`浮层名称：证据详情。`，类型 label，来源 manual visual ledger。
...
- 文本 28：`浮层重点：查看单条 PCAP/会话/日志证据的元数据、十六进制摘要和下载动作`，类型 legend，来源 manual visual ledger。
- 文本 29：`浮层布局：宽 Modal，左侧证据属性，右侧数据预览和链路完整性。`，类型 hint，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/overlays/modal-evidence-detail/target.png`
- 实现截图：`evidence/ui-image-breakdowns/overlays/modal-evidence-detail/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/overlays/modal-evidence-detail/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/overlays/modal-evidence-detail/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/overlays/modal-evidence-detail/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/overlays/modal-evidence-detail/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/overlays/modal-evidence-detail/metrics.json`
- verification：`evidence/ui-image-breakdowns/overlays/modal-evidence-detail/verification.json`
