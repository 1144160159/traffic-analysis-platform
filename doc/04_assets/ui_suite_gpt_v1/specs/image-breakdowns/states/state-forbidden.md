# state-forbidden.png 逐图精拆记录

## 基本信息

- 分类：states
- 标题：无权限
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/states/state-forbidden.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/state-forbidden.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/state-forbidden.json`
- 对应路由/宿主路由：无直接业务路由
...
| 状态容器 | `280,160,1360,760` | 2 | 无权限 | 容器尺寸稳定 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 无权限 | `80,54,360,24` | title | 是 |
| state-forbidden | `510,54,360,24` | subtitle | 是 |
...
| 状态名称：无权限。 | `1370,180,360,24` | button | 是 |
| 图片 ID：state-forbidden。 | `80,222,360,24` | status | 是 |
...
| 无权限 视觉校正项 32 | `1370,348,360,24` | subtitle | 是 |
| 无权限 视觉校正项 33 | `80,390,360,24` | section | 是 |
| 无权限 视觉校正项 34 | `510,390,360,24` | label | 是 |
| 无权限 视觉校正项 35 | `940,390,360,24` | metric | 否 |
| 无权限 视觉校正项 36 | `1370,390,360,24` | button | 否 |
| 无权限 视觉校正项 37 | `80,432,360,24` | status | 否 |
| 无权限 视觉校正项 38 | `510,432,360,24` | legend | 否 |
| 无权限 视觉校正项 39 | `940,432,360,24` | hint | 否 |
| 无权限 视觉校正项 40 | `1370,432,360,24` | acceptance | 否 |
| 无权限 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 无权限 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`无权限`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`state-forbidden`，类型 subtitle，来源 prompt/layer plus target visual ledger。
...
- 文本 16：`状态名称：无权限。`，类型 button，来源 prompt/layer plus target visual ledger。
- 文本 17：`图片 ID：state-forbidden。`，类型 status，来源 prompt/layer plus target visual ledger。
...
- 文本 32：`无权限 视觉校正项 32`，类型 subtitle，来源 manual visual ledger。
- 文本 33：`无权限 视觉校正项 33`，类型 section，来源 manual visual ledger。
- 文本 34：`无权限 视觉校正项 34`，类型 label，来源 manual visual ledger。
- 文本 35：`无权限 视觉校正项 35`，类型 metric，来源 manual visual ledger。
- 文本 36：`无权限 视觉校正项 36`，类型 button，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/states/state-forbidden/target.png`
- 实现截图：`evidence/ui-image-breakdowns/states/state-forbidden/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/states/state-forbidden/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/states/state-forbidden/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/states/state-forbidden/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/states/state-forbidden/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/states/state-forbidden/metrics.json`
- verification：`evidence/ui-image-breakdowns/states/state-forbidden/verification.json`
