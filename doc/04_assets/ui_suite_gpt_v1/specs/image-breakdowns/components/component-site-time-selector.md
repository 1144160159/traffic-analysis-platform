# component-site-time-selector.png 逐图精拆记录

## 基本信息

- 分类：components
- 标题：站点与时间选择
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/components/component-site-time-selector.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/component-site-time-selector.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/component-site-time-selector.json`
- 对应路由/宿主路由：无直接业务路由
...
- 业务重点：Documents the 站点与时间选择 component family, visible states, token usage, and implementation split.
...
| 顶部标题区 | `40,34,1840,80` | 2 | 站点与时间选择 | 标题、副标题、图片 ID 和基准说明 |
...
| 图片 ID 标识 | `1470,54,360,32` | 3 | component-site-time-selector | 作为证据文件映射标识 |
...
- 区域 `id-reference`：位置 `1470,54,360,32`。
  用途：component-site-time-selector
  组件：SpecMeta
  视觉要求：作为证据文件映射标识
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 站点与时间选择 | `80,54,360,24` | title | 是 |
| component-site-time-selector | `510,54,360,24` | subtitle | 是 |
| 站点与时间选择。 | `940,54,360,24` | section | 是 |
...
| 组件板名称：站点与时间选择。 | `80,222,360,24` | status | 是 |
| 图片 ID：component-site-time-selector。 | `510,222,360,24` | legend | 是 |
...
- 文本 01：`站点与时间选择`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`component-site-time-selector`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`站点与时间选择。`，类型 section，来源 prompt/layer plus target visual ledger。
...
- 文本 17：`组件板名称：站点与时间选择。`，类型 status，来源 prompt/layer plus target visual ledger。
- 文本 18：`图片 ID：component-site-time-selector。`，类型 legend，来源 prompt/layer plus target visual ledger。
...
| 区域 | 组件/元素 | 实现方式 | 状态 | 备注 |
|---|---|---|---|---|
| `canvas` | `Form` | web/ui/src/components/Form.tsx | default | Form must keep stable dimensions and match the recorded bbox/token mapping. |
| `primary-section` | `ComponentSpecimen` | web/ui/src/components/ComponentSpecimen.tsx | interactive | ComponentSpecimen must keep stable dimensions and match the recorded bbox/token mapping. |
| `primary-section` | `BreadcrumbContext` | web/ui/src/components/BreadcrumbContext.tsx | data-ready | BreadcrumbContext must keep stable dimensions and match the recorded bbox/token mapping. |
| `primary-section` | `Breadcrumb` | web/ui/src/components/Breadcrumb.tsx | default | Breadcrumb must keep stable dimensions and match the recorded bbox/token mapping. |
...
| `state-section` | `ContextChip` | web/ui/src/components/ContextChip.tsx | data-ready | ContextChip must keep stable dimensions and match the recorded bbox/token mapping. |
| `state-section` | `StateMatrix` | web/ui/src/components/StateMatrix.tsx | default | StateMatrix must keep stable dimensions and match the recorded bbox/token mapping. |
| `state-section` | `TokenStrip` | web/ui/src/components/TokenStrip.tsx | interactive | TokenStrip must keep stable dimensions and match the recorded bbox/token mapping. |
...
- 目标图：`evidence/ui-image-breakdowns/components/component-site-time-selector/target.png`
- 实现截图：`evidence/ui-image-breakdowns/components/component-site-time-selector/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/components/component-site-time-selector/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/components/component-site-time-selector/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/components/component-site-time-selector/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/components/component-site-time-selector/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/components/component-site-time-selector/metrics.json`
- verification：`evidence/ui-image-breakdowns/components/component-site-time-selector/verification.json`
