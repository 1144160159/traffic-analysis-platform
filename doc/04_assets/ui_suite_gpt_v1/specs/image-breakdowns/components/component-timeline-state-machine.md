# component-timeline-state-machine.png 逐图精拆记录

## 基本信息

- 分类：components
- 标题：时间线/状态机
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/components/component-timeline-state-machine.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/component-timeline-state-machine.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/component-timeline-state-machine.json`
- 对应路由/宿主路由：无直接业务路由
...
- 业务重点：Documents the 时间线/状态机 component family, visible states, token usage, and implementation split.
...
| 顶部标题区 | `40,34,1840,80` | 2 | 时间线/状态机 | 标题、副标题、图片 ID 和基准说明 |
...
| 图片 ID 标识 | `1470,54,360,32` | 3 | component-timeline-state-machine | 作为证据文件映射标识 |
...
- 区域 `id-reference`：位置 `1470,54,360,32`。
  用途：component-timeline-state-machine
  组件：SpecMeta
  视觉要求：作为证据文件映射标识
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 时间线/状态机 | `80,54,360,24` | title | 是 |
| component-timeline-state-machine | `510,54,360,24` | subtitle | 是 |
| 时间线/状态机。 | `940,54,360,24` | section | 是 |
...
| 组件板名称：时间线/状态机。 | `80,222,360,24` | status | 是 |
| 图片 ID：component-timeline-state-machine。 | `510,222,360,24` | legend | 是 |
...
- 文本 01：`时间线/状态机`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`component-timeline-state-machine`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`时间线/状态机。`，类型 section，来源 prompt/layer plus target visual ledger。
...
- 文本 17：`组件板名称：时间线/状态机。`，类型 status，来源 prompt/layer plus target visual ledger。
- 文本 18：`图片 ID：component-timeline-state-machine。`，类型 legend，来源 prompt/layer plus target visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/components/component-timeline-state-machine/target.png`
- 实现截图：`evidence/ui-image-breakdowns/components/component-timeline-state-machine/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/components/component-timeline-state-machine/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/components/component-timeline-state-machine/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/components/component-timeline-state-machine/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/components/component-timeline-state-machine/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/components/component-timeline-state-machine/metrics.json`
- verification：`evidence/ui-image-breakdowns/components/component-timeline-state-machine/verification.json`
