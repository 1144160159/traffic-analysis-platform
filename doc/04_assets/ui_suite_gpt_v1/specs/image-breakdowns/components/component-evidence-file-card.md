# component-evidence-file-card.png 逐图精拆记录

## 基本信息

- 分类：components
- 标题：证据文件卡
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/components/component-evidence-file-card.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/component-evidence-file-card.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/component-evidence-file-card.json`
- 对应路由/宿主路由：无直接业务路由
...
- 业务重点：Documents the 证据文件卡 component family, visible states, token usage, and implementation split.
...
| 顶部标题区 | `40,34,1840,80` | 2 | 证据文件卡 | 标题、副标题、图片 ID 和基准说明 |
...
| 图片 ID 标识 | `1470,54,360,32` | 3 | component-evidence-file-card | 作为证据文件映射标识 |
...
- 区域 `id-reference`：位置 `1470,54,360,32`。
  用途：component-evidence-file-card
  组件：SpecMeta
  视觉要求：作为证据文件映射标识
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 证据文件卡 | `80,54,360,24` | title | 是 |
| component-evidence-file-card | `510,54,360,24` | subtitle | 是 |
| 证据文件卡。 | `940,54,360,24` | section | 是 |
...
| 组件板名称：证据文件卡。 | `80,222,360,24` | status | 是 |
| 图片 ID：component-evidence-file-card。 | `510,222,360,24` | legend | 是 |
| 业务样例必须覆盖：PCAP 切片、Session 重建、日志包、图谱路径证据、模型样本、合规报告、SOAR 执行产物、规则测试样本。 | `940,222,360,24` | hint | 是 |
| 组件板必须展示正常、悬停、选中、禁用、加载、错误或危险等关键状态，必要时展示尺寸、间距、颜色和交互语义。 | `1370,222,360,24` | acceptance | 是 |
| 组件内容必须贴合全流量采集分析系统，例如告警、资产、证据、规则、模型、审计、采集链路、数据质量和响应动作。 | `80,264,360,24` | title | 是 |
| 组件要能被前端拆成 React + Ant Design + ECharts 组件，不要只做装饰图。 | `510,264,360,24` | subtitle | 是 |
| 输出要求：只输出 UI 视觉效果图本身，不要额外解释，不要水印，不要伪造浏览器地址栏。画面需要能作为前端开发和 Figma 设计参考。 | `940,264,360,24` | section | 是 |
| 01 主样例：业务内容区上下文条 | `1370,264,360,24` | label | 是 |
| 02 结构与状态 | `80,306,360,24` | metric | 是 |
| 03 上下文 Chip 组合 | `510,306,360,24` | button | 是 |
| 04 职责边界 | `940,306,360,24` | status | 是 |
| 05 可实现拆分 | `1370,306,360,24` | legend | 是 |
| 告警中心 | `80,348,360,24` | hint | 是 |
| 高危告警 | `510,348,360,24` | acceptance | 是 |
| AL-20260620-000123 | `940,348,360,24` | title | 是 |
| 园区边界出口 | `1370,348,360,24` | subtitle | 是 |
| 影响资产 32 | `80,390,360,24` | section | 是 |
| 证据链完整 | `510,390,360,24` | label | 是 |
| 返回上一级 | `940,390,360,24` | metric | 否 |
| 复制路径 | `1370,390,360,24` | button | 否 |
| 刷新上下文 | `80,432,360,24` | status | 否 |
| 默认态 | `510,432,360,24` | legend | 否 |
| 悬停态 | `940,432,360,24` | hint | 否 |
| 选中态 | `1370,432,360,24` | acceptance | 否 |
| 禁用态 | `80,474,360,24` | title | 否 |
| 加载态 | `510,474,360,24` | subtitle | 否 |
| 错误态 | `940,474,360,24` | section | 否 |
| Breadcrumb | `1370,474,360,24` | label | 否 |
| Context Bar | `80,516,360,24` | metric | 否 |
| Status Chip | `510,516,360,24` | button | 否 |
| Object Summary | `940,516,360,24` | status | 否 |
| Action Button | `1370,516,360,24` | legend | 否 |
| 验收口径：面包屑与上下文条只能解释当前位置和对象上下文，不得重复顶部站点/时间、用户/通知或左侧导航。 | `80,558,360,24` | hint | 否 |

### 文本人工校正说明

- 文本 01：`证据文件卡`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`component-evidence-file-card`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`证据文件卡。`，类型 section，来源 prompt/layer plus target visual ledger。
...
- 文本 17：`组件板名称：证据文件卡。`，类型 status，来源 prompt/layer plus target visual ledger。
- 文本 18：`图片 ID：component-evidence-file-card。`，类型 legend，来源 prompt/layer plus target visual ledger。
- 文本 19：`业务样例必须覆盖：PCAP 切片、Session 重建、日志包、图谱路径证据、模型样本、合规报告、SOAR 执行产物、规则测试样本。`，类型 hint，来源 manual visual ledger。
- 文本 20：`组件板必须展示正常、悬停、选中、禁用、加载、错误或危险等关键状态，必要时展示尺寸、间距、颜色和交互语义。`，类型 acceptance，来源 manual visual ledger。
- 文本 21：`组件内容必须贴合全流量采集分析系统，例如告警、资产、证据、规则、模型、审计、采集链路、数据质量和响应动作。`，类型 title，来源 manual visual ledger。
- 文本 22：`组件要能被前端拆成 React + Ant Design + ECharts 组件，不要只做装饰图。`，类型 subtitle，来源 manual visual ledger。
- 文本 23：`输出要求：只输出 UI 视觉效果图本身，不要额外解释，不要水印，不要伪造浏览器地址栏。画面需要能作为前端开发和 Figma 设计参考。`，类型 section，来源 manual visual ledger。
- 文本 24：`01 主样例：业务内容区上下文条`，类型 label，来源 manual visual ledger。
- 文本 25：`02 结构与状态`，类型 metric，来源 manual visual ledger。
- 文本 26：`03 上下文 Chip 组合`，类型 button，来源 manual visual ledger。
- 文本 27：`04 职责边界`，类型 status，来源 manual visual ledger。
- 文本 28：`05 可实现拆分`，类型 legend，来源 manual visual ledger。
- 文本 29：`告警中心`，类型 hint，来源 manual visual ledger。
- 文本 30：`高危告警`，类型 acceptance，来源 manual visual ledger。
- 文本 31：`AL-20260620-000123`，类型 title，来源 manual visual ledger。
- 文本 32：`园区边界出口`，类型 subtitle，来源 manual visual ledger。
- 文本 33：`影响资产 32`，类型 section，来源 manual visual ledger。
- 文本 34：`证据链完整`，类型 label，来源 manual visual ledger。
- 文本 35：`返回上一级`，类型 metric，来源 manual visual ledger。
- 文本 36：`复制路径`，类型 button，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/components/component-evidence-file-card/target.png`
- 实现截图：`evidence/ui-image-breakdowns/components/component-evidence-file-card/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/components/component-evidence-file-card/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/components/component-evidence-file-card/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/components/component-evidence-file-card/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/components/component-evidence-file-card/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/components/component-evidence-file-card/metrics.json`
- verification：`evidence/ui-image-breakdowns/components/component-evidence-file-card/verification.json`
