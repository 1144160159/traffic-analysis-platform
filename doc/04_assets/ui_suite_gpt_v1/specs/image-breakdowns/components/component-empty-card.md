# component-empty-card.png 逐图精拆记录

## 基本信息

- 分类：components
- 标题：空状态卡
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/components/component-empty-card.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/component-empty-card.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/component-empty-card.json`
- 对应路由/宿主路由：无直接业务路由
...
- 业务重点：Documents the 空状态卡 component family, visible states, token usage, and implementation split.
...
| 顶部标题区 | `40,34,1840,80` | 2 | 空状态卡 | 标题、副标题、图片 ID 和基准说明 |
...
| 图片 ID 标识 | `1470,54,360,32` | 3 | component-empty-card | 作为证据文件映射标识 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 空状态卡 | `80,54,360,24` | title | 是 |
| component-empty-card | `510,54,360,24` | subtitle | 是 |
| 空状态卡。 | `940,54,360,24` | section | 是 |
...
| 组件板名称：空状态卡。 | `80,222,360,24` | status | 是 |
| 图片 ID：component-empty-card。 | `510,222,360,24` | legend | 是 |
...
- 文本 01：`空状态卡`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`component-empty-card`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`空状态卡。`，类型 section，来源 prompt/layer plus target visual ledger。
...
- 文本 17：`组件板名称：空状态卡。`，类型 status，来源 prompt/layer plus target visual ledger。
- 文本 18：`图片 ID：component-empty-card。`，类型 legend，来源 prompt/layer plus target visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/components/component-empty-card/target.png`
- 实现截图：`evidence/ui-image-breakdowns/components/component-empty-card/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/components/component-empty-card/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/components/component-empty-card/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/components/component-empty-card/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/components/component-empty-card/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/components/component-empty-card/metrics.json`
- verification：`evidence/ui-image-breakdowns/components/component-empty-card/verification.json`
