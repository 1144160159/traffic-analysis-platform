# component-data-table.png 逐图精拆记录

## 基本信息

- 分类：components
- 标题：高密度表格
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/components/component-data-table.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/component-data-table.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/component-data-table.json`
- 对应路由/宿主路由：无直接业务路由
...
- 业务重点：Documents the 高密度表格 component family, visible states, token usage, and implementation split.
...
| 顶部标题区 | `40,34,1840,80` | 2 | 高密度表格 | 标题、副标题、图片 ID 和基准说明 |
...
| 图片 ID 标识 | `1470,54,360,32` | 3 | component-data-table | 作为证据文件映射标识 |
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 高密度表格 | `80,54,360,24` | title | 是 |
| component-data-table | `510,54,360,24` | subtitle | 是 |
| 高密度表格。 | `940,54,360,24` | section | 是 |
...
| 组件板名称：高密度表格。 | `80,222,360,24` | status | 是 |
| 图片 ID：component-data-table。 | `510,222,360,24` | legend | 是 |
...
- 文本 01：`高密度表格`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`component-data-table`，类型 subtitle，来源 prompt/layer plus target visual ledger。
- 文本 03：`高密度表格。`，类型 section，来源 prompt/layer plus target visual ledger。
...
- 文本 17：`组件板名称：高密度表格。`，类型 status，来源 prompt/layer plus target visual ledger。
- 文本 18：`图片 ID：component-data-table。`，类型 legend，来源 prompt/layer plus target visual ledger。
...
| 区域 | 组件/元素 | 实现方式 | 状态 | 备注 |
|---|---|---|---|---|
| `canvas` | `Table` | web/ui/src/components/Table.tsx | default | Table must keep stable dimensions and match the recorded bbox/token mapping. |
| `primary-section` | `Pagination` | web/ui/src/components/Pagination.tsx | interactive | Pagination must keep stable dimensions and match the recorded bbox/token mapping. |
| `primary-section` | `Dropdown` | web/ui/src/components/Dropdown.tsx | data-ready | Dropdown must keep stable dimensions and match the recorded bbox/token mapping. |
| `primary-section` | `Tag` | web/ui/src/components/Tag.tsx | default | Tag must keep stable dimensions and match the recorded bbox/token mapping. |
| `state-section` | `ComponentSpecimen` | web/ui/src/components/ComponentSpecimen.tsx | interactive | ComponentSpecimen must keep stable dimensions and match the recorded bbox/token mapping. |
| `state-section` | `BreadcrumbContext` | web/ui/src/components/BreadcrumbContext.tsx | data-ready | BreadcrumbContext must keep stable dimensions and match the recorded bbox/token mapping. |
| `state-section` | `Breadcrumb` | web/ui/src/components/Breadcrumb.tsx | default | Breadcrumb must keep stable dimensions and match the recorded bbox/token mapping. |
| `state-section` | `ContextBar` | web/ui/src/components/ContextBar.tsx | interactive | ContextBar must keep stable dimensions and match the recorded bbox/token mapping. |
| `implementation-section` | `ContextChip` | web/ui/src/components/ContextChip.tsx | data-ready | ContextChip must keep stable dimensions and match the recorded bbox/token mapping. |
| `implementation-section` | `StateMatrix` | web/ui/src/components/StateMatrix.tsx | default | StateMatrix must keep stable dimensions and match the recorded bbox/token mapping. |
| `implementation-section` | `TokenStrip` | web/ui/src/components/TokenStrip.tsx | interactive | TokenStrip must keep stable dimensions and match the recorded bbox/token mapping. |
| `implementation-section` | `AcceptanceStrip` | web/ui/src/components/AcceptanceStrip.tsx | data-ready | AcceptanceStrip must keep stable dimensions and match the recorded bbox/token mapping. |

- `Table` 映射到 web/ui/src/components/Table.tsx。
  状态口径：default
  复核点：Table must keep stable dimensions and match the recorded bbox/token mapping.
- `Pagination` 映射到 web/ui/src/components/Pagination.tsx。
  状态口径：interactive
  复核点：Pagination must keep stable dimensions and match the recorded bbox/token mapping.
...
| `Pagination` | web/ui/src/components/Pagination.tsx |
...
- 目标图：`evidence/ui-image-breakdowns/components/component-data-table/target.png`
- 实现截图：`evidence/ui-image-breakdowns/components/component-data-table/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/components/component-data-table/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/components/component-data-table/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/components/component-data-table/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/components/component-data-table/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/components/component-data-table/metrics.json`
- verification：`evidence/ui-image-breakdowns/components/component-data-table/verification.json`
