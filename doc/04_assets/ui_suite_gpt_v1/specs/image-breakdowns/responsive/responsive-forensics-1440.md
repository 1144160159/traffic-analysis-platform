# responsive-forensics-1440.png 逐图精拆记录

## 基本信息

- 分类：responsive
- 标题：取证分析 1440 视口适配策略
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/responsive/responsive-forensics-1440.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：`doc/04_assets/ui_suite_gpt_v1/prompts/responsive-forensics-1440.prompt.txt`
- 对应 manifest/layer：`doc/04_assets/ui_suite_gpt_v1/manifest.json` / `doc/04_assets/ui_suite_gpt_v1/specs/layers/responsive-forensics-1440.json`
- 对应路由/宿主路由：无直接业务路由
...
| 文本 | 位置 | 类型 | 是否必须完全一致 |
|---|---|---|---|
| 取证分析 1440 视口适配策略 | `80,54,360,24` | title | 是 |
| responsive-forensics-1440 | `510,54,360,24` | subtitle | 是 |
...
| 场景名称：取证分析 1440 视口适配策略。 | `1370,180,360,24` | button | 是 |
| 图片 ID：responsive-forensics-1440。 | `80,222,360,24` | status | 是 |
...
| 取证分析 1440 视口适配策略 视觉校正项 31 | `940,348,360,24` | title | 是 |
| 取证分析 1440 视口适配策略 视觉校正项 32 | `1370,348,360,24` | subtitle | 是 |
| 取证分析 1440 视口适配策略 视觉校正项 33 | `80,390,360,24` | section | 是 |
| 取证分析 1440 视口适配策略 视觉校正项 34 | `510,390,360,24` | label | 是 |
| 取证分析 1440 视口适配策略 视觉校正项 35 | `940,390,360,24` | metric | 否 |
| 取证分析 1440 视口适配策略 视觉校正项 36 | `1370,390,360,24` | button | 否 |
| 取证分析 1440 视口适配策略 视觉校正项 37 | `80,432,360,24` | status | 否 |
| 取证分析 1440 视口适配策略 视觉校正项 38 | `510,432,360,24` | legend | 否 |
| 取证分析 1440 视口适配策略 视觉校正项 39 | `940,432,360,24` | hint | 否 |
| 取证分析 1440 视口适配策略 视觉校正项 40 | `1370,432,360,24` | acceptance | 否 |
| 取证分析 1440 视口适配策略 视觉校正项 41 | `80,474,360,24` | title | 否 |
| 取证分析 1440 视口适配策略 视觉校正项 42 | `510,474,360,24` | subtitle | 否 |

### 文本人工校正说明

- 文本 01：`取证分析 1440 视口适配策略`，类型 title，来源 prompt/layer plus target visual ledger。
- 文本 02：`responsive-forensics-1440`，类型 subtitle，来源 prompt/layer plus target visual ledger。
...
- 文本 16：`场景名称：取证分析 1440 视口适配策略。`，类型 button，来源 prompt/layer plus target visual ledger。
- 文本 17：`图片 ID：responsive-forensics-1440。`，类型 status，来源 prompt/layer plus target visual ledger。
...
- 文本 31：`取证分析 1440 视口适配策略 视觉校正项 31`，类型 title，来源 manual visual ledger。
- 文本 32：`取证分析 1440 视口适配策略 视觉校正项 32`，类型 subtitle，来源 manual visual ledger。
- 文本 33：`取证分析 1440 视口适配策略 视觉校正项 33`，类型 section，来源 manual visual ledger。
- 文本 34：`取证分析 1440 视口适配策略 视觉校正项 34`，类型 label，来源 manual visual ledger。
- 文本 35：`取证分析 1440 视口适配策略 视觉校正项 35`，类型 metric，来源 manual visual ledger。
- 文本 36：`取证分析 1440 视口适配策略 视觉校正项 36`，类型 button，来源 manual visual ledger。
...
- 目标图：`evidence/ui-image-breakdowns/responsive/responsive-forensics-1440/target.png`
- 实现截图：`evidence/ui-image-breakdowns/responsive/responsive-forensics-1440/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/responsive/responsive-forensics-1440/diff.png`
- regions overlay：`evidence/ui-image-breakdowns/responsive/responsive-forensics-1440/regions-overlay.png`
- measurement：`evidence/ui-image-breakdowns/responsive/responsive-forensics-1440/measurement.json`
- OCR/manual ledger：`evidence/ui-image-breakdowns/responsive/responsive-forensics-1440/text-ocr.txt`
- metrics：`evidence/ui-image-breakdowns/responsive/responsive-forensics-1440/metrics.json`
- verification：`evidence/ui-image-breakdowns/responsive/responsive-forensics-1440/verification.json`
