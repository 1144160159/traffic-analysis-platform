# foundation-generation-reference.png 逐图精拆记录

## 基本信息

- 分类：foundations
- 源图：`doc/04_assets/ui_suite_gpt_v1/screens/foundations/foundation-generation-reference.png`
- 源图尺寸：1920 x 1080
- 对应 prompt：无独立 prompt；该图与 `foundation-current-screen-reference.png` 的目标 PNG 像素完全一致，仍以 `screens/pages/screen.png` 为当前态势大屏参考源。
- 对应 manifest：无独立 manifest item；该图用于约束后续 prompt 和页面复刻。
- 对应路由/宿主路由：无。该图是 generation reference 规范板，不是业务路由。
- 当前状态：`pixel-accepted`
- 复刻等级：已完成逐图视觉读取、结构化拆解、独立 Windows Chrome evidence、零容忍 diff、辅助审查和主线程最终判断。
...
- 特殊点：本图与 `foundation-current-screen-reference.png` 的 SHA-256 完全一致，但作为独立 canonical PNG 仍单独建档、单独截图、单独 diff、单独验收；左侧嵌入的态势大屏截图必须被视为第一参考图，不允许后续生成把顶栏高度、侧边栏宽度、底部状态栏坐标按旧模板漂移。
...
- URL：`http://10.0.5.8:35559/evidence/ui-image-breakdowns/foundations/foundation-generation-reference/implementation.html`
- 视口：`1920x1080`
- 目标图：`evidence/ui-image-breakdowns/foundations/foundation-generation-reference/target.png`
- 实现文件：`evidence/ui-image-breakdowns/foundations/foundation-generation-reference/implementation.html`
- 实现截图：`evidence/ui-image-breakdowns/foundations/foundation-generation-reference/implementation.png`
- diff 图：`evidence/ui-image-breakdowns/foundations/foundation-generation-reference/diff.png`
- 区域 overlay：`evidence/ui-image-breakdowns/foundations/foundation-generation-reference/regions-overlay.png`
- verification：`evidence/ui-image-breakdowns/foundations/foundation-generation-reference/verification.json`
- measurement：`evidence/ui-image-breakdowns/foundations/foundation-generation-reference/measurement.json`
- text ledger：`evidence/ui-image-breakdowns/foundations/foundation-generation-reference/text-ocr.txt`
...
- 已关闭：独立 Windows Chrome screenshot、视觉 diff、区域 overlay、verification、辅助智能体审查和主线程判定。
- 范围说明：该图虽然与第二张像素相同，但已独立完成 evidence 和主线程判定；生产 React 组件语义实现仍需依据本拆解记录落地。
