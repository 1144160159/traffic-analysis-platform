#!/usr/bin/env python3
"""Validate the M09 N005 encrypted adapter/plan production bundle in Kubernetes."""
...
BUNDLE_CHECK = " && ".join(
    (
        ...
        "grep -R -q '/v1/encrypted-traffic/stats' /usr/share/nginx/html/assets",
        "grep -R -q 'Encrypted Stats API' /usr/share/nginx/html/assets",
        "grep -R -q '未生成任何替代数据' /usr/share/nginx/html/assets",
        "echo PASS",
    )
)
...
def main() -> None:
    ...
    job_name = f"m09-n005-adapter-{suffix}"
    applied = False
    logs = ""
    evidence = None
    try:
        job = base.build_job(job_name, args.image, args.run_id, args.node)
        labels = {**job["metadata"]["labels"], "app.kubernetes.io/name": "m09-encrypted-adapter-plan-canary"}
    ...
    envelope = {
        "artifact_kind": "M09_ENCRYPTED_ADAPTER_PLAN_TEST_RESULT",
        "task_id": "T1-M09-N005",
        "run_id": args.run_id,
        "status": "PASS",
