#!/usr/bin/env python3
"""Synchronize the M07 campaign dual-rail migration into the K8s PG init job."""
...
MIGRATION = Path("deployments/postgres/migrations/202608101030_campaign_rail_correlation_v1.sql")
K8S_ENTRYPOINT = Path("deployments/kubernetes/init-jobs/02-postgres-schema.yaml")
K8S_KEY = "44-m07-campaign-rail-correlation-v1.sql"
PREVIOUS_KEY = "43-m07-attack-chain-snapshot-v1.sql"
BEGIN_MARKER = "  # BEGIN GENERATED T1-M07 CAMPAIGN RAIL CORRELATION V1\n"
END_MARKER = "  # END GENERATED T1-M07 CAMPAIGN RAIL CORRELATION V1"
...
def synchronize(source: str, block: str) -> str:
    start, end = source.find(BEGIN_MARKER), source.find(END_MARKER)
    if (start < 0) != (end < 0):
        raise ValueError("M07 campaign-rail Kubernetes generated block has only one marker")
    if start < 0:
        ...
        if end < start:
            raise ValueError("M07 campaign-rail Kubernetes block end precedes begin")
        end += len(END_MARKER)
    ...
    if names.count(K8S_KEY) > 1:
        raise ValueError("M07 campaign-rail migration is duplicated in Kubernetes runner")
    if K8S_KEY not in names:
        if not names or names[-1] != PREVIOUS_KEY:
            raise ValueError(f"M07 campaign-rail migration must append after {PREVIOUS_KEY}")
...
def main() -> int:
    ...
    if args.check:
        if current != expected:
            print(f"{K8S_ENTRYPOINT}: generated M07 campaign-rail block is missing or stale", file=sys.stderr)
            return 1
        return 0
    if current != expected:
