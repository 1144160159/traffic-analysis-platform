#!/usr/bin/env python3
"""Freeze the M01 early-trust v2 design candidate from one clean Git HEAD.

The design candidate contains only Git blobs owned by v2 leaves whose target
state is PLANNED.  PLANNED_OUTPUT evidence, reviews, external image artifacts,
the implementation-candidate manifest, authorization, and registry switching
remain later stages and are never created by this command.
"""
...
CATALOG_REL = "contracts/alignment/m01-early-trust-train-catalog.v2.json"
CATALOG_SCHEMA_REL = "contracts/alignment/m01-early-trust-train-catalog.v2.schema.json"
MANIFEST_SCHEMA_REL = "contracts/alignment/design-candidate-manifest.schema.json"
OUTPUT = "doc/02_acceptance/topic1/m01/candidates/m01-early-trust-v2/design-candidate-manifest.json"
CANDIDATE_ID = "DESIGN-T1-M01-EARLY-TRUST-V2"
REQUIRED_SOURCE_COUNT = 81
REQUIRED_SOURCE_EXACT_SET_SHA256 = "f8425368b56964bab900b492b0842fa7a4f2f7ab0ebe75abd3e67267a0241d5a"
...
def required_sources(root: Path) -> list[str]:
    catalog_path = safe_existing_file(root, CATALOG_REL)
    catalog = json.loads(catalog_path.read_text(encoding="utf-8"))
    validate_against_schema(catalog, safe_existing_file(root, CATALOG_SCHEMA_REL))
    sources = sorted(
        {
            locator.split("#", 1)[0]
            for leaf in catalog["candidate_leaves"]
            if leaf["target_state"] == "PLANNED"
    ...
    if len(sources) != REQUIRED_SOURCE_COUNT:
        raise ValueError(
            f"M01 required source count drifted: expected {REQUIRED_SOURCE_COUNT}, got {len(sources)}"
        )
    if semantic_sha256(sources) != REQUIRED_SOURCE_EXACT_SET_SHA256:
        raise ValueError("M01 required source path exact-set hash drifted")
    return sources
...
def ensure_clean(root: Path) -> None:
    ...
    if completed.stdout:
        ...
        preview = ", ".join(rows[:5])
        raise ValueError(
            "candidate worktree contains uncommitted tracked or untracked changes "
            f"outside the manifest: {preview}"
        )
...
def build(root: Path, candidate_commit: str) -> dict[str, Any]:
    ...
    blobs: dict[str, str] = {}
    for relative in required_sources(root):
    ...
    payload = {
        ...
        "scope": "T1-M01 early trust v2 exact 81 planned-source design candidate",
    ...
    validate_against_schema(payload, safe_existing_file(root, MANIFEST_SCHEMA_REL))
    return payload
...
def main() -> int:
    ...
    if args.write:
        ...
        if not target.is_file() or target.is_symlink() or target.read_bytes() != body:
            raise ValueError("M01 design candidate manifest is missing or differs from clean Git HEAD")
        print(f"PASS {OUTPUT}")
    print(
        f"candidate_commit={args.candidate_commit} "
        f"source_count={len(payload['source_blob_sha256'])}"
    )
