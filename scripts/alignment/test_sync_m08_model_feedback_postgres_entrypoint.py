...
MODULE_PATH = Path(__file__).with_name(
    "sync_m08_model_feedback_postgres_entrypoint.py"
)
SPEC = importlib.util.spec_from_file_location(
    "sync_m08_model_feedback_postgres_entrypoint", MODULE_PATH
)
...
class M08ModelFeedbackPostgresSyncTest(unittest.TestCase):
    def setUp(self) -> None:
        self.block = (
            MODULE.BEGIN_MARKER
            + f"  {MODULE.K8S_KEY}: |\n    SELECT 45;\n"
        ...
        self.source = (
            ...
            "---\napiVersion: batch/v1\nkind: Job\nspec:\n  template:\n"
            "    spec:\n      containers:\n      - args:\n        - |\n"
            "          for f in 43-m07-attack-chain-snapshot-v1.sql "
            "44-m07-campaign-rail-correlation-v1.sql; do\n"
            "            echo $f\n          done\n"
        )
    ...
    def test_append_is_ordered_and_idempotent(self) -> None:
        ...
        self.assertIn(
            "44-m07-campaign-rail-correlation-v1.sql "
            "45-m08-model-feedback-revision-inbox-v1.sql; do",
            rendered,
        )
    ...
    def test_out_of_order_append_is_rejected(self) -> None:
        source = self.source.replace(
            "44-m07-campaign-rail-correlation-v1.sql; do",
            "44-m07-campaign-rail-correlation-v1.sql future.sql; do",
        )
