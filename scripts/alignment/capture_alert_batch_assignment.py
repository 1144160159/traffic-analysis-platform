...
SOURCE_ARTIFACTS = (
    ...
    "go/control-plane/internal/alert/api/alert_batch_assignment_compensation_v1.go",
    ...
    "go/control-plane/internal/alert/consumer/alert_batch_assignment_pipeline.go",
    "go/control-plane/internal/alert/consumer/alert_batch_assignment_compensation_pipeline.go",
    "go/control-plane/internal/alert/consumer/alert_batch_assignment_pipeline_test.go",
    "go/control-plane/internal/alert/consumer/alert_batch_assignment_pipeline_postgres_integration_test.go",
    ...
    "scripts/alignment/sync_alert_batch_assignment_execution_entrypoints.py",
    "scripts/alignment/sync_alert_batch_assignment_compensation_entrypoints.py",
...
def main() -> int:
    ...
    manifest = {
        ...
        "coverage_status": "PARTIAL_OWNED_REAL_POSTGRES_ASSIGNMENT_AND_COMPENSATION_PIPELINE_WITH_FAKE_CLICKHOUSE_AUTHORITY_G1",
        ...
        "gate_status": {
            ...
            "G1": "PASS_FOR_OWNED_SENTINEL_PROTECTED_POSTGRES_ACCEPTANCE_INDEPENDENT_CONSUMER_TERMINAL_RECEIPTS_AND_DLQ_BARRIER_WITH_FAKE_CLICKHOUSE_AUTHORITY",
        ...
        "proven": [
            ...
            "requested and changed events use a strict canonical envelope and an exact independently idempotent PostgreSQL inbox",
            "changed events are rejected before external projection unless their canonical outbox identity and complete projecting item set match PostgreSQL",
            "assignee activity scope and each expected alert revision are rechecked before deterministic projection intent is committed",
            "terminal per-item projection receipts batch accounting history and audit commit atomically, and exact Kafka redelivery does not repeat projection",
            "a permanent poison event requires canonical DLQ acknowledgement followed by an idempotent PostgreSQL source-tuple and audit barrier before source offset commit",
            "compensation accepts one revision-bound aggregate per terminal batch, restores only still-authoritative applied items at a new version and records exact replay receipts without blind overwrite",
        ...
        "open": [
            ...
            "replace the G1 fake ClickHouse authority with fixed-digest real Kafka and ClickHouse components, then prove source-offset retention and exact target-version replay under faults",
            "materialize and reconcile the OpenSearch assignment projection instead of treating ClickHouse completion as cross-read-model completion",
            ...
            "replace the G1 fake ClickHouse compensation authority with fixed-digest real Kafka and ClickHouse execution, then reconcile the inverse projection across stores",
