#!/usr/bin/env python3
"""Run M09-N016 report jobs against existing K8s PostgreSQL and MinIO."""
...
DEFAULT_OUTPUT = (
    ROOT / "doc/02_acceptance/topic1/tasks/t1-m09-n016"
    / "k8s-alert-report-latest.json"
)
IMAGE_RE = re.compile(r"^[a-zA-Z0-9][a-zA-Z0-9._/@:-]+$")
NODE_RE = re.compile(r"^[a-z0-9]([-a-z0-9.]*[a-z0-9])?$")
INTEGRATION_TEST = "^TestAlertReportK8sPostgresMinIOIntegration$"
CLEANUP_TEST = "^TestAlertReportK8sCleanupOracle$"
UNIT_TEST = "^Test(AlertReportManifestExposesExpiryAndFailsClosedAfterDeadline|AlertReportFreshManifestKeepsBoundedDownloadAuthority|DownloadAlertReportRejectsExpiredArtifactBeforeObjectAccess|AlertReportWorkerUploadsArtifactThenCommitsManifestAndCompletionEvent|AlertReportCancellationCleanupRemovesExactObjectBeforeTerminalState)$"
BUNDLE_CHECK = " && ".join((
    ...
    "grep -R -q '/reports/export' /usr/share/nginx/html/assets",
    "grep -R -q 'manifest_version' /usr/share/nginx/html/assets",
    "grep -R -q '报告对象下载权限已过期' /usr/share/nginx/html/assets",
    "grep -R -q '任务、冻结快照、水位和 manifest 仍可查询' /usr/share/nginx/html/assets",
    "echo PASS",
))
...
def labels(run_id: str) -> dict[str, str]:
    return {
        "app.kubernetes.io/name": "m09-alert-report-canary",
...
def report_env(suffix: str) -> list[dict[str, Any]]:
    return [
        {"name": "ALERT_REPORT_K8S_INTEGRATION", "value": "run-scoped-only"},
        {"name": "ALERT_REPORT_K8S_SUFFIX", "value": suffix},
        {"name": "ALERT_REPORT_K8S_PG_HOST", "value": "postgres-primary.databases.svc"},
        {"name": "ALERT_REPORT_K8S_PG_PASSWORD", "valueFrom": {"secretKeyRef": {
            "name": "traffic-credentials", "key": "PG_PASSWORD",
        }}},
        {"name": "ALERT_REPORT_K8S_MINIO_ENDPOINT", "value": "minio.minio.svc:9000"},
        {"name": "ALERT_REPORT_K8S_MINIO_ACCESS_KEY", "valueFrom": {"secretKeyRef": {
            "name": "traffic-credentials", "key": "MINIO_ACCESS_KEY",
        }}},
        {"name": "ALERT_REPORT_K8S_MINIO_SECRET_KEY", "valueFrom": {"secretKeyRef": {
            "name": "traffic-credentials", "key": "MINIO_SECRET_KEY",
        }}},
    ]
...
def test_job(name: str, run_id: str, suffix: str, node: str, image: str, suite: str) -> dict[str, Any]:
    ...
    if suite in ("integration", "cleanup"):
        ...
        container["env"] = report_env(suffix)
    elif suite == "unit":
    ...
    return {
        ...
        "metadata": {
            ...
            "annotations": {
                "traffic.analysis/shared-postgres-minio-touched": str(suite == "integration").lower(),
...
def main() -> None:
    ...
    source_paths = [
        ...
        ROOT / "go/control-plane/internal/alert/api/handler_alert_reports.go",
        ...
        ROOT / "contracts/alignment/features/F-ALERT-001.json",
        ROOT / "deployments/kubernetes/applications/go-services.yaml",
    ]
    envelope = {
        "artifact_kind": "M09_ALERT_REPORT_TEST_RESULT",
        "task_id": "T1-M09-N016",
        "run_id": args.run_id,
        "status": "PASS",
        "profile_id": "M09-N016-K8S-POSTGRES-MINIO-UI-V1",
        "coverage_status": "PASS_FOR_RUN_SCOPED_K8S_POSTGRES_MINIO_AND_IMMUTABLE_UI_BUNDLE",
        "inputs": {
        ...
        "frozen_snapshot_and_source_watermarks": True,
        "versioned_object_manifest": True,
        "same_key_retry_single_object": True,
        "cooperative_cancel_removes_exact_object": True,
        "expired_job_queryable_without_download_authority": True,
        "expired_download_fails_closed": True,
        "tenant_isolation": True,
        "shared_postgres_and_minio_touched": True,
        "run_scoped_postgres_rows_and_bucket_removed": True,
        "mock_enabled": False,
        "backend_runtime_flag_enabled_in_candidate_manifest": False,
        "production_applied": False,
        "run_scoped_kubernetes_resources_removed": not args.keep,
        "does_not_prove": [
            "production rollout", "large-report queue performance", "Windows Chrome acceptance",
            "Kafka outbox publication ACK", "global milestone completion",
            "authorization to enable ALERT_REPORT_JOBS_V1_ENABLED",
        ],
    }
