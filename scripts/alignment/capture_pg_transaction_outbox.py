...
def main() -> int:
    ...
    manifest = {
        ...
        "proven": [
            ...
            "the default-off dashboard outbox dispatcher publishes only before marking the leased row published, while its idempotent Kafka inbox drives leased execution and compensation attempts to provider-backed terminal receipts",
            "lost execution and compensation responses are reconciled through optional provider authority lookups; the resulting receipt, history, audit, result outbox and attempt completion commit atomically",
            "after a canonical dlq.v1 broker acknowledgement, the shared consumer invokes a dashboard-owned PostgreSQL barrier that atomically records one source-tuple receipt and quarantine audit before the source offset can commit",
        ...
        "open": [
            ...
            "operate the default-off dashboard task dispatcher consumer executor compensator authority lookup and DLQ barrier against one approved release candidate and reconcile PG Kafka provider and audit watermarks",
            "prove the dashboard poison-event barrier with replicated release-candidate Kafka and PostgreSQL failure restart, including DLQ target partition and offset reconciliation when exposed by the broker client",
            "apply the additive migrations through 202608090945 and topic/ACL expansion to an approved candidate environment",
