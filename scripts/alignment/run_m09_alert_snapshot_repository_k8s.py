#!/usr/bin/env python3
"""Run the M09 N006 alert snapshot repository contract tests in Kubernetes."""
...
TEST_PATTERN = "Test(AlertSnapshot|AlertSearchProjection|PostgresAlertSnapshot)"
...
def main() -> None:
    ...
    job_name = f"m09-n006-snapshot-{suffix}"
    applied = False
    logs = ""
    evidence = None
    try:
        ...
        labels = {
            **job["metadata"]["labels"],
            "app.kubernetes.io/name": "m09-alert-snapshot-repository-canary",
        }
    ...
    if evidence is None:
        raise base.CanaryError("Kubernetes alert snapshot repository validation produced no result")

    envelope = {
        "artifact_kind": "M09_ALERT_SNAPSHOT_REPOSITORY_TEST_RESULT",
        "task_id": "T1-M09-N006",
        "run_id": args.run_id,
        "status": "PASS",
