#!/usr/bin/env python3
"""Synchronize the M07 attack-chain snapshot migration into the K8s PG init job."""
...
MIGRATION = Path("deployments/postgres/migrations/202608142100_m07_attack_chain_snapshot_v1.sql")
K8S_ENTRYPOINT = Path("deployments/kubernetes/init-jobs/02-postgres-schema.yaml")
K8S_KEY = "43-m07-attack-chain-snapshot-v1.sql"
PREVIOUS_KEY = "42-m07-graph-reconcile-v1.sql"
BEGIN_MARKER = "  # BEGIN GENERATED T1-M07 ATTACK CHAIN SNAPSHOT V1\n"
END_MARKER = "  # END GENERATED T1-M07 ATTACK CHAIN SNAPSHOT V1"
...
def synchronize(source: str, block: str) -> str:
    ...
    if (start < 0) != (end < 0):
        raise ValueError("M07 attack-chain Kubernetes generated block has only one marker")
    if start < 0:
        ...
        if end < start:
            raise ValueError("M07 attack-chain Kubernetes block end precedes begin")
        end += len(END_MARKER)
    ...
    if names.count(K8S_KEY) > 1:
        raise ValueError("M07 attack-chain migration is duplicated in Kubernetes runner")
    if K8S_KEY not in names:
        if not names or names[-1] != PREVIOUS_KEY:
            raise ValueError(f"M07 attack-chain migration must append after {PREVIOUS_KEY}")
...
def main() -> int:
    ...
    if args.check:
        if current != expected:
            print(f"{K8S_ENTRYPOINT}: generated M07 attack-chain block is missing or stale", file=sys.stderr)
            return 1
        return 0
    if current != expected:
