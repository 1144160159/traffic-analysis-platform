#!/usr/bin/env python3
"""Synchronize the N016 rollback-v2 migration into the K8s PG init Job."""
...
K8S_KEY = "47-m08-model-rollback-v2.sql"
PREVIOUS_KEY = "46-m08-model-drift-candidate-v1.sql"
BEGIN_MARKER = "  # BEGIN GENERATED T1-M08 MODEL ROLLBACK V2\n"
END_MARKER = "  # END GENERATED T1-M08 MODEL ROLLBACK V2"
...
def synchronize(source: str, block: str) -> str:
    ...
    if (start < 0) != (end < 0):
        raise ValueError("M08 model-rollback Kubernetes block has only one marker")
    if start < 0:
        ...
        if end < start:
            raise ValueError("M08 model-rollback Kubernetes block end precedes begin")
        end += len(END_MARKER)
    ...
    if names.count(K8S_KEY) > 1:
        raise ValueError("M08 model-rollback migration is duplicated in Kubernetes runner")
    if K8S_KEY not in names:
        if not names or names[-1] != PREVIOUS_KEY:
            raise ValueError(f"M08 model-rollback migration must append after {PREVIOUS_KEY}")
...
def main() -> int:
    ...
    if args.check:
        if current != expected:
            print(
                f"{K8S_ENTRYPOINT}: generated M08 model-rollback block is missing or stale",
                file=sys.stderr,
            )
            return 1
        return 0
