...
-- F-ALERT-005 tenant-bound immutable alert evidence manifest v1.
BEGIN;
...
CREATE TABLE IF NOT EXISTS alert_evidence_manifests (
  ...
  event_id TEXT NOT NULL DEFAULT '', evidence_type TEXT NOT NULL CHECK(evidence_type<>''), source_store TEXT NOT NULL CHECK(source_store IN ('postgresql','clickhouse','opensearch','minio','arkime')),
  ...
  size_bytes BIGINT NOT NULL DEFAULT 0 CHECK(size_bytes>=0), content_type TEXT NOT NULL DEFAULT '', state TEXT NOT NULL DEFAULT 'available' CHECK(state IN ('available','missing','expired','unavailable','integrity_failed')),
  ...
  PRIMARY KEY(tenant_id,alert_id,evidence_id), CHECK(source_store<>'minio' OR (object_bucket<>'' AND object_key LIKE ('tenants/'||tenant_id||'/%') AND object_key NOT LIKE '%..%' AND object_sha256 ~ '^[0-9a-f]{64}$'))
);
...
CREATE TABLE IF NOT EXISTS alert_evidence_manifest_history (
  tenant_id TEXT NOT NULL, alert_id TEXT NOT NULL, evidence_id TEXT NOT NULL, event_id TEXT NOT NULL DEFAULT '', evidence_type TEXT NOT NULL, source_store TEXT NOT NULL,
  object_bucket TEXT NOT NULL DEFAULT '', object_key TEXT NOT NULL DEFAULT '', object_version TEXT NOT NULL DEFAULT '', object_sha256 TEXT NOT NULL DEFAULT '', size_bytes BIGINT NOT NULL,
  content_type TEXT NOT NULL DEFAULT '', state TEXT NOT NULL, revision BIGINT NOT NULL CHECK(revision>0), source_watermarks JSONB NOT NULL, observed_at TIMESTAMPTZ NOT NULL,
  expires_at TIMESTAMPTZ, recorded_at TIMESTAMPTZ NOT NULL DEFAULT now(), PRIMARY KEY(tenant_id,alert_id,evidence_id,revision)
);
...
-- BEGIN GENERATED F-DASHBOARD-002 DLQ RECEIPT
BEGIN;
...
ON CONFLICT (version) DO NOTHING;
COMMIT;
-- END GENERATED F-DASHBOARD-002 DLQ RECEIPT
...
SET provider=CASE WHEN receipt.simulated THEN 'internal-simulation' ELSE 'legacy-approval-guard' END,
    ...
    trace_id=action.trace_id, receipt_sha256=repeat('0',64), executed_at=receipt.created_at
FROM alert_response_actions action WHERE action.job_id=receipt.job_id
...
-- Canonical bootstrap counterpart of migration 202608091230_alert_response_dlq_receipt_v1.sql.
BEGIN;
...
BEGIN;
ALTER TABLE alert_response_control_requests DROP CONSTRAINT IF EXISTS alert_response_control_requests_state_check;
ALTER TABLE alert_response_control_requests ADD CONSTRAINT alert_response_control_requests_state_check CHECK (state IN ('cancelled','blocked_external_executor','queued'));
CREATE UNIQUE INDEX IF NOT EXISTS uq_alert_response_actions_tenant_idempotency ON alert_response_actions(tenant_id,idempotency_key) WHERE idempotency_key<>'';
CREATE TABLE IF NOT EXISTS alert_response_execution_authority_rechecks (
  ...
  tenant_id TEXT NOT NULL, trace_id TEXT NOT NULL,
  status TEXT NOT NULL CHECK (status IN ('pending','checking','resolved','exhausted')),
  attempts INTEGER NOT NULL DEFAULT 0 CHECK (attempts >= 0),
  max_attempts INTEGER NOT NULL CHECK (max_attempts BETWEEN 1 AND 100),
  next_attempt_at TIMESTAMPTZ NOT NULL, locked_until TIMESTAMPTZ,
  locked_by TEXT NOT NULL DEFAULT '', last_authority_state TEXT NOT NULL DEFAULT '',
...
CREATE INDEX IF NOT EXISTS idx_alert_response_execution_rechecks_due ON alert_response_execution_authority_rechecks(next_attempt_at,recheck_id) WHERE status IN ('pending','checking');
CREATE TABLE IF NOT EXISTS alert_response_compensation_attempts (
  ...
  job_id TEXT NOT NULL REFERENCES alert_response_actions(job_id) ON DELETE RESTRICT,
  tenant_id TEXT NOT NULL, alert_id TEXT NOT NULL, original_action_id TEXT NOT NULL,
  compensation_action_id TEXT NOT NULL, original_provider TEXT NOT NULL,
  ...
  requested_by TEXT NOT NULL, reason TEXT NOT NULL, trace_id TEXT NOT NULL,
  aggregate_version BIGINT NOT NULL CHECK (aggregate_version>0),
...
CREATE INDEX IF NOT EXISTS idx_alert_response_compensation_due ON alert_response_compensation_attempts(next_attempt_at,request_id) WHERE status IN ('pending','executing','authority_pending');
CREATE INDEX IF NOT EXISTS idx_alert_response_compensation_tenant_job ON alert_response_compensation_attempts(tenant_id,job_id,created_at DESC);
CREATE TABLE IF NOT EXISTS alert_response_compensation_receipts (
  ...
  event_id UUID NOT NULL, job_id TEXT NOT NULL, tenant_id TEXT NOT NULL,
  ...
  result JSONB NOT NULL DEFAULT '{}'::jsonb, error_code TEXT NOT NULL DEFAULT '',
  error_message TEXT NOT NULL DEFAULT '', receipt_sha256 TEXT NOT NULL CHECK (receipt_sha256 ~ '^[0-9a-f]{64}$'),
  authority_lookup JSONB NOT NULL DEFAULT '{}'::jsonb, trace_id TEXT NOT NULL,
  compensated_at TIMESTAMPTZ NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE(provider,provider_receipt_id)
);
CREATE INDEX IF NOT EXISTS idx_alert_response_compensation_receipts_trace ON alert_response_compensation_receipts(tenant_id,trace_id,compensated_at DESC);
CREATE TABLE IF NOT EXISTS alert_response_authority_check_history (
  ...
  subject_id TEXT NOT NULL, attempt INTEGER NOT NULL CHECK (attempt>0),
  tenant_id TEXT NOT NULL, job_id TEXT NOT NULL, trace_id TEXT NOT NULL,
  provider TEXT NOT NULL DEFAULT '', authority_state TEXT NOT NULL,
  error_code TEXT NOT NULL DEFAULT '', error_message TEXT NOT NULL DEFAULT '',
  detail JSONB NOT NULL DEFAULT '{}'::jsonb, checked_at TIMESTAMPTZ NOT NULL,
  PRIMARY KEY(subject_type,subject_id,attempt)
);
CREATE INDEX IF NOT EXISTS idx_alert_response_authority_history_trace ON alert_response_authority_check_history(tenant_id,trace_id,checked_at DESC);
INSERT INTO alignment_schema_migrations(version,description)
VALUES ('202608091500','bounded alert response authority reconciliation and external compensation')
ON CONFLICT (version) DO NOTHING;
COMMIT;

-- BEGIN GENERATED F-ALERT-004 ASSIGNMENT EXECUTION V1
...
COMMIT;
-- END GENERATED F-ALERT-004 ASSIGNMENT EXECUTION V1
-- BEGIN GENERATED F-ALERT-004 ASSIGNMENT COMPENSATION V1
