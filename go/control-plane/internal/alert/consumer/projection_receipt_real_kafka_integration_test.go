...
func TestAlertProjectionReceiptRealKafka(t *testing.T) {
	...
	defer writer.Close()
	if err := writer.WriteMessages(ctx, segmentkafka.Message{
		Topic: topic, Key: []byte(alertProjectionKafkaTenant + ":" + alertProjectionKafkaEvent), Value: payload,
		Headers: []segmentkafka.Header{
			{Key: "tenant_id", Value: []byte(alertProjectionKafkaTenant)},
			{Key: "event_id", Value: []byte(alertProjectionKafkaEvent)},
	...
	t.Logf("PASS_ALERT_PROJECTION_REAL_KAFKA_RECEIPT event_id=%s committed_offset=%d lag=%d source_sha256=%s",
		lastEvent, lag.TotalCommittedOffset, lag.TotalLag, sourceHash)
}
