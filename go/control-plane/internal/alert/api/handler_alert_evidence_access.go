...
type alertEvidenceObjectStore interface {
	Stat(context.Context, string, string) (alertEvidenceObjectInfo, error)
...
func (h *Handler) CreateAlertEvidenceAccess(w http.ResponseWriter, r *http.Request) {
	...
	objectInfo, err := objectStore.Stat(ctx, objectRef.Bucket, objectRef.Key)
	if err != nil {
	...
	secret := alertEvidenceDownloadSecret()
	if secret == "" {
	...
	signature := signAlertEvidenceDownload(secret, tenantID, alertID, evidence.EvidenceID, expires)
	downloadURL := fmt.Sprintf(
		"/v1/alerts/%s/evidence/%s/download?expires=%d&signature=%s",
		url.PathEscape(alertID),
		url.PathEscape(evidence.EvidenceID),
		expires,
...
func (h *Handler) DownloadAlertEvidence(w http.ResponseWriter, r *http.Request) {
	...
	reader, err := objectStore.Open(ctx, objectRef.Bucket, objectRef.Key)
	if err != nil {
...
func (store *minioAlertEvidenceObjectStore) Stat(ctx context.Context, bucket, key string) (alertEvidenceObjectInfo, error) {
	info, err := store.client.StatObject(ctx, bucket, key, minio.StatObjectOptions{})
	if err != nil {
		return alertEvidenceObjectInfo{}, err
	}
	return alertEvidenceObjectInfo{Size: info.Size, ContentType: info.ContentType}, nil
}

func (store *minioAlertEvidenceObjectStore) Open(ctx context.Context, bucket, key string) (io.ReadCloser, error) {
